...
def create_http_app() -> FastAPI:
    """创建 FastAPI 应用（工厂函数）"""
    app = FastAPI(title='CampusWorld API', description='CampusWorld 后端 API - 支持 HTTP REST 和 WebSocket', version='1.0.0', lifespan=_http_app_lifespan)
    app.state.limiter = auth_limiter
    app.add_exception_handler(RateLimitExceeded, _rate_limit_exceeded_handler)
    _config = get_config()
    _cors_origins = _config.get('cors.allowed_origins', ['http://localhost:3000'])
    app.add_middleware(CORSMiddleware, allow_origins=_cors_origins, allow_credentials=True, allow_methods=['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'], allow_headers=['Authorization', 'Content-Type', 'X-Requested-With', 'Accept', 'Origin'], expose_headers=['X-Request-ID'], max_age=86400)
    app.include_router(api_router, prefix='/api/v1')

    @app.get('/')
    async def root():
        return {'name': 'CampusWorld API', 'version': '1.0.0', 'status': 'running'}
