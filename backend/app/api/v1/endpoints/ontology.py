...
from app.services.data_access_policy import apply_ontology_node_type_read_policy, apply_ontology_relationship_type_read_policy, load_policy, ontology_relationship_type_visible, ontology_type_visible
...
def list_node_types(request: Request, type_code: Optional[str]=None, parent_type_code: Optional[str]=None, name_eq: Optional[str]=None, type_name_eq: Optional[str]=None, name_like: Optional[str]=None, tags_any: Optional[str]=None, trait_class: Optional[str]=None, is_active: Optional[bool]=None, offset: int=Query(default=0, ge=0), limit: int=Query(default=100, ge=1, le=500), db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.read'))):
...
def create_node_type(payload: NodeTypeIn, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.write'))):
    (policy, deny_all) = _policy_from_principal(principal)
    ...
    row = NodeType(type_code=payload.type_code, parent_type_code=payload.parent_type_code, type_name=payload.type_name, typeclass=payload.typeclass, status=0 if payload.is_active else 1, classname=payload.classname, module_path=payload.module_path, description=payload.description, schema_definition=payload.schema_definition, schema_default=payload.schema_default, inferred_rules=payload.inferred_rules, tags=payload.tags, ui_config=payload.ui_config, trait_class=payload.trait_class, trait_mask=payload.trait_mask)
    db.add(row)
    db.commit()
    db.refresh(row)
    return row.to_dict()
...
def get_node_type(type_code: str, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.read'))):
    (policy, deny_all) = _policy_from_principal(principal)
...
def patch_node_type(type_code: str, payload: dict, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.write'))):
    (policy, deny_all) = _policy_from_principal(principal)
    row = db.query(NodeType).filter(NodeType.type_code == type_code).first()
    if not row:
        return _problem(404, 'Not Found', f'node type `{type_code}` not found.', request)
    ...
    allowed = {'parent_type_code', 'type_name', 'typeclass', 'classname', 'module_path', 'description', 'schema_definition', 'schema_default', 'inferred_rules', 'tags', 'ui_config', 'trait_class', 'trait_mask', 'is_active'}
...
def delete_node_type(type_code: str, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.write'))):
    (policy, deny_all) = _policy_from_principal(principal)
    row = db.query(NodeType).filter(NodeType.type_code == type_code).first()
    if not row:
        return _problem(404, 'Not Found', f'node type `{type_code}` not found.', request)
...
def list_relationship_types(request: Request, type_code: Optional[str]=None, name_eq: Optional[str]=None, type_name_eq: Optional[str]=None, name_like: Optional[str]=None, tags_any: Optional[str]=None, trait_class: Optional[str]=None, is_active: Optional[bool]=None, offset: int=Query(default=0, ge=0), limit: int=Query(default=100, ge=1, le=500), db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.read'))):
    (policy, deny_all) = _policy_from_principal(principal)
    err = _validate_query(name_like, tags_any, request)
    if err:
        return err
...
def create_relationship_type(payload: RelationshipTypeIn, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.write'))):
    (policy, deny_all) = _policy_from_principal(principal)
    ...
    row = RelationshipType(type_code=payload.type_code, type_name=payload.type_name, typeclass=payload.typeclass, status=0 if payload.is_active else 1, constraints=payload.constraints, description=payload.description, schema_definition=payload.schema_definition, inferred_rules=payload.inferred_rules, tags=payload.tags, ui_config=payload.ui_config, trait_class=payload.trait_class, trait_mask=payload.trait_mask, is_directed=payload.is_directed, is_symmetric=payload.is_symmetric, is_transitive=payload.is_transitive)
    db.add(row)
    db.commit()
    db.refresh(row)
    return row.to_dict()
...
def get_relationship_type(type_code: str, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.read'))):
    (policy, deny_all) = _policy_from_principal(principal)
...
def patch_relationship_type(type_code: str, payload: dict, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.write'))):
    (policy, deny_all) = _policy_from_principal(principal)
    row = db.query(RelationshipType).filter(RelationshipType.type_code == type_code).first()
    if not row:
        return _problem(404, 'Not Found', f'relationship type `{type_code}` not found.', request)
    if not ontology_relationship_type_visible(type_code, policy, deny_all):
        return _forbidden_data_access(request)
    allowed = {'type_name', 'typeclass', 'constraints', 'description', 'schema_definition', 'inferred_rules', 'tags', 'ui_config', 'trait_class', 'trait_mask', 'is_directed', 'is_symmetric', 'is_transitive', 'is_active'}
    for (k, v) in payload.items():
        if k in allowed:
            if k == 'is_active':
                row.status = 0 if bool(v) else 1
...
def delete_relationship_type(type_code: str, request: Request, db: Session=Depends(get_db), principal: APIPrincipal=Depends(require_api_permission('ontology.write'))):
    (policy, deny_all) = _policy_from_principal(principal)
    row = db.query(RelationshipType).filter(RelationshipType.type_code == type_code).first()
    if not row:
        return _problem(404, 'Not Found', f'relationship type `{type_code}` not found.', request)
