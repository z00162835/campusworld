...
def derive_agent_status(node: Node, session: Session) -> str:
    ...
    if not enabled_allows(attrs):
        return 'unavailable'
    running = session.query(AgentRunRecord).filter(AgentRunRecord.agent_node_id == node.id, AgentRunRecord.ended_at.is_(None), AgentRunRecord.status == 'running').first()
...
def _query_active_npc_agent_nodes(session: Session) -> List[Node]:
    return session.query(Node).filter(Node.type_code == 'npc_agent', Node.is_active == True).order_by(Node.id).all()
...
def _agent_row_dict(node: Node, session: Session) -> Dict[str, Any]:
    return {'id': _logical_agent_id(node), 'name': node.name or '', 'status': derive_agent_status(node, session), 'agent_node_id': node.id}
...
def _format_agent_list_message(rows: List[Dict[str, Any]], locale: str) -> str:
    ...
    if not rows:
        return _agent_list_text(locale, 'list.empty', 'agent list: no active npc_agent nodes found.\nExpected: nodes with type_code=npc_agent, is_active=true')

    def status_label(machine: str) -> str:
        m = (machine or '').strip().lower()
        if not m:
            return machine or '-'
        return _agent_list_text(locale, f'status_value.{m}', machine)
    ...
    lines: List[str] = [f'{h_idcol:<{id_w}} {h_name:<{name_w}} {h_status:<{status_w}} {h_nid}', '-' * (id_w + name_w + status_w + nid_w + 3)]
    for r in rows:
...
def _excluded_by_policy_on_allowlist(node: Node, effective: List[str]) -> List[str]:
    raw = (node.attributes or {}).get('tool_allowlist') or []
...
def _format_agent_tools_table(rows: List[Dict[str, Any]], locale: str) -> str:
    ...
    lines = [f'{h_idcol:<{id_w}} {h_name:<{name_w}} {h_status:<{status_w}} {h_n:<{n_w}}  {h_tools}', '-' * (id_w + name_w + status_w + n_w + tools_w + 5)]
    for r in rows:
        lid = str(r.get('id', ''))[:id_w]
        name = str(r.get('name') or '-')[:name_w]
...
def _apply_allowlist_change(node: Node, *, action: str, requested: List[str]) -> Tuple[List[str], List[str], List[str], List[str]]:
    attrs = dict(node.attributes or {})
...
def _format_agent_tools_write_message(locale: str, *, action: str, logical_id: str, added: List[str], removed: List[str], unchanged: List[str]) -> str:
    lid = logical_id
    if action == 'add':
        if added and (not unchanged):
            return _agent_tool_text(locale, 'summary.add_success', 'add {sid} {tools} succeeded (allowlist updated).').format(sid=lid, tools=_join_tool_names(added))
        if added and unchanged:
            return _agent_tool_text(locale, 'summary.add_mixed', 'add {sid} {added} succeeded. Already in allowlist (no change): {unchanged}.').format(sid=lid, added=_join_tool_names(added), unchanged=_join_tool_names(unchanged))
        return _agent_tool_text(locale, 'summary.add_noop', 'No change: {tools} already in allowlist for {sid}.').format(sid=lid, tools=_join_tool_names(unchanged))
    if removed and (not unchanged):
        return _agent_tool_text(locale, 'summary.del_success', 'del {sid} {tools} succeeded (allowlist updated).').format(sid=lid, tools=_join_tool_names(removed))
    if removed and unchanged:
        return _agent_tool_text(locale, 'summary.del_mixed', 'del {sid} {removed} succeeded. Not in allowlist (no change): {unchanged}.').format(sid=lid, removed=_join_tool_names(removed), unchanged=_join_tool_names(unchanged))
    return _agent_tool_text(locale, 'summary.del_noop', 'No change: {tools} not in allowlist for {sid}.').format(sid=lid, tools=_join_tool_names(unchanged))

def _agent_tool_list_all(context: CommandContext, locale: str) -> CommandResult:
    if not context.db_session:
        return CommandResult.error_result(_agent_tool_text(locale, 'error.no_session', 'database session required'))
    nodes = _query_active_npc_agent_nodes(context.db_session)
    rows: List[Dict[str, Any]] = []
    for node in nodes:
        tools = _effective_tools_for_agent(context.db_session, node, context)
        excluded = _excluded_by_policy_on_allowlist(node, tools)
        rows.append({'id': _logical_agent_id(node), 'name': node.name or '', 'status': derive_agent_status(node, context.db_session), 'agent_node_id': node.id, 'tool_count': len(tools), 'tools': tools, 'excluded_by_policy': excluded})
    rows.sort(key=lambda r: (r.get('id') or '', r.get('name') or ''))
    message = _format_agent_tools_table(rows, locale)
    return CommandResult.success_result(message, data={'agents': rows, 'total': len(rows)})
...
def _agent_tool_single(context: CommandContext, handle: str, locale: str) -> CommandResult:
    ...
    return CommandResult.success_result(message, data={'id': lid, 'agent_node_id': node.id, 'tools': ids, 'excluded_by_policy': excluded})

def _agent_tool_write(context: CommandContext, action: str, rest: List[str], locale: str) -> CommandResult:
    ...
    if not context.db_session:
        return CommandResult.error_result(_agent_tool_text(locale, 'error.no_session', 'database session required'))
    if not permission_checker.check_permission(list(context.permissions or []), AGENT_TOOLS_MANAGE_PERMISSION):
        template = _agent_tool_text(locale, 'error.forbidden', 'Permission denied for agent tool {action}')
        return CommandResult.error_result(template.format(action=action), error=AGENT_TOOLS_FORBIDDEN)
    if len(rest) < 2:
    ...
    (new_list, added, removed, unchanged) = _apply_allowlist_change(node, action=action, requested=primaries)
    flag_modified(node, 'attributes')
    context.db_session.commit()
    lid = _logical_agent_id(node)
    message = _format_agent_tools_write_message(locale, action=action, logical_id=lid, added=added, removed=removed, unchanged=unchanged)
    return CommandResult.success_result(message, data={'id': lid, 'agent_node_id': node.id, 'action': action, 'tool_allowlist': list(new_list), 'added': added, 'removed': removed, 'unchanged': unchanged})
...
def _agent_tool_dispatch(context: CommandContext, rest: List[str], locale: str) -> CommandResult:
    ...
    if len(rest) >= 2:
        ...
        if first not in _AGENT_TOOLS_WRITE_SUBCOMMANDS and second in _AGENT_TOOLS_WRITE_SUBCOMMANDS:
            ...
            msg = _agent_tool_text(locale, 'error.misordered_add_del', 'The `add` or `del` subcommand must be the first token after `agent tool`. Use: {expected}\n(This line was parsed as a query for agent `{first}` only; "{second}" and following tokens were ignored.)').format(expected=expected, first=rest[0], second=second)
...
def _agent_show_dispatch(context: CommandContext, rest: List[str], locale: str) -> CommandResult:
    if not rest:
        return CommandResult.error_result(_agent_show_text(locale, 'usage', 'usage: agent show <id>'), is_usage=True)
    if not context.db_session:
        return CommandResult.error_result('database session required')
    sid = rest[0]
    ...
    data = {'id': logical_id, 'agent_node_id': node.id, 'typeclass': typeclass, 'decision_mode': (node.attributes or {}).get('decision_mode'), 'cognition': (node.attributes or {}).get('cognition_profile_ref'), 'capabilities': ['command.execute', 'agent.memory']}
...
class AicoCommand(SystemCommand):
    ...
    def get_usage(self) -> str:
        from app.commands.aico_exec import AICO_USAGE_LINE
        return AICO_USAGE_LINE

    def _get_specific_help(self) -> str:
        return '\nEquivalent to: @aico <message> | @aico -l | … (same flags)\n  -i [text]   Interactive SSH REPL (Ctrl+Q exit). Optional opening message.\n  -nd [text]  New dialogue thread (optional first tick).\n  -l [-a]     List threads (default 8; -a all).\n  -his <uuid> [-a]   Thread transcript (default last 8 rounds; -a full).\n  -d <uuid>   Request delete; then run: aico -d confirm\n  -cd <uuid>  Continue an existing thread.\n'

    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        from app.commands.aico_exec import execute_aico_command
        return execute_aico_command(context, args)
...
class AgentCommand(SystemCommand):
    ...
    tool_semantics = CommandToolSemantics(
        ...
        data_scope=('npc_agent',),
        error_schema=build_error_schema(('NOT_FOUND', 'PERMISSION_DENIED', 'POLICY_DENIED')),
    )
    ...
    def get_usage(self) -> str:
        return 'agent list | agent status <id> | agent tool [args] | agent show <id>'

    def _get_specific_help(self) -> str:
        return '\nSubcommands:\n  list          — table of visible npc_agent rows\n  status <id>   — one row JSON by handle/service_id\n  tool          — list tools (no args) or one agent; agent tool add|del <id> <tool>...\n  show <id>     — static capability summary JSON\n'

    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        from app.commands.i18n.locale_text import resolve_locale
        locale = resolve_locale(context)
        if not context.db_session:
            return CommandResult.error_result(_agent_list_text(locale, 'error.no_session', 'database session required'))
        if not args:
        ...
        if sub == 'status':
            if len(args) < 2:
                return CommandResult.error_result(_agent_list_text(locale, 'status.usage', 'usage: agent status <id>'), is_usage=True)
            handle = args[1].strip()
            matches = _find_npc_agent_nodes_by_handle(session, handle)
            if len(matches) != 1:
                return CommandResult.error_result(AGENT_STATUS_ACCESS_ERROR)
            return CommandResult.success_result(json.dumps(_agent_row_dict(matches[0], session), ensure_ascii=False))
        if sub == 'tool':
...
def get_agent_commands() -> List[SystemCommand]:
    ...
    with _agent_commands_lock:
        if _agent_commands_cache is None:
            _agent_commands_cache = [AicoCommand(), AgentCommand()]
    return _agent_commands_cache
