...
from app.game_engine.agent_runtime.conversation_stm_service import aggregate_stm_messages_for_thread, clear_conversation_thread_for_transport, delete_conversation_thread_for_owner, list_threads_for_owner_agent, persist_conversation_thread_for_transport, set_thread_id_on_context
...
AICO_USAGE_LINE = 'aico [-i [text...]] | [-l [-a]] | [-his <uuid> [-a]] | [-d <uuid>|confirm] | [-nd [text...]] | [-cd <uuid>] | <message...>'
...
def _format_aico_thread_table(rows: List[AgentConversationThread]) -> Tuple[str, List[Dict[str, Any]]]:
    lines = [f"{'thread_id':<40} {'title':<32} {'last_message_at':<28}", '-' * 104]
...
def _aico_delete_confirm(context: CommandContext, node, cid: int) -> CommandResult:
    ...
    ok = delete_conversation_thread_for_owner(context.db_session, owner_account_node_id=cid, agent_node_id=node.id, thread_id=tid)
    if not ok:
        ep.pop(AICO_DELETE_PENDING_KEY, None)
...
def execute_aico_command(context: CommandContext, args: List[str]) -> CommandResult:
    ...
    if args[0] == '-l':
        ...
        rows = list_threads_for_owner_agent(context.db_session, owner_account_node_id=cid, agent_node_id=node.id, limit=limit)
        if not rows:
    ...
    if args[0] == '-his':
        ...
        thr = context.db_session.query(AgentConversationThread).filter(AgentConversationThread.id == hid, AgentConversationThread.owner_account_node_id == cid, AgentConversationThread.agent_node_id == node.id).first()
        if thr is None:
            return CommandResult.error_result('thread not found')
        msgs = aggregate_stm_messages_for_thread(context.db_session, owner_account_node_id=cid, agent_node_id=node.id, conversation_thread_id=hid)
        if not want_all:
    ...
    if args[0] == '-d':
        ...
        try:
            ...
            return CommandResult.error_result('invalid thread uuid')
        row = context.db_session.query(AgentConversationThread).filter(AgentConversationThread.id == did, AgentConversationThread.owner_account_node_id == cid, AgentConversationThread.agent_node_id == node.id).first()
        if row is None:
            return CommandResult.error_result('thread not found')
        ep = _ensure_command_ephemeral(context)
        if ep is None:
            return CommandResult.error_result('session storage required for delete confirmation')
        ep[AICO_DELETE_PENDING_KEY] = {'thread_id': str(did), 'agent_node_id': int(node.id), 'expires_at': time.time() + AICO_DELETE_TTL_SEC}
        context.db_session.commit()
        return CommandResult.success_result('Pending deletion for this thread. To confirm, run: aico -d confirm\n(confirmation expires in about two minutes.)')
    if args[0] == '-cd':
        if len(args) < 2:
        ...
        try:
            ...
            return CommandResult.error_result('invalid thread uuid')
        row = context.db_session.query(AgentConversationThread).filter(AgentConversationThread.id == tid, AgentConversationThread.owner_account_node_id == cid, AgentConversationThread.agent_node_id == node.id).first()
        if row is None:
            return CommandResult.error_result('thread not found')
    ...
    if args[0] == '-i':
        ...
        if hud_tid:
            try:
                ...
                tr = context.db_session.query(AgentConversationThread).filter(AgentConversationThread.id == u, AgentConversationThread.owner_account_node_id == cid, AgentConversationThread.agent_node_id == node.id).first()
                if tr and (tr.title_snippet or '').strip():
                    title = (tr.title_snippet or '').strip()[:80]
            except Exception:
                pass
        lines = ['AICO interactive mode (SSH). Ctrl+Q exits; Ctrl+C cancels current reply.', f"thread_id: {hud_tid or '(implicit until first message)'}"]
        if title:
