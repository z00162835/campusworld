...
# §17.12 H2 — server-sent tokens only; user-visible copy lives on clients.
...
def emit_tick_lifecycle_meta(
    ...
    *,
    ...
    empty_reply: Optional[bool] = None,
) -> None:
...
def emit_assistant_stream_ndjson(
    emit: Optional[Callable[[str], None]],
    assistant_text: str,
    *,
    thread_id: Optional[uuid.UUID] = None,
    ...
    for i in range(0, len(text), chunk_size):
        emit(
            json.dumps(
                {"kind": "delta", "text": text[i : i + chunk_size]},
                ensure_ascii=False,
            )
        )
