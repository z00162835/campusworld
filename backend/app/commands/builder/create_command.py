...
class CreateCommand(BaseCommand):
    ...
    def execute(self, context, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            if not model_class:
                ...
                return CommandResult.error_result(f"未找到模型类 '{model_name}'。可用模型: {', '.join(available_models)}")
    ...
    def _parse_create_command(self, command_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'model_name': model_name, 'parameters': parameters}
        except Exception as e:
    ...
    def _validate_parameters(self, model_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if missing_params:
            return {'success': False, 'error': f"缺少必需参数: {', '.join(missing_params)}"}
        for (param_name, param_value) in parameters.items():
            if param_name in metadata['parameters']:
                param_info = metadata['parameters'][param_name]
                expected_type = param_info['type']
                if expected_type and (not isinstance(param_value, expected_type)):
                    return {'success': False, 'error': f'参数 {param_name} 类型错误，期望 {expected_type.__name__}，实际 {type(param_value).__name__}'}
        return {'success': True}
    ...
    def _create_object(self, model_class: Type[DefaultObject], parameters: Dict[str, Any], context) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': False, 'error': f'创建对象失败: {str(e)}'}
...
class CreateInfoCommand(BaseCommand):
    ...
    def __init__(self):
        super().__init__(name='create_info', description='显示模型详细信息', aliases=['cinfo', 'model_info'], command_type=CommandType.ADMIN)
    ...
    def execute(self, context, args: List[str]) -> CommandResult:
        ...
        if not model_info:
            available_models = ModelDiscoverer.list_models()
            return CommandResult.error_result(f"未找到模型 '{model_name}'。可用模型: {', '.join(available_models)}")
