...
class CmdSet(ABC):
    ...
    def add_command(self, command: BaseCommand) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Add command '{command.name}' failed: {e}")
            return False
    ...
    def remove_command(self, command_name: str) -> bool:
        ...
        if command_name in self.commands:
            ...
            aliases_to_remove = [alias for (alias, cmd_name) in self.aliases.items() if cmd_name == command_name]
    ...
    def clear(self):
        ...
        self.commands.clear()
        self.aliases.clear()
        self.logger.debug('CmdSet cleared')
    ...
    def merge(self, other_cmdset: 'CmdSet') -> bool:
        ...
        try:
            ...
            self.logger.error(f'Failed to merge CmdSet: {e}')
            return False
...
class CmdSetManager:
    ...
    def clear(self):
        ...
        self.logger.debug('All CmdSets cleared')

    def get_cmdset_info(self) -> Dict[str, Any]:
        """获取CmdSet信息"""
        return {'total_cmdsets': len(self.cmdset_stack), 'total_commands': len(self.get_all_commands()), 'cmdsets': [{'name': cmdset.__class__.__name__, 'priority': cmdset.priority, 'commands': len(cmdset.get_commands())} for cmdset in self.cmdset_stack]}
