...
class CommandContext:
    ...
    def add_message(self, message: str, message_type: str='info'):
        """添加消息"""
        self.messages.append({'message': message, 'type': message_type, 'timestamp': datetime.now()})
        return self
    ...
    def get_context_summary(self) -> Dict[str, Any]:
        """获取上下文摘要"""
        return {'caller': str(self.caller) if self.caller else None, 'target': str(self.target) if self.target else None, 'location': str(self.location) if self.location else None, 'execution_time': self.execution_time, 'created_at': self.created_at.isoformat(), 'is_executing': self.is_executing, 'execution_success': self.execution_success, 'execution_error': self.execution_error, 'required_permissions': self.required_permissions, 'required_roles': self.required_roles, 'access_level': self.access_level, 'input_data': self.input_data, 'output_data': self.output_data, 'message_count': len(self.messages), 'context_data': self.context_data}
...
class CommandExecutionContext:
    ...
    def get_context_statistics(self) -> Dict[str, Any]:
        ...
        return {'total_contexts': total_contexts, 'executing_contexts': executing_contexts, 'completed_contexts': completed_contexts, 'max_contexts': self.max_contexts}
