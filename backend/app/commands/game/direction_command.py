...
class MovementCommand(GameCommand):
    ...
    def _move(self, user_id: str, direction: str) -> Tuple[bool, str, Optional[str]]:
        with db_session_context() as session:
            user = session.query(Node).filter(Node.id == user_id, Node.type_code == 'account', Node.is_active == True).first()
            if not user:
            ...
            room_pkg_id = ''
            if user.location_id:
                loc = session.query(Node).filter(Node.id == user.location_id, Node.type_code == 'room', Node.is_active == True).first()
                if loc:
            ...
            if room is None:
                ...
                room = session.query(Node).filter(Node.type_code == 'room', Node.attributes['world_id'].astext == world_id, Node.attributes['package_node_id'].astext == room_pkg_id, Node.is_active == True).first()
            if not room:
            ...
            candidates = session.query(Relationship, Node).join(Node, Relationship.target_id == Node.id).filter(Relationship.source_id == room.id, Relationship.type_code == 'connects_to', Relationship.is_active == True, Node.attributes['world_id'].astext == world_id, Node.is_active == True).all()
            ...
            if len(matched_same_world) == 1:
                ...
                return (False, f'方向 {dir_norm} 存在多个可达目标（数据冲突），候选: {hint}', 'WORLD_DIRECTION_AMBIGUOUS')
            if not target:
                bridge_candidates = session.query(Relationship, Node).join(Node, Relationship.target_id == Node.id).filter(Relationship.source_id == room.id, Relationship.type_code == 'connects_to', Relationship.is_active == True, Node.is_active == True).all()
            ...
            if not target:
                if available_dirs:
                    return (False, f"无法向 {direction} 移动，可用方向: {', '.join(sorted(set(available_dirs)))}", None)
            ...
            if tw_final and tw_final != world_id:
                return (True, f'你通过跨世界连接向 {normalize_direction(direction)} 移动，来到 {target_name}（{tw_final}）', None)
...
def build_direction_commands() -> List[GameCommand]:
    return [MovementCommand(), FixedDirectionCommand(name='north', aliases=['n'], description='向北移动', direction='north'), FixedDirectionCommand(name='south', aliases=['s'], description='向南移动', direction='south'), FixedDirectionCommand(name='east', aliases=['e'], description='向东移动', direction='east'), FixedDirectionCommand(name='west', aliases=['w'], description='向西移动', direction='west'), FixedDirectionCommand(name='northeast', aliases=['ne'], description='向东北移动', direction='northeast'), FixedDirectionCommand(name='northwest', aliases=['nw'], description='向西北移动', direction='northwest'), FixedDirectionCommand(name='southeast', aliases=['se'], description='向东南移动', direction='southeast'), FixedDirectionCommand(name='southwest', aliases=['sw'], description='向西南移动', direction='southwest'), FixedDirectionCommand(name='up', aliases=['u'], description='向上移动', direction='up'), FixedDirectionCommand(name='down', aliases=['d'], description='向下移动', direction='down'), FixedDirectionCommand(name='in', aliases=[], description='向内移动', direction='enter'), FixedDirectionCommand(name='out', aliases=['o'], description='向外移动', direction='out')]
