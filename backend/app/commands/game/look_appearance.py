    ...
    if exit_entries:
        ...
        for d in sorted(by_dir.keys()):
            ...
            if len(group) == 1:
                ...
                targets = ' / '.join((str(x.get('target_display_name') or x.get('target_package_node_id') or '?').strip() for x in group))
...
def short_label_from_scoped_title(name: str, attrs: Dict[str, Any]) -> Optional[str]:
    ...
    for cand in (str(name or '').strip(), str(attrs.get('display_name') or '').strip()):
        if ' · ' in cand:
...
def return_appearance_room(room: Dict[str, Any], context: Any=None) -> str:
    ...
    buckets: Dict[str, List[Dict[str, Any]]] = {'portals': [], 'world_exits': [], 'fixtures': [], 'characters': [], 'things': []}
    ...
    kwargs = {'header': '', 'name': name, 'extra_name_info': extra, 'desc': desc, 'exits': exits_block, 'portals': portals, 'fixtures': fixtures, 'characters': characters, 'things': things, 'footer': footer}
    text = safe_format_template(tmpl, **kwargs)
    return format_appearance(text)

def return_appearance_object(context: Any, obj: Dict[str, Any], target_args: Optional[List[str]]=None) -> str:
    del target_args
    ...
    if model_cls:
        try:
            inst = model_cls(name=obj.get('name', tc), description=str(obj.get('description') or ''), attributes=attrs, disable_auto_sync=True)
        except TypeError:
            try:
    ...
    if inst is not None:
        ...
        extra = ''
        desc = str(obj.get('description') or attrs.get('long_description') or attrs.get('short_description') or attrs.get('description') or attrs.get('entry_description') or attrs.get('look_description') or attrs.get('examine_desc') or '').strip()
        inner = attrs.get('attributes')
        if not desc and isinstance(inner, dict):
            desc = str(inner.get('long_description') or inner.get('short_description') or inner.get('description') or inner.get('look_description') or inner.get('examine_desc') or '').strip()
        if not desc:
    ...
    body = safe_format_template(DEFAULT_OBJECT_LOOK_TEMPLATE, name=name, extra_name_info=extra, desc=desc, footer=footer)
