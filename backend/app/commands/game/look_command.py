...
_EXIT_DIRECTION_ORDER = ('north', 'northeast', 'east', 'southeast', 'south', 'southwest', 'west', 'northwest', 'up', 'down', 'enter', 'out')
...
def _merge_room_exit_labels_from_attrs_and_graph(room_attrs: Dict[str, Any], graph_labels: List[str]) -> List[str]:
...
class LookCommand(GameCommand):
    ...
    def _look_room(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            self.logger.error(f'Failed to inspect room: {e}')
    ...
    def _look_object(self, context: CommandContext, target: str, target_args: Optional[List[str]]=None) -> CommandResult:
        ...
        try:
            ...
            self.logger.error(f'Failed to inspect item: {e}')
    ...
    def _get_current_room(self, context: CommandContext) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f'Failed to get current room: {e}')
            return None
        ...
        try:
            ...
            with db_session_context() as session:
                ...
                try:
                    ...
                    uid_int = None
                if uid_int is None:
                    self.logger.warning(f'Invalid or missing user_id, unable to resolve location: {raw_uid!r}')
                    return None
                user_node = session.query(Node).filter(Node.id == uid_int, Node.type_code == 'account', Node.is_active == True).first()
                if not user_node:
                    self.logger.warning(f'User node not found: {uid_int}')
                    return None
                location_id = user_node.location_id
                if location_id:
                    location_node = session.query(Node).filter(Node.id == location_id, Node.is_active == True).first()
                    if location_node and location_node.type_code == 'room':
                        return {'scope': 'system', 'room_id': str(location_node.id)}
                return None
        except Exception as e:
            self.logger.error(f'Failed to get user location from graph data: {e}')
            return None

    def _target_room_display(self, node: Any) -> Dict[str, str]:
        attrs = dict(getattr(node, 'attributes', {}) or {})
        display_name = str(attrs.get('room_name') or attrs.get('display_name') or getattr(node, 'name', '') or attrs.get('package_node_id') or '未知目标').strip()
        short_desc = str(attrs.get('room_short_description') or '').strip()
        pkg = str(attrs.get('package_node_id') or '').strip().lower()
        return {'target_display_name': display_name, 'target_short_desc': short_desc, 'target_package_node_id': pkg}
    ...
    def _get_room_from_graph_data(self, context: CommandContext, room_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            from app.models.graph import Node
            with db_session_context() as session:
                room_node = session.query(Node).filter(Node.id == int(room_id) if room_id.isdigit() else None, Node.is_active == True).first()
                if not room_node:
                    room_node = session.query(Node).filter(Node.attributes['room_name'].astext == room_id, Node.type_code == 'room', Node.is_active == True).first()
                if room_node:
                    attrs = room_node.attributes
                    contents = session.query(Node).filter(Node.location_id == room_node.id, Node.is_active == True).all()
                    ra = dict(attrs or {})
                    content_entries = []
                    room_objects = []
                    for n in contents:
                        ...
                        if not self._is_visible_in_room(context, n):
                            continue
                        entry_d = {'node_id': n.id, 'name': n.name, 'type_code': n.type_code, 'type': n.type_code, 'description': n.description or '', 'attributes': dict(n.attributes or {})}
                    ...
                    rdesc = (ra.get('room_description') or '').strip() or ra.get('display_name') or '这里没有什么特别的。'
    ...
    def _is_visible_in_room(self, context: Optional[CommandContext], node: Any) -> bool:
        ...
        try:
            return evaluate_policy_expr(view_lock, user_permissions=list(getattr(context, 'permissions', []) or []) if context else [], user_roles=list(getattr(context, 'roles', []) or []) if context else [], object_attrs=attrs)
    ...
    def _get_graph_object_in_room_by_name(self, context: CommandContext, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            from app.models.graph import Node
            with db_session_context() as session:
                user_node = session.query(Node).filter(Node.id == context.user_id, Node.type_code == 'account', Node.is_active == True).first()
                if not user_node:
                    return None
                room_ref = self._get_user_current_room_ref(context)
                room_node_id = None
                ...
                if not room_node_id:
                    return None
                candidates = session.query(Node).filter(Node.location_id == room_node_id, Node.is_active == True).all()
                node = None
                ...
                if not node:
                    return None
                return {'id': str(node.id), 'name': node.name, 'type': node.type_code, 'node_id': node.id, 'type_code': node.type_code, 'description': node.description, 'attributes': dict(node.attributes or {})}
        except Exception:
            return None
    ...
    def _get_item_info(self, context: CommandContext, item_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f'Failed to get item info: {e}')
            return None
    ...
    def _build_object_description(self, context: CommandContext, obj: Dict[str, Any], target_args: Optional[List[str]]=None) -> str:
        ...
        try:
            ...
            if type_code:
                try:
                    ...
                    if model_cls and hasattr(model_cls, 'get_appearance'):
                        try:
                            inst = model_cls(name=obj.get('name', type_code), description=str(obj.get('description') or ''), attributes=dict(obj.get('attributes') or {}), disable_auto_sync=True)
                        except TypeError:
                            try:
            ...
            self.logger.error(f'Failed to build item description: {e}')
    ...
    def _graph_object_dict_by_node_id(self, context: CommandContext, node_id: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            from app.models.graph import Node
            with db_session_context() as session:
                user = session.query(Node).filter(Node.id == context.user_id, Node.type_code == 'account', Node.is_active == True).first()
                if not user or not user.location_id:
                    return None
                node = session.query(Node).filter(Node.id == node_id, Node.is_active == True).first()
                if not node or node.location_id != user.location_id:
                    return None
                return {'id': str(node.id), 'name': node.name, 'node_id': node.id, 'type_code': node.type_code, 'type': node.type_code, 'description': node.description, 'attributes': dict(node.attributes or {})}
        except Exception:
            return None

    def _object_from_disambig_entry(self, context: CommandContext, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if not isinstance(entry, dict):
            return None
    ...
    def _resolve_look_disambiguation_index(self, context: CommandContext, target: str, target_args: List[str]) -> CommandResult:
        if not context.session or not hasattr(context.session, 'command_ephemeral'):
            return CommandResult.error_result('当前连接不支持「look <编号>」消歧；请使用完整对象名称查看。')
        ep = getattr(context.session, 'command_ephemeral', None)
        if not isinstance(ep, dict):
            return CommandResult.error_result('当前连接不支持「look <编号>」消歧；请使用完整对象名称查看。')
        entries = ep.get(LOOK_DISAMBIGUATION_KEY)
        if not isinstance(entries, list) or not entries:
            return CommandResult.error_result('当前没有待选列表。请先输入关键词出现「多个匹配」后再使用「look <编号>」，或直接「look <完整名称>」。')
        idx = int(target) - 1
        if idx < 0 or idx >= len(entries):
            return CommandResult.error_result(f'编号 {target} 无效（上次列表共 {len(entries)} 项）。请重新搜索或改用完整名称。')
        ...
        return CommandResult.success_result(self._build_object_description(context, obj, target_args=target_args))
    ...
    def _show_multiple_matches(self, context: CommandContext, target: str, objects: List[Dict[str, Any]]) -> CommandResult:
        ...
        try:
            ...
            for (i, obj) in enumerate(objects, 1):
                obj_name = room_list_label_for_entry({'name': obj.get('name'), 'type_code': obj.get('type_code') or obj.get('type'), 'attributes': dict(obj.get('attributes') or {}), 'description': str(obj.get('description') or '')})
            ...
            self.logger.error(f'Failed to display multiple matches: {e}')
    ...
    def _get_room_status(self, room: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.logger.error(f'Failed to get room state: {e}')
            return ''
    ...
    def _get_other_players(self, context: CommandContext, room: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            self.logger.error(f'Failed to get other users: {e}')
            return []
    ...
    def _get_object_status(self, obj: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.logger.error(f'Failed to get item state: {e}')
            return ''
    ...
    def _get_object_attributes(self, obj: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.logger.error(f'Failed to get item attributes: {e}')
            return ''
