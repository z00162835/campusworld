...
def principal_from_context(ctx: CommandContext) -> Principal:
    ...
    if raw_id is None or raw_id == '' or str(raw_id).lower() == 'guest':
        raise UnauthenticatedActor(f'command actor is unauthenticated (user_id={raw_id!r}); cannot construct task principal')
    try:
        actor_id = int(raw_id)
    except (TypeError, ValueError) as exc:
        raise UnauthenticatedActor(f'command actor user_id={raw_id!r} is not a numeric account id; task system requires an authenticated user principal') from exc
    if actor_id <= 0:
        raise UnauthenticatedActor(f'command actor user_id={actor_id} is not a valid account id')
    return Principal(id=actor_id, kind='user', roles=frozenset(ctx.roles or []), permissions=frozenset(ctx.permissions or []))

def resolve_principal_or_error(ctx: CommandContext) -> Tuple[Optional[Principal], Optional[CommandResult]]:
...
def derive_idempotency_key(*, actor: Principal, command_name: str, args: List[str], correlation_id: Optional[str]) -> str:
...
def require_permission(ctx: CommandContext, code: str) -> Optional[CommandResult]:
    ...
    if permission_checker.check_permission(ctx.permissions or [], code):
        return None
    msg = i18n(ctx, 'error.forbidden', default=f'Permission denied: {code}', detail=code)
...
__all__ = ['ParsedArgv', 'parse_argv', 'principal_from_context', 'resolve_principal_or_error', 'derive_idempotency_key', 'i18n', 'task_error_to_result', 'usage_result', 'require_permission', 'correlation_id_from_context', 'trace_id_from_context']
