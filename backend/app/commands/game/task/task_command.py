...
class TaskCommand(GameCommand):
    ...
    def _do_create(self, ctx: CommandContext, args: List[str]) -> CommandResult:
        ...
        bad = self._usage_unknown_bool(ctx, parsed, allowed=_CREATE_BOOL_FLAGS, usage_key='usage.create', usage_default='task create --title <T> ...')
        if bad is not None:
            return bad
        ...
        if err is not None:
            return err
        idem = parsed.flags.get('idempotency-key') or derive_idempotency_key(actor=actor, command_name='task.create', args=args, correlation_id=correlation_id_from_context(ctx))
        ...
        if pool_key:
            with db_session_context() as session:
                ...
                if row is None:
                    return CommandResult.error_result(i18n(ctx, 'error.pool_not_found', default=f'pool {pool_key} not found', detail=pool_key), error='commands.task.error.pool_not_found')
                pool_id = int(row[0])
                ...
                pool_default_workflow_key = pool_workflow_ref.get('key') if isinstance(pool_workflow_ref, dict) else None
                if not pool_active:
                    return CommandResult.error_result(i18n(ctx, 'error.pool_inactive', default=f'pool {pool_key} is inactive', detail=pool_key), error='commands.task.error.pool_inactive')
                decision = evaluate_acl(actor, pool_publish_acl)
                if not decision.allow:
                    return task_error_to_result(ctx, PublishAclDenied(f'publish_acl denied for pool={pool_key}: {decision.reason}'))
            ...
            with db_session_context() as session:
                with session.begin():
                    ...
                    if pool_id is None or keep_draft:
                        ...
                        res = transition(task_id=created.task_id, event='publish', actor_principal=actor, expected_version=created.state_version, idempotency_key=f'{idem}:publish', correlation_id=correlation_id, trace_id=trace_id, payload={'pool_id': pool_id}, db_session=session)
        except TaskSystemError as exc:
            return task_error_to_result(ctx, exc)
        return self._success(ctx, 'create', res, extra={'pool_key': pool_key, 'kept_draft': keep_draft})
    ...
    def _do_assign(self, ctx: CommandContext, args: List[str]) -> CommandResult:
        ...
        bad = self._usage_unknown_bool(ctx, parsed, allowed=_TRANSITION_BOOL_FLAGS, usage_key='usage.assign', usage_default='task assign <id> --to <principal_id> [--kind ...]')
        if bad is not None:
            return bad
        ...
        if target_kind == 'group':
            ...
            try:
                ...
                return usage_result(ctx, 'usage.assign', 'task assign <id> --to <principal_id>')
        return self._run_transition(ctx, task_id=task_id, event='assign', parsed=parsed, payload=payload, sub_args=args)

    def _do_publish(self, ctx: CommandContext, args: List[str]) -> CommandResult:
        parsed = parse_argv(args, bool_flags=_TRANSITION_BOOL_FLAGS)
        bad = self._usage_unknown_bool(ctx, parsed, allowed=_TRANSITION_BOOL_FLAGS, usage_key='usage.publish', usage_default='task publish <id> --to-pool <key>')
        if bad is not None:
            return bad
        if not parsed.positional:
        ...
        if not pool_key:
            return usage_result(ctx, 'usage.publish', 'task publish <id> --to-pool <key>')
        return self._run_transition(ctx, task_id=task_id, event='publish', parsed=parsed, payload={'pool_key': pool_key}, sub_args=args)

    def _do_list(self, ctx: CommandContext, args: List[str]) -> CommandResult:
        parsed = parse_argv(args, bool_flags=_LIST_BOOL_FLAGS)
        bad = self._usage_unknown_bool(ctx, parsed, allowed=_LIST_BOOL_FLAGS, usage_key='usage.list', usage_default='task list [--limit N] [--offset O]')
        if bad is not None:
            return bad
        (actor, err) = resolve_principal_or_error(ctx)
        if err is not None:
        ...
        with db_session_context() as session:
            ...
            if 'mine' in parsed.bools:
                ...
                narrow_clause = " AND EXISTS (SELECT 1 FROM task_assignments a  WHERE a.task_node_id = n.id AND a.is_active    AND a.role = 'executor'    AND a.principal_id = :pid AND a.principal_kind = :pkind)"
            pool_clause = ''
            if pool_key:
        ...
        items = [{'id': int(r.id), 'state': r.state, 'title': r.title, 'priority': r.priority, 'pool_key': r.pool_key} for r in rows]
        end = offset + len(items)
        if not items:
            msg = i18n(ctx, 'list.empty', default='No tasks match.')
        else:
            header = i18n(ctx, 'list.header', default=f'Tasks (total={total})', total=total, offset=offset, end=end)
            row_tmpl = i18n(ctx, 'list.row', default='#{id}  state={state}  pool={pool_key}  priority={priority}  title={title}')
            lines = [header]
            for it in items:
    ...
    def _do_show(self, ctx: CommandContext, args: List[str]) -> CommandResult:
        ...
        with db_session_context() as session:
            ...
            if not self._can_view_task_row(actor=actor, row=row, attrs=attrs, session=session):
                return CommandResult.error_result(i18n(ctx, 'error.not_found', default=f'task {task_id} not found', task_id=task_id), error='commands.task.error.not_found')
    ...
    def _can_view_task_row(self, *, actor, row, attrs: Dict[str, Any], session) -> bool:
    ...
    def _dispatch_event(self, ctx: CommandContext, args: List[str], *, event: str, usage_key: str, usage_default: str) -> CommandResult:
        parsed = parse_argv(args, bool_flags=_TRANSITION_BOOL_FLAGS)
        bad = self._usage_unknown_bool(ctx, parsed, allowed=_TRANSITION_BOOL_FLAGS, usage_key=usage_key, usage_default=usage_default)
        if bad is not None:
            return bad
        if not parsed.positional:
            return usage_result(ctx, usage_key, usage_default)
        try:
            ...
            return usage_result(ctx, usage_key, usage_default)
        return self._run_transition(ctx, task_id=task_id, event=event, parsed=parsed, payload={}, sub_args=args)

    def _run_transition(self, ctx: CommandContext, *, task_id: int, event: str, parsed, payload: Dict[str, Any], sub_args: List[str]) -> CommandResult:
        (actor, err) = resolve_principal_or_error(ctx)
        if err is not None:
            return err
        idem = parsed.flags.get('idempotency-key') or derive_idempotency_key(actor=actor, command_name=f'task.{event}', args=sub_args, correlation_id=correlation_id_from_context(ctx))
        with db_session_context() as session:
            row = session.execute(text("SELECT (attributes->>'state_version')::int FROM nodes WHERE id = :id AND type_code='task'"), {'id': task_id}).first()
            if row is None:
                return CommandResult.error_result(i18n(ctx, 'error.not_found', default=f'task {task_id} not found', task_id=task_id), error='commands.task.error.not_found')
            expected_version = int(row[0] or 0)
        try:
            res = transition(task_id=task_id, event=event, actor_principal=actor, expected_version=expected_version, idempotency_key=idem, correlation_id=correlation_id_from_context(ctx), trace_id=trace_id_from_context(ctx), payload=payload)
        except TaskSystemError as exc:
            return task_error_to_result(ctx, exc)
        return self._success(ctx, event, res, extra={'pool_key': payload.get('pool_key') if event == 'publish' else None})

    def _success(self, ctx: CommandContext, event: str, res: TransitionResult, *, extra: Optional[Dict[str, Any]]=None) -> CommandResult:
        extra = extra or {}
        msg = i18n(ctx, f'{event}.success', default=f'task {event} ok: #{res.task_id} → {res.to_state}', task_id=res.task_id, to_state=res.to_state, from_state=res.from_state, state_version=res.state_version, event_seq=res.event_seq, event=res.event, pool_key=extra.get('pool_key') or '-')
        data = {'task_id': res.task_id, 'from_state': res.from_state, 'to_state': res.to_state, 'event_seq': res.event_seq, 'state_version': res.state_version, 'event': res.event, 'idempotent_replay': res.idempotent_replay, 'correlation_id': res.correlation_id, 'trace_id': res.trace_id}
