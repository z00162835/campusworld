...
from app.game_engine.world_bridge_service import WORLD_BRIDGE_INVALID_ARGUMENT, WORLD_BRIDGE_PERMISSION_DENIED, world_bridge_service
from app.game_engine.world_entry_service import world_entry_service
from app.games.hicampus.package.content_overlay import apply_spatial_content_overlay, content_validate_report, diff_spatial_vs_db
...
def _format_world_list_message(rows: List[Dict[str, Any]]) -> str:
    ...
    if not rows:
        return 'world list: no packages found under game search paths.\nExpected layout: .../games/<world_id>/manifest.yaml'
    lines = [f"{'world_id':<16} {'display_name':<26} {'loaded':<6} {'version':<8} {'api':<6} source", '-' * 90]
    for r in rows:
...
class WorldCommand(AdminCommand):
    ...
    def __init__(self):
        super().__init__(name='world', description='世界包管理(list/install/uninstall/reload/status/validate/repair/content/bridge)', aliases=['worlds'])
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if not self._has_sub_permission(context, action):
            return CommandResult.error_result(f'Permission denied for world {action}', error='WORLD_FORBIDDEN')
        if action == 'list':
    ...
    def get_usage(self) -> str:
        return '用法: world <list|install|uninstall|reload|status|validate|repair|content> ...; <world_id> 可为包目录名world_id（见 world list）、大小写不敏感或 manifest.display_name（唯一时）; world content <validate|diff|apply> <world_ref> ...; world bridge <add|remove|list|validate> ... [--dry-run] [--two-way] [--bridge-type portal|gate|transit]'
    ...
    def _bridge_perm_read(self, context: CommandContext) -> bool:
        return permission_checker.check_permission(list(context.permissions or []), 'admin.world.bridge.read')

    def _bridge_perm_manage(self, context: CommandContext) -> bool:
        return permission_checker.check_permission(list(context.permissions or []), 'admin.world.bridge.manage')
    ...
    def _bridge(self, context: CommandContext, args: List[str]) -> CommandResult:
        if not args:
            return CommandResult.error_result('用法: world bridge <add|remove|list|validate> ...', error=WORLD_BRIDGE_INVALID_ARGUMENT)
        sub = str(args[0]).lower().strip()
        (rest, flags) = self._split_bridge_cli(args[1:])
        if sub in ('add', 'remove'):
            if not self._bridge_perm_manage(context):
                return CommandResult.error_result('Permission denied for world bridge management', error=WORLD_BRIDGE_PERMISSION_DENIED)
        elif sub in ('list', 'validate'):
            if not self._bridge_perm_read(context):
                return CommandResult.error_result('Permission denied for world bridge read', error=WORLD_BRIDGE_PERMISSION_DENIED)
        else:
            return CommandResult.error_result(f'未知 bridge 子命令: {sub}', error=WORLD_BRIDGE_INVALID_ARGUMENT)
        if sub == 'add':
            if len(rest) < 5:
                return CommandResult.error_result('用法: world bridge add <src_world> <src_room> <direction> <dst_world> <dst_room> [--two-way] [--bridge-type <portal|gate|transit>] [--dry-run]', error=WORLD_BRIDGE_INVALID_ARGUMENT)
            (sw, sn, direc, dw, dn) = (rest[0], rest[1], rest[2], rest[3], rest[4])
            out = world_bridge_service.add_bridge(operator=str(context.username or ''), src_world=sw, src_node_pkg=sn, direction=direc, dst_world=dw, dst_node_pkg=dn, two_way=bool(flags.get('two_way')), bridge_type=str(flags.get('bridge_type') or 'portal'), dry_run=bool(flags.get('dry_run')))
            if out.get('ok'):
                return CommandResult.success_result(str(out.get('message') or 'bridge ok'), data=out, command_type=self.command_type)
            return CommandResult(success=False, message=str(out.get('message') or 'bridge add failed'), data=out, error=str(out.get('error') or WORLD_BRIDGE_INVALID_ARGUMENT), command_type=self.command_type)
        if sub == 'remove':
            bridge_id = None
            ...
            if len(rrest) == 1:
                ...
                return CommandResult.error_result('用法: world bridge remove <bridge_id> | <src_world> <src_room> <direction> [--dry-run]', error=WORLD_BRIDGE_INVALID_ARGUMENT)
            out = world_bridge_service.remove_bridge(bridge_id=bridge_id, src_world=src_world, src_node_pkg=src_node, direction=direction, dry_run=bool(rflags.get('dry_run')), force=bool(rflags.get('force', True)))
            if out.get('ok'):
                return CommandResult.success_result(str(out.get('message') or 'bridge removed'), data=out, command_type=self.command_type)
            return CommandResult(success=False, message=str(out.get('message') or 'bridge remove failed'), data=out, error=str(out.get('error') or WORLD_BRIDGE_INVALID_ARGUMENT), command_type=self.command_type)
        if sub == 'list':
            world_filter = rest[0] if rest else None
            out = world_bridge_service.list_bridges(world_id=world_filter, include_disabled=bool(flags.get('include_disabled')))
            return CommandResult.success_result(f"world bridge list ok, total={out.get('total', 0)}", data=out, command_type=self.command_type)
        if sub == 'validate':
            if not rest:
                return CommandResult.error_result('用法: world bridge validate <world_id>', error=WORLD_BRIDGE_INVALID_ARGUMENT)
            ...
            if not out.get('ok') and out.get('error') == WORLD_BRIDGE_INVALID_ARGUMENT:
                return CommandResult.error_result(str(out.get('message') or 'validate failed'), error=WORLD_BRIDGE_INVALID_ARGUMENT)
            msg = f'world bridge validate passed: {wid}' if out.get('ok') else f"world bridge validate issues: {out.get('issue_count', 0)}"
            if out.get('ok'):
                return CommandResult.success_result(msg, data=out, command_type=self.command_type)
            return CommandResult(success=False, message=msg, data=out, error='WORLD_BOUNDARY_VIOLATION', command_type=self.command_type)
    ...
    def _resolve_world_ref(self, ref: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        if not engine or not getattr(engine, 'loader', None):
            return (None, f"unknown world '{ref}'. Known package ids: {', '.join(available) or '(none)'}; try: world list")
        hits: List[str] = []
        ...
        if len(hits) == 1:
            return (hits[0], None)
        if len(hits) > 1:
            return (None, f"ambiguous display_name '{ref}' matches: {', '.join(hits)}. Use world_id from: world list")
        return (None, f"unknown world '{ref}'. Use: world list (canonical id is the package directory name).")
    ...
    def _row_for_list(self, wid: str, loaded: set) -> Dict[str, Any]:
        ...
        if not display:
            display = wid
        return {'world_id': wid, 'display_name': display, 'version': brief.get('version'), 'api_version': brief.get('api_version'), 'loaded': wid in loaded, 'resolved_path': where['resolved_path'], 'source_type': where['source_type']}
    ...
    def _content(self, context: CommandContext, args: List[str]) -> CommandResult:
        if not args:
            return CommandResult.error_result('用法: world content <validate|diff|apply> <world_ref> [--report] [--dry-run] [--no-snapshot]')
        sub = str(args[0]).lower().strip()
        ...
        if not data_root:
            return CommandResult.error_result(f'未找到世界数据目录: {world_id}（期望 <game_root>/data）', error='WORLD_DATA_UNAVAILABLE')
        if sub == 'validate':
            try:
            ...
            return CommandResult.success_result(f'world content validate ok: {world_id}', data=payload, command_type=self.command_type)
        if sub == 'diff':
        ...
        if sub == 'apply':
            with db_session_context() as session:
                out = apply_spatial_content_overlay(session, world_id, data_root, dry_run=bool(flags.get('dry_run')), write_revision_snapshot=not bool(flags.get('no_snapshot')))
            msg = f"world content apply dry-run: would touch {len(out.get('applied_node_ids', []))} nodes" if out.get('dry_run') else f"world content apply ok: updated {len(out.get('applied_node_ids', []))} nodes"
            return CommandResult.success_result(msg, data=out, command_type=self.command_type)
    ...
    def _list_worlds(self, context: CommandContext) -> CommandResult:
        ...
        return CommandResult.success_result(msg, data={'items': rows, 'total': len(rows)}, command_type=self.command_type)
    ...
    def _status(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        payload = {'world_id': world_id, 'game_info': info, 'runtime_state': runtime, **where}
        return CommandResult.success_result(f'world status ok: {world_id}', data=payload, command_type=self.command_type)
    ...
    def _validate(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        msg = f'world validate passed: {world_id}' if report.get('ok') else f"world validate found issues: {report.get('issue_count', 0)}"
        ...
        return CommandResult(success=False, message=msg, data=payload, error='WORLD_TOPOLOGY_INVALID', command_type=self.command_type)
    ...
    def _repair(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        msg = f"world repair dry-run ready: planned={len(report.get('planned_actions', []))}" if dry_run else f"world repair done: applied={len(report.get('applied_actions', []))}"
        payload = {'world_id': world_id, 'dry_run': dry_run, 'force': force, 'report': report}
        if report.get('ok'):
            return CommandResult.success_result(msg, data=payload, command_type=self.command_type)
        return CommandResult(success=False, message=msg, data=payload, error='WORLD_REPAIR_INCOMPLETE', command_type=self.command_type)
    ...
    def _from_world_result(self, action: str, world_id: str, out: Dict[str, Any]) -> CommandResult:
        ...
        if ok:
            return CommandResult.success_result(msg, data=payload, command_type=self.command_type)
        return CommandResult(success=False, message=msg, data=payload, error=str(out.get('error_code') or ''), command_type=self.command_type)
