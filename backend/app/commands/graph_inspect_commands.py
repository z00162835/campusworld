...
class _ParsedFindArgs:
    __slots__ = ('query', 'node_id', 'account_name', 'type_code', 'exact', 'startswith', 'in_location', 'limit', 'name', 'describe', 'all', 'error')
...
class _FlagSpec:
    ...
    bad_value_msg: str = ''
    error_msg: str = ''
_FLAG_SPEC: Dict[str, _FlagSpec] = {'--type': _FlagSpec('value', dest='type_code', coerce=_coerce_type, need_value_msg='--type requires a value (e.g. room, account, world_entrance)'), '--limit': _FlagSpec('value', dest='limit', coerce=_coerce_limit, need_value_msg='--limit requires a number', bad_value_msg='--limit must be an integer'), '--in': _FlagSpec('value', dest='in_location', coerce=_coerce_loc, need_value_msg='--in requires a room id', bad_value_msg='--in must be an integer room id'), '--name': _FlagSpec('value', dest='name', coerce=str, need_value_msg='--name requires a value'), '--describe': _FlagSpec('value', dest='describe', coerce=str, need_value_msg='--describe requires a value'), '--exact': _FlagSpec('bool', dest='exact'), '--startswith': _FlagSpec('bool', dest='startswith'), '--all': _FlagSpec('bool', dest='all'), '--location': _FlagSpec('error', error_msg='--location has been renamed in v3; use --in <room_id> or -loc <room_id>')}
_FLAG_ALIASES: Dict[str, str] = {'-t': '--type', '-l': '--limit', '-loc': '--in', '-n': '--name', '-des': '--describe', '-a': '--all'}
...
def _parse_find_args(args: List[str]) -> _ParsedFindArgs:
    ...
    if positional and (out.name is not None or out.describe is not None):
        out.error = 'positional query and --name/--describe are mutually exclusive; pick one form'
        return out
    if positional:
        first = positional[0]
        if first.startswith('#') and len(first) > 1 and all((ch.isdigit() for ch in first[1:])):
    ...
    has_any_predicate = any((v is not None for v in (out.query, out.node_id, out.account_name, out.name, out.describe, out.type_code, out.in_location)))
...
def _parse_describe_args(args: List[str]) -> _ParsedDescribeArgs:
    ...
    for token in args:
        ...
        if canonical == '--all':
            ...
            continue
        if token.startswith('--') or (token.startswith('-') and len(token) > 1 and (not token[1].isdigit())):
            return _ParsedDescribeArgs(identifier=None, show_all=False, error=f'unknown flag: {token}')
        positional_count += 1
        if positional_count > 1:
            return _ParsedDescribeArgs(identifier=None, show_all=False, error='describe accepts a single <id|#id|name>')
        identifier = token
...
class FindCommand(SystemCommand):
    ...
    def get_usage(self) -> str:
        return 'find [<query> | #<id> | *<account>] [-n <name>] [-des <desc>] [-t <type>] [-loc <room_id>] [-l <N>] [-a] [--exact] [--startswith]'
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if parsed.node_id is not None:
            ...
            if row is None:
                return CommandResult.success_result(f'No active node for id={parsed.node_id}.', data=_empty_find_payload(parsed))
        ...
        return _render_find_results(rows, total=total, parsed=parsed, safety_ceiling=safety_ceiling)
...
class DescribeCommand(SystemCommand):
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if node is None:
            return CommandResult.error_result(f'no active node found for identifier {original!r}')
        ...
        out_edges = _sample_out_edges(session, node.id, limit=None if show_all else 8)
        if out_edges:
            lines.append('')
            lines.append('out-edges:' if show_all else 'out-edges (sample):')
            for (rel, other) in out_edges:
                other_label = f'[{other.id}] {other.type_code}: {other.name}' if other is not None else f'(target #{rel.target_id} missing)'
                role = f' role={rel.target_role}' if rel.target_role else ''
                lines.append(f'  -[{rel.type_code}{role}]-> {other_label}')
        data: Dict[str, Any] = {'id': node.id, 'type_code': node.type_code, 'name': node.name, 'location_id': node.location_id}
        if show_all:
            data['attributes'] = dict(attrs) if isinstance(attrs, dict) else {}
            data['out_edges'] = [{'type_code': rel.type_code, 'target_id': rel.target_id, 'target_type': getattr(other, 'type_code', None), 'target_name': getattr(other, 'name', None), 'target_role': rel.target_role} for (rel, other) in out_edges]
...
def _find_query_descriptor(parsed: _ParsedFindArgs) -> dict:
    ...
    """
    return {'text': parsed.query, 'account': parsed.account_name, 'node_id': parsed.node_id, 'type_code': parsed.type_code, 'in_location': parsed.in_location, 'exact': parsed.exact, 'startswith': parsed.startswith, 'limit': parsed.limit, 'name': parsed.name, 'describe': parsed.describe, 'all': parsed.all}

def _empty_find_payload(parsed: _ParsedFindArgs) -> dict:
    return {'results': [], 'total': 0, 'next_offset': None, 'query': _find_query_descriptor(parsed)}
...
def _describe_no_match(parsed: _ParsedFindArgs) -> str:
    ...
    return 'No active nodes matched ' + ', '.join(parts) + '.' if parts else 'No active nodes matched.'

def _render_find_results(rows: List[Any], *, total: int, parsed: _ParsedFindArgs, safety_ceiling: Optional[int]=None) -> CommandResult:
    ...
    if safety_ceiling is not None and total > safety_ceiling:
        ...
        truncated_note = ''
    header = f'Found {len(rows)} node(s)' + truncated_note + (' for ' + ' '.join(descriptor) if descriptor else '') + ':'
    lines.append(header)
    result_data: List[dict] = []
    for n in rows:
        brief = _truncate(n.description, _DESC_PREVIEW_CHARS)
        loc_part = f' loc={n.location_id}' if n.location_id else ''
        lines.append(f'  [{n.id}] {n.type_code}: {n.name}{loc_part}' + (f'\n      {brief}' if brief else ''))
        result_data.append({'id': n.id, 'type_code': n.type_code, 'name': n.name, 'location_id': n.location_id, 'description': brief})
    lines.append('')
    ...
    if safety_ceiling is not None and total > safety_ceiling:
        ...
        next_offset = None
    return CommandResult.success_result('\n'.join(lines), data={'results': result_data, 'total': total, 'next_offset': next_offset, 'query': _find_query_descriptor(parsed)})

def _lookup_by_id(session, node_id: int):
    from app.models.graph import Node
    return session.query(Node).filter(Node.id == node_id, Node.is_active == True).first()
...
def _warn_if_short_trgm(value: Optional[str], *, label: str, min_len: int) -> None:
    ...
    if len(stripped) < min_len:
        import logging
        logging.getLogger(_FIND_LOGGER_NAME).warning('find: short trgm query on %s (%r, len=%d < %d); pg_trgm may fall back to seq-scan', label, stripped, len(stripped), min_len)

def _run_find_query(session, *, parsed: _ParsedFindArgs) -> Tuple[List[Any], int, Optional[int]]:
    ...
    from app.models.graph import Node
    cfg = _find_cfg()
    q = session.query(Node).filter(Node.is_active == True)
    if parsed.type_code:
    ...
    if search_term:
        _warn_if_short_trgm(search_term, label='query', min_len=cfg.min_trgm_chars)
        if parsed.exact:
    ...
    if parsed.name:
        ...
        if parsed.exact:
            ...
            q = q.filter(Node.name.ilike(f'%{parsed.name}%'))
    if parsed.describe:
        _warn_if_short_trgm(parsed.describe, label='--describe', min_len=cfg.min_trgm_chars)
    ...
    if len(probed) <= sql_limit:
        ...
        if parsed.all and total >= cfg.explain_on_all_over:
            try:
                import logging
                logging.getLogger(_FIND_LOGGER_NAME).warning('find: --all matched %d rows (>= %d); consider narrowing by --type/--in', total, cfg.explain_on_all_over)
            except Exception:
                pass
...
def _resolve_node_by_id_or_name(session, raw: str):
    ...
    try:
        ...
        pass
    return session.query(Node).filter(Node.name == raw, Node.is_active == True).first()
...
def _sample_out_edges(session, node_id: int, *, limit: Optional[int]=8):
    ...
    try:
        q = session.query(Relationship).filter(Relationship.source_id == node_id, Relationship.is_active == True).order_by(Relationship.type_code.asc(), Relationship.id.asc())
