...
def get_command_i18n_map(command_name: str, field: str='description') -> Dict[str, str]:
...
def get_localized_string_from_resource(command_name: str, field: str, requested_locale: str) -> str:
    m = get_command_i18n_map(command_name, field=field)
    if not m:
        return ''
    p = pick_i18n(m, requested_locale, fallbacks=FALLBACK_CHAIN, legacy_default=None)
    return (p.value or '').strip()

def get_command_i18n_text(command_name: str, key_path: str, locale: str, default: str) -> str:
...
def merge_description_i18n_for_ability(command_name: str, legacy: Optional[Dict[str, str]]) -> Dict[str, str]:
