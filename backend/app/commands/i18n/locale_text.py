...
_LOCALE_ALIASES: Dict[str, str] = {'': DEFAULT_LOCALE, 'zh': 'zh-CN', 'zh-cn': 'zh-CN', 'zh_hans': 'zh-CN', 'zh-hans': 'zh-CN', 'en': 'en-US', 'en-us': 'en-US'}
...
def pick_i18n(mapping: Optional[Dict[str, str]], requested_locale: str, fallbacks: Tuple[str, ...]=FALLBACK_CHAIN, *, legacy_default: Optional[str]=None) -> I18nPick:
    """
    ...
    for loc in unique_order:
        ...
        if v is not None and str(v).strip():
            return I18nPick(value=str(v).strip(), hit_locale=loc, fallback_used=loc != norm)
    base = (legacy_default or '').strip()
    return I18nPick(value=base, hit_locale=norm, fallback_used=True)

def _graph_account_language(db_session: Any, user_id: Optional[str], username: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        if user_id is not None:
            try:
                row = db_session.query(Node).filter(Node.id == int(user_id), Node.type_code == 'account').first()
                if row is not None and isinstance(row.attributes, dict):
                    return row.attributes.get('language')
            except (TypeError, ValueError):
                pass
        if username:
            row = db_session.query(Node).filter(Node.type_code == 'account', Node.name == str(username), Node.is_active == True).first()
            if row is not None and isinstance(row.attributes, dict):
                return row.attributes.get('language')
    except Exception:
        return None
...
def resolve_locale(context: 'CommandContext') -> str:
    """
    Effective locale: metadata override, then account node language, then default.
    """
    md: Optional[Dict] = getattr(context, 'metadata', None)
    if isinstance(md, dict) and md.get('locale') is not None and str(md.get('locale', '')).strip():
        return normalize_locale(str(md.get('locale')))
    lang = _graph_account_language(getattr(context, 'db_session', None), getattr(context, 'user_id', None), getattr(context, 'username', None))
...
def help_shell_for_locale(locale: str) -> Dict[str, str]:
    ...
    if loc.startswith('en'):
        return {'title_list': 'Available Commands', 'title_detail': 'Command Help: {name}', 'line_usage': 'Usage: {text}', 'line_aliases': 'Aliases: {items}', 'line_type': 'Type: {text}', 'line_description': 'Description: {text}', 'footer': "Type 'help <command>' for more.", 'err_not_found': 'Command not found: {name}'}
    return {'title_list': '可用命令', 'title_detail': '命令帮助: {name}', 'line_usage': '用法: {text}', 'line_aliases': '别名: {items}', 'line_type': '类型: {text}', 'line_description': '描述: {text}', 'footer': "使用 'help <command>' 查看单条命令帮助。", 'err_not_found': '未找到命令: {name}'}

def initial_metadata_for_session(*, db_session: Optional[Any], user_id: Optional[str], username: Optional[str], extra: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
