...
def invoke_command_line(*, actor_id: str, actor_name: str, session_id: str='npc_session', permissions: Optional[List[str]]=None, roles: Optional[List[str]]=None, command_line: str, caller: Optional[Any]=None, game_state: Optional[Dict[str, Any]]=None) -> CommandResult:
    ...
    with db_session_context() as db_session:
        ctx = CommandContext(user_id=str(actor_id), username=str(actor_name), session_id=str(session_id), permissions=list(permissions or []), roles=list(roles or []), caller=caller, game_state=game_state or {}, metadata={}, db_session=db_session)
        cmd = command_registry.get_command(command_name)
        if not cmd:
            return CommandResult.error_result(f"Command '{command_name}' not found")
        ability_node = db_session.query(Node).filter(Node.type_code == 'system_command_ability', Node.attributes['command_name'].astext == command_name, Node.is_active == True).first()
        if ability_node:
            ...
            try:
                ok = evaluate_policy_expr(invoke_lock, user_permissions=list(permissions or []), user_roles=list(roles or []), object_attrs=attrs)
