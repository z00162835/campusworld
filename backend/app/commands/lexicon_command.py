...
class LexiconCommand(AdminCommand):
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if not permission_checker.check_permission(list(context.permissions or []), self._PERM):
            return CommandResult.error_result(
                self._msg(context, "error.permission_denied", "Permission denied for lexicon management.")
            )
    ...
    def _build(self, context: CommandContext, _args: List[str]) -> CommandResult:
        ...
        msg = self._msg(
            context,
            "success.build",
            "Lexicon built id={id} revision={revision} rows={rows}",
            id=new_id,
            revision=revision,
            rows=len(rows),
        )
    ...
    def _list(self, context: CommandContext) -> CommandResult:
        ...
        for p in sorted(root.iterdir()):
            ...
            lines.append(
                self._msg(
                    context,
                    "list.row",
                    "{id}\tactive={active}\trev={rev}\tbuilt_at={built_at}",
                    id=vid,
                    active=is_act_label,
                    rev=rev,
                    built_at=built,
        ...
        if not lines:
            return CommandResult.success_result(
                self._msg(context, "success.empty_versions", "(no lexicon versions)")
            )
    ...
    def _delete(self, context: CommandContext, args: List[str]) -> CommandResult:
        if len(args) < 1:
            return CommandResult.usage_result(
                self._msg(context, "usage_line.delete", "lexicon -d <id>")
            )
        vid = args[0].strip()
        active = None
        ap = lexicon_paths.lexicon_active_pointer_path()
        ...
        if active and active == vid:
            return CommandResult.error_result(
                self._msg(
                    context,
                    "error.delete_active",
                    "Cannot delete the active lexicon; activate another version first.",
                )
            )
        p = lexicon_paths.lexicon_version_dir(vid)
        if not p.is_dir():
            return CommandResult.error_result(
                self._msg(context, "error.unknown_id", "Unknown lexicon id: {id}", id=vid)
            )
        shutil.rmtree(p)
    ...
    def _activate(self, context: CommandContext, args: List[str]) -> CommandResult:
        if len(args) < 1:
            return CommandResult.usage_result(
                self._msg(context, "usage_line.activate", "lexicon -a <id>")
            )
        vid = args[0].strip()
        p = lexicon_paths.lexicon_version_dir(vid)
        if not p.is_dir():
