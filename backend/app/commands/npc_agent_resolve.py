...
def resolve_npc_agent_by_handle(session: Session, handle: str) -> Tuple[Optional[Node], Optional[str]]:
    ...
    rows: List[Node] = session.query(Node).filter(Node.type_code == 'npc_agent', Node.is_active == True).all()
    matches: List[Node] = []
    seen: set[int] = set()
    for n in rows:
    ...
    if len(matches) == 0:
        return (None, f"unknown agent handle {h!r}. Type 'help' for available commands.")
    node = matches[0]
