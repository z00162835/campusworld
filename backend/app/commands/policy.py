...
class CommandPolicyEvaluator:
    ...
    def evaluate(self, command, context) -> AuthzDecision:
        ...
        try:
            ...
            logger.exception('authz policy_lookup_error actor=%s command=%s', actor, command_name)
        ...
        if required_roles_any:
            ok_role = any((permission_checker.check_role(user_roles, r) for r in required_roles_any))
            if not ok_role:
                return AuthzDecision(allowed=False, reason='missing_any_role', missing=list(required_roles_any), required_any=required_any, required_all=required_all, required_roles_any=required_roles_any)
        missing = [p for p in required_all if not permission_checker.check_permission(user_permissions, p)]
        if missing:
            return AuthzDecision(allowed=False, reason='missing_all_permissions', missing=missing, required_any=required_any, required_all=required_all, required_roles_any=required_roles_any)
        if required_any:
            ok_any = any((permission_checker.check_permission(user_permissions, p) for p in required_any))
            if not ok_any:
                return AuthzDecision(allowed=False, reason='missing_any_permissions', missing=list(required_any), required_any=required_any, required_all=required_all, required_roles_any=required_roles_any)
        return AuthzDecision(allowed=True, reason='allowed', missing=[], required_any=required_any, required_all=required_all, required_roles_any=required_roles_any)
