...
class CommandPolicy(Base):
    ...
    __table_args__ = (UniqueConstraint('command_name', name='uq_command_policy_command_name'),)
...
class CommandPolicyRepository:
    ...
    def get_policy(self, command_name: str) -> Optional[CommandPolicy]:
        if not command_name:
            return None
        return self.session.query(CommandPolicy).filter(CommandPolicy.command_name == command_name).first()

    def list_policies(self, *, enabled_only: bool=False) -> List[CommandPolicy]:
        query = self.session.query(CommandPolicy)
        if enabled_only:
            query = query.filter(CommandPolicy.enabled == True)
        return query.order_by(CommandPolicy.command_name.asc()).all()

    def upsert_policy(self, command_name: str, *, required_permissions_any: Optional[List[str]]=None, required_permissions_all: Optional[List[str]]=None, required_roles_any: Optional[List[str]]=None, enabled: bool=True, scope: str='global', updated_by: Optional[str]=None, commit: bool=True) -> CommandPolicy:
        row = self.get_policy(command_name)
        if row is None:
            row = CommandPolicy(command_name=command_name, required_permissions_any=list(required_permissions_any or []), required_permissions_all=list(required_permissions_all or []), required_roles_any=list(required_roles_any or []), enabled=bool(enabled), scope=scope or 'global', version=1, updated_by=updated_by)
    ...
    def to_policy_dict(self, row: CommandPolicy) -> Dict[str, Any]:
        return {'id': row.id, 'command_name': row.command_name, 'required_permissions_any': list(row.required_permissions_any or []), 'required_permissions_all': list(row.required_permissions_all or []), 'required_roles_any': list(row.required_roles_any or []), 'enabled': bool(row.enabled), 'scope': row.scope, 'version': row.version, 'updated_by': row.updated_by}
...
def ensure_default_command_policies(session: Session) -> int:
    ...
    for cmd in command_registry.get_all_commands():
        ...
        if not seed['required_permissions_any'] and (not seed['required_permissions_all']) and (not seed['required_roles_any']) and (getattr(cmd, 'command_type', None) == CommandType.ADMIN):
            seed['required_permissions_any'] = ['admin.*']
        repo.upsert_policy(cmd.name, required_permissions_any=seed['required_permissions_any'], required_permissions_all=seed['required_permissions_all'], required_roles_any=seed['required_roles_any'], enabled=True, updated_by='bootstrap', commit=False)
        created += 1
    session.commit()
    return created
