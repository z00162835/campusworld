...
def target_room_display(node: Any) -> Dict[str, str]:
    ...
    display_name = str(attrs.get('room_name') or attrs.get('display_name') or getattr(node, 'name', '') or attrs.get('package_node_id') or '?').strip()
...
def connects_to_exits_from_room(session: Any, room_node_id: int) -> List[Dict[str, Any]]:
    """
    ...
    outgoing = session.query(Relationship, Node).join(Node, Relationship.target_id == Node.id).filter(Relationship.source_id == room_node_id, Relationship.type_code == 'connects_to', Relationship.is_active == True, Node.is_active == True).all()
    for (rel, target) in outgoing:
        ...
        entries.append({'direction': direction, 'target_id': int(target.id), 'target_display_name': tgt['target_display_name'], 'target_short_desc': tgt['target_short_desc'], 'target_package_node_id': tgt['target_package_node_id'], 'is_cross_world': False})
...
def connects_to_exit_entries_for_look(session: Any, room_node_id: int) -> List[Dict[str, Any]]:
    ...
    for row in connects_to_exits_from_room(session, room_node_id):
        out.append({'direction': row['direction'], 'target_display_name': row['target_display_name'], 'target_short_desc': row['target_short_desc'], 'target_package_node_id': row['target_package_node_id'], 'is_cross_world': row.get('is_cross_world', False)})
    return out
