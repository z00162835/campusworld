...
def parse_space_args(args: Sequence[str]) -> _SpaceParse:
    ...
    while i < len(toks):
        ...
        if t == '-t':
            ...
            return _SpaceParse(error_key='error.unknown_flag', error_tokens={'token': t})
        else:
            return _SpaceParse(mode='usage')
...
def _node_space_trait(session: Any, node: Any) -> bool:
    ...
    if str(getattr(nt, 'trait_class', '') or '').upper() == 'SPACE':
        return True
    if str(getattr(node, 'type_code', '') or '') in ('building', 'building_floor', 'room', 'world'):
        return True
    return False
...
def _rows_occupants(session: Any, space_id: int) -> List[Any]:
    ...
    return session.query(Node).join(NodeType, Node.type_id == NodeType.id).filter(Node.location_id == space_id, Node.is_active == True, (NodeType.trait_class == 'AGENT') | (Node.type_code == 'account')).order_by(Node.id).all()

def _rows_devices(session: Any, space_id: int) -> List[Any]:
    from app.models.graph import Node, NodeType
    return session.query(Node).join(NodeType, Node.type_id == NodeType.id).filter(Node.location_id == space_id, Node.is_active == True, NodeType.trait_class == 'DEVICE').order_by(Node.id).all()

def _child_space_nodes(session: Any, space: Any) -> Tuple[List[Any], str, bool]:
    ...
    def _by_location(want_type: Optional[str]=None) -> List[Any]:
        q = session.query(Node).join(NodeType, Node.type_id == NodeType.id).filter(Node.location_id == int(space.id), Node.is_active == True)
        if want_type:
    ...
    if tc == 'building':
        ...
        if bpkg:
            ...
            hit = _nodes_by_attr_in_package(session, type_code='building_floor', attr_key='building_id', attr_value=bpkg, world_id=wid)
            if hit:
    ...
    if tc == 'building_floor':
        ...
        if fpkg:
            wid = str(attrs.get('world_id') or '').strip()
            hit = _nodes_by_attr_in_package(session, type_code='room', attr_key='floor_id', attr_value=fpkg, world_id=wid)
            if hit:
                return (hit, 'fallback_attr', True)
        return ([], 'children', False)
...
def _nodes_by_attr_in_package(session: Any, *, type_code: str, attr_key: str, attr_value: str, world_id: str='') -> List[Any]:
    """
    ...
    try:
        q = session.query(Node).filter(Node.type_code == type_code, Node.is_active == True, Node.attributes[attr_key].astext == v)
        if w:
            q = q.filter(Node.attributes['world_id'].astext == w)
        return list(q.order_by(Node.id).all())
    except Exception:
        acc: List[Any] = []
        q2 = session.query(Node).filter(Node.type_code == type_code, Node.is_active == True).order_by(Node.id)
        for n in q2.all():
...
class SpaceCommand(SystemCommand):
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if session is None:
            return CommandResult.error_result(_t(loc, 'error.no_session', 'requires an active DB session'))
        p = parse_space_args(args)
        if not p.ok:
            if p.error_key == 'error.unknown_flag' and p.error_tokens:
                msg = _t(loc, p.error_key, 'unknown flag: {token}', token=p.error_tokens.get('token', '?'))
            else:
                msg = _t(loc, p.error_key or 'usage', self.get_usage())
            return CommandResult.error_result(msg, is_usage=True)
        if p.mode == 'usage':
            return CommandResult.error_result(_t(loc, 'usage.line', self.get_usage()), is_usage=True)
        if p.mode == 'types':
            codes = _list_space_type_codes(session)
            if not codes:
                return CommandResult.success_result(_t(loc, 'type_list.empty', '(no SPACE node types)'), data={'type_codes': []})
            title = _t(loc, 'type_list.title', 'SPACE type_code')
            lines = [title, '=' * 32]
            for c in codes:
                lines.append(c)
            lines.append(_t(loc, 'type_list.footer', 'total: {n}').format(n=len(codes)))
            return CommandResult.success_result('\n'.join(lines), data={'type_codes': codes})
        assert p.node_id is not None
        node = session.query(Node).filter(Node.id == int(p.node_id), Node.is_active == True).first()
        if node is None or not _node_space_trait(session, node):
            return CommandResult.error_result(_t(loc, 'error.not_found', 'node not found or not a space'))
        nt = session.query(NodeType).filter(NodeType.id == node.type_id).first()
        ...
        (s4_rows, s4_data, s4_mode, s4_fb) = self._section4(session, node, ntype_name, loc)
        titles = [_t(loc, 'section.title.1', 'Space description:'), _t(loc, 'section.title.2', 'Space occupants:'), _t(loc, 'section.title.3', 'Space devices:'), _t(loc, 'section.title.4', 'Space relations:')]
        msg_parts: List[str] = [f'{titles[0]}\n' + ('\n'.join(s1_lines) if s1_lines else '-'), f'\n{titles[1]}\n' + ('\n'.join(s2_rows) if s2_rows else _t(loc, 'empty.row', '-')), f'\n{titles[2]}\n' + ('\n'.join(s3_rows) if s3_rows else _t(loc, 'empty.row', '-')), f'\n{titles[3]}\n' + ('\n'.join(s4_rows) if s4_rows else _t(loc, 'empty.row', '-'))]
        message = '\n'.join(msg_parts)
        s1_text = '\n'.join(s1_lines) if s1_lines else ''
        data: Dict[str, Any] = {'space_node': {'id': int(node.id), 'type_code': str(node.type_code or ''), 'name': str(node.name or ''), 'parent_id': int(node.location_id) if node.location_id else None}, 'section1_appearance': {'message_fragment': s1_text, 'lines': s1_lines}, 'section2_occupants': s2_data, 'section3_devices': s3_data, 'section4_next_or_adjacent': s4_data, 'section4_mode': s4_mode, 'section4_fallback': s4_fb}
        return CommandResult.success_result(message, data=data)

    def _section1_lines(self, session: Any, node: Any, ntype: str, loc: str) -> List[str]:
        ...
        if ntype == 'room':
            ...
            for e in connects_to_exits_from_room(session, int(node.id)):
                ...
                lines.append(_t(loc, 's1.exit', 'exit {dir}: {name}', dir=d, name=tdn))
            return lines
        _name_line()
    ...
    def _section_occupants(self, nodes: List[Any], loc: str) -> Tuple[List[str], List[Dict[str, Any]]]:
        if not nodes:
            return ([], [])
        ...
        for n in nodes:
            ...
            data.append({'id': int(n.id), 'name': dname, 'type_code': str(n.type_code or '')})
            rows.append(_format_table_row([str(n.id), dname, str(n.type_code or '')], widths))
        return (rows, data)

    def _section_devices(self, nodes: List[Any], loc: str) -> Tuple[List[str], List[Dict[str, Any]]]:
        if not nodes:
            return ([], [])
        h_id = _t(loc, 'table.header.id', 'id')
        h_name = _t(loc, 'table.header.name', 'name')
        ...
        for n in nodes:
            a = dict(n.attributes or {})
            st = str(a.get('status') or a.get('operational_status') or a.get('device_status') or '-')
            dname = _node_display_name(n)
            data.append({'id': int(n.id), 'name': dname, 'status': st})
            rows.append(_format_table_row([str(n.id), dname, st], widths))
        return (rows, data)

    def _section4(self, session: Any, node: Any, ntype: str, loc: str) -> Tuple[List[str], List[Dict[str, Any]], str, bool]:
        from app.models.graph import Node
        if ntype == 'room':
            ...
            for e in ex:
                ...
                desc = ''
                if tnode:
                    desc = str(tnode.description or (tnode.attributes or {}).get('room_short_description') or '').strip()[:200]
                name = str(e.get('target_display_name') or '')
                direction = str(e.get('direction') or '')
                drow = {'id': tid, 'name': name, 'direction': direction, 'description': desc}
                data.append(drow)
        ...
        for c in children:
            ...
            data2.append({'id': int(c.id), 'name': cname, 'direction': None, 'description': desc})
            lines2.append(_format_table_row([str(c.id), cname, desc], widths2))
