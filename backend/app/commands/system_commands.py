...
class StatsCommand(SystemCommand):
    ...
    def execute(self, context, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            err_template = get_command_i18n_text('stats', 'error', loc, 'Failed to get system stats: {error}')
...
class WhoamiCommand(SystemCommand):
    ...
    def execute(self, context, args: List[str]) -> CommandResult:
        ...
        from app.commands.i18n.locale_text import resolve_locale
        loc = resolve_locale(context)
        template = get_command_i18n_text('whoami', 'current_user', loc, 'Current user: {username}')
...
class WhoCommand(SystemCommand):
    ...
    def execute(self, context, args: List[str]) -> CommandResult:
        ...
        if session_manager is None or not hasattr(session_manager, 'get_active_sessions'):
            return CommandResult.error_result(self._who_text(loc, 'error.unavailable', 'Online user list unavailable.'), error='session_manager not available in context')
        sessions = list(session_manager.get_active_sessions() or [])
        if not sessions:
            return CommandResult.success_result(self._who_text(loc, 'error.no_sessions', 'No users online.'))
        ...
        lines = [self._who_text(loc, 'title', 'CampusWorld Online Users'), '========================', f"{self._who_text(loc, 'header.player', 'Player'):<12} {self._who_text(loc, 'header.location', 'Location'):<28} {self._who_text(loc, 'header.idle', 'Idle'):<5}  {self._who_text(loc, 'header.duration', 'Duration'):<8}", '----------   ----------------------------  -----  --------']
        ...
        for sess in sessions:
            ...
            if gh is not None and hasattr(gh, 'get_user_location') and (user_id is not None):
                try:
                    ...
                    location_error = True
                    location = '-'
            idle = self._format_idle(self._seconds_since(getattr(sess, 'last_activity', None), now))
            duration = self._format_duration(self._seconds_since(getattr(sess, 'connected_at', None), now))
            lines.append(f'{username:<12} {location:<28} {idle:<5}  {duration:<8}')
        lines.append('')
        lines.append(self._who_text(loc, 'footer', '{n} users online ({m} active sessions).').format(n=len(sessions), m=len(sessions)))
        if location_error:
            lines.append(self._who_text(loc, 'warning.location_unavailable', 'Location information unavailable.'))
...
def _parse_type_args(args: List[str]) -> _ParsedTypeArgs:
    ...
    for token in args:
        ...
        return _ParsedTypeArgs(show_all=False, error='type does not accept positional arguments; use -a/--all to show all')
...
class TypeCommand(SystemCommand):
    ...
    def execute(self, context, args: List[str]) -> CommandResult:
        ...
        if session is None:
            return CommandResult.error_result(self._t(loc, 'error.no_session', 'Node type list unavailable (no DB session).'), error='db_session not available in context')
        from app.models.graph import NodeType
        try:
            rows = list(NodeType.get_active_types(session))
        except Exception as e:
            return CommandResult.error_result(self._t(loc, 'error.no_session', 'Node type list unavailable (no DB session).'), error=f'node_types query failed: {e}')
        rows.sort(key=self._sort_key)
        total = len(rows)
        if total == 0:
            return CommandResult.success_result(self._t(loc, 'error.empty', 'No active node types.'), data={'show_all': parsed.show_all, 'total': 0, 'items': []})
        ...
        for r in shown:
            ...
            items.append({'type_code': getattr(r, 'type_code', None), 'type_name': getattr(r, 'type_name', None), 'description': raw_desc})
        lines.append('')
        if parsed.show_all:
            footer = self._t(loc, 'footer.all', '{total} active types.').format(total=total)
        else:
            footer = self._t(loc, 'footer.preview', 'Showing {shown} of {total}. Pass -a to see all.').format(shown=len(shown), total=total)
        lines.append(footer)
        data = {'show_all': parsed.show_all, 'total': total, 'items': items}
        return CommandResult.success_result('\n'.join(lines), data=data)
SYSTEM_COMMANDS = [HelpCommand(), StatsCommand(), VersionCommand(), QuitCommand(), TimeCommand(), TypeCommand(), WhoCommand(), WhoamiCommand()]
