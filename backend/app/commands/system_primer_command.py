...
class PrimerCommand(SystemCommand):
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if parsed.raw and 'admin.doc.read' not in (context.permissions or []):
            return CommandResult.error_result("primer --raw requires permission 'admin.doc.read'")
        if parsed.for_agent and 'admin.agent.read' not in (context.permissions or []):
            return CommandResult.error_result("primer --for requires permission 'admin.agent.read'")
        try:
            text = build_ontology_primer(section=parsed.section, for_agent=parsed.for_agent, raw=parsed.raw, session=getattr(context, 'db_session', None), primer_command_context=context)
