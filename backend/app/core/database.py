    ...
    if _engine is None:
        try:
            ...
            if 'sqlite' in database_url:
                ...
                connect_args = {'connect_timeout': 10, 'application_name': 'campusworld'}
            poolclass = StaticPool if 'sqlite' in database_url else QueuePool
            _engine = create_engine(database_url, poolclass=poolclass, pool_pre_ping=db_config.get('pool_pre_ping', True), pool_recycle=db_config.get('pool_recycle', 300), pool_size=db_config.get('pool_size', 20), max_overflow=db_config.get('max_overflow', 30), echo=db_config.get('echo', False), connect_args=connect_args)
        except Exception as e:
...
def init_db() -> bool:
    ...
    try:
        import app.models
        import app.commands.policy_store
        engine = _create_engine()
        if not Base.metadata.tables:
            raise RuntimeError('未加载到任何 ORM 表定义（Base.metadata.tables 为空）。请检查模型是否正确导入并继承 app.core.database.Base。')
...
def get_database_info() -> Dict[str, Any]:
    ...
    try:
        config_manager = _get_config_manager()
        db_config = _get_database_config()
        return {'engine': db_config.get('engine', 'unknown'), 'host': db_config.get('host', 'unknown'), 'port': db_config.get('port', 'unknown'), 'name': db_config.get('name', 'unknown'), 'pool_size': db_config.get('pool_size', 20), 'max_overflow': db_config.get('max_overflow', 30), 'echo': db_config.get('echo', False), 'connected': check_db_connection()}
    except Exception as e:
...
def db_session_context() -> Session:
    ...
    try:
        ...
        logger.error(f'Database operation failed, rolling back transaction: {e}')
        session.rollback()
        raise
    except Exception as e:
...
def execute_write(write_func: Callable[[Session], Any], default_return=None) -> Any:
    ...
    try:
        ...
        return default_return
__all__ = ['get_db', 'get_db_session', 'get_engine', 'init_db', 'check_db_connection', 'get_database_info', 'Base', 'SessionLocal', 'engine', 'db_session_context', 'get_session', 'run_in_session', 'run_in_session_with_result', 'execute_query', 'execute_write']
