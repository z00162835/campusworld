...
def log_aico_llm_call(config_manager: Any, *, phase: str, system: str, user: str, spec: Any, skipped: bool) -> None:
    ...
    spec_dict = {'mode': mode.value if mode is not None and hasattr(mode, 'value') else str(mode), 'model': getattr(spec, 'model', None), 'temperature': getattr(spec, 'temperature', None), 'max_tokens': getattr(spec, 'max_tokens', None), 'timeout_sec': getattr(spec, 'timeout_sec', None)}
    log.debug('aico_llm_call phase=%s skipped=%s run_id=%s correlation_id=%s spec=%s\n--- system ---\n%s\n--- user ---\n%s', phase, skipped, get_aico_run_id_from_context(), get_aico_correlation_from_context(), spec_dict, truncate_for_aico_log(system or '', budget), truncate_for_aico_log(user or '', budget))
...
def log_aico_tool_observations_text(config_manager: Any, *, phase: str, observation_text: str) -> None:
    ...
    budget = get_aico_max_phase_output_chars(config_manager)
    log = logging.getLogger(LoggerNames.AICO_AGENT)
    log.debug('aico_tool_observations phase=%s run_id=%s correlation_id=%s chars=%s\n%s', phase, get_aico_run_id_from_context(), get_aico_correlation_from_context(), len(observation_text), truncate_for_aico_log(observation_text, budget))

def log_aico_http_exchange(config_manager: Any, *, url: str, status_code: int, elapsed_ms: float, request_body: Any, response_data: Any) -> None:
    if not should_emit_aico_full_chain_logs(config_manager):
        return
    budget = get_aico_max_phase_output_chars(config_manager)
    log = logging.getLogger(LoggerNames.AICO_AGENT)
    ...
    log.debug('aico_http url=%s status=%s elapsed_ms=%.2f run_id=%s correlation_id=%s\n--- request ---\n%s\n--- response ---\n%s', url, status_code, elapsed_ms, get_aico_run_id_from_context(), get_aico_correlation_from_context(), req_preview, resp_preview)
...
def configure_aico_observability_logging(config_manager: Any) -> None:
    ...
    fh = FlushingRotatingFileHandler(full_path, maxBytes=max_size, backupCount=backup_count, encoding='utf-8')
