...
class LoggingContext:
    ...
    def set_user_context(self, user_id: str, username: str=None, session_id: str=None) -> None:
        ...
        Args:
            ...
            session_id: 会话ID
        """
        self.set_context(user_id=user_id, username=username, session_id=session_id)
    ...
    def set_request_context(self, request_id: str, method: str=None, path: str=None, ip: str=None) -> None:
        ...
        Args:
            ...
            ip: IP地址
        """
        self.set_context(request_id=request_id, method=method, path=path, ip=ip)
    ...
    def set_operation_context(self, operation: str, resource: str=None, action: str=None) -> None:
        ...
        Args:
            ...
            action: 动作名称
        """
        self.set_context(operation=operation, resource=resource, action=action)
    ...
    def get_user_context(self) -> Dict[str, Any]:
        ...
        return {'user_id': context.get('user_id'), 'username': context.get('username'), 'session_id': context.get('session_id')}
    ...
    def get_request_context(self) -> Dict[str, Any]:
        ...
        """
        context = self.get_context()
        return {'request_id': context.get('request_id'), 'method': context.get('method'), 'path': context.get('path'), 'ip': context.get('ip')}
    ...
    def get_operation_context(self) -> Dict[str, Any]:
        ...
        """
        context = self.get_context()
        return {'operation': context.get('operation'), 'resource': context.get('resource'), 'action': context.get('action')}
