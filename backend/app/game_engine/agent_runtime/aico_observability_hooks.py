...
class AicoObservabilityTickHooks:
    ...
    def __init__(self, logger: logging.Logger, *, max_phase_output_chars: int=4000):
        self._log = logger
    ...
    def on_before_phase(self, phase: ThinkingPhaseId, ctx: FrameworkRunContext) -> None:
        ...
        self._log.info('aico_tick_start agent_node_id=%s correlation_id=%s user_message_chars=%s', ctx.agent_node_id, ctx.correlation_id, len(msg.strip()))

    def on_after_phase(self, phase: ThinkingPhaseId, ctx: FrameworkRunContext, *, phase_llm_output: str, skipped: bool=False) -> None:
        out = phase_llm_output or ''
        preview = self._truncate(out).replace('\n', '\\n')
        self._log.info('aico_phase_end phase=%s agent_node_id=%s correlation_id=%s skipped=%s output_chars=%s output_preview=%s', phase.value, ctx.agent_node_id, ctx.correlation_id, skipped, len(out), preview)
