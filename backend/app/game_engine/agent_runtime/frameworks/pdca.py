...
class PDCAFramework(ThinkingFramework):
    ...
    def run(self, ctx: FrameworkRunContext) -> FrameworkRunResult:
        ...
        self._memory.start_run(run_id=run_id, correlation_id=correlation, phase=PDCAPhase.plan.value, command_trace=list(trace), status='running')
        ...
        cmd_entry: Dict[str, Any] = {'command': 'graph.patch_device_state', 'args': [str(device_node_id)] if device_node_id is not None else [], 'patch': patch}
        trace.append(cmd_entry)
        self._memory.update_run(run_id, PDCAPhase.do.value, trace, 'running', graph_ops_summary={'patched_nodes': [device_node_id] if device_node_id else []})
        ok = True
        ...
        self._memory.finish_run(run_id, PDCAPhase.act.value, trace, 'success' if ok else 'failed', graph_ops_summary=summary)
        self._memory.append_raw('audit', {'framework': 'PDCA', 'run_id': str(run_id), 'result': 'success' if ok else 'failed'})
