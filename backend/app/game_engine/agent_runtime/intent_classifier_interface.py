...
class IntentClassifier(Protocol):
    ...
    def classify_intent(self, user_message: str, *, agent_id: Optional[str]=None, metadata: Optional[Dict[str, object]]=None) -> IntentClassification:
        ...
...
class ChainedIntentClassifier:
    ...
    def classify_intent(self, user_message: str, *, agent_id: Optional[str]=None, metadata: Optional[Dict[str, object]]=None) -> IntentClassification:
        try:
            ...
            _LOG.warning('intent_classifier_primary_failed', extra={'error': str(exc)})
...
class RuleFallbackIntentClassifier:
    ...
    def classify_intent(self, user_message: str, *, agent_id: Optional[str]=None, metadata: Optional[Dict[str, object]]=None) -> IntentClassification:
        ...
        execute_patterns = ('请执行', '确认执行', '继续执行', '帮我创建', '\\bexecute\\b', '\\bcreate\\b')
        verify_patterns = ('是否存在', '现在是什么状态', '查一下', '\\bcurrent state\\b', '\\bdoes it exist\\b')
        informational_patterns = ('例子', '怎么用', '语法', '\\bhelp\\b', '\\bexample\\b', '\\busage\\b')
        reason_tokens: List[str] = []
        ...
        if execute_hit and has_confirmation:
            return IntentClassification(intent='execute', confidence=0.75, reason_tokens=reason_tokens, source='rule_fallback')
        if verify_hit:
            return IntentClassification(intent='verify_state', confidence=0.7, reason_tokens=reason_tokens, source='rule_fallback')
        if execute_hit and (not has_confirmation):
            reason_tokens.append('execute_without_confirmation_downgraded')
        return IntentClassification(intent='informational', confidence=0.65, reason_tokens=reason_tokens, source='rule_fallback')

def classify_intent(user_message: str, *, agent_id: Optional[str]=None, metadata: Optional[Dict[str, object]]=None, classifier: Optional[IntentClassifier]=None) -> IntentClassification:
    ...
    try:
        ...
        pass
    return IntentClassification(intent='informational', confidence=0.0, reason_tokens=['classifier_failure_default_informational'], source='fallback_default')
