...
_INTENT_KEYS = frozenset({'use_intent_slm', 'artifact_dir', 'intent_classifier_allowed_path_prefixes', 'max_new_tokens', 'system_prompt_file'})
...
def default_intent_classifier_allow_prefixes(root: Path) -> List[Path]:
    ...
    return [(root / 'app/models/agent_model/intent_classifier/artifacts').resolve()]
...
def merge_intent_classifier_settings(extra: Optional[Mapping[str, Any]], node_attributes: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
...
def _normalize_prefix_list(raw: Any, backend: Path) -> List[Path]:
...
def _load_default_system_prompt() -> str:
    pkg = backend_root() / 'app/models/agent_model/intent_classifier/prompts/intent_classifier_system_prompt.txt'
...
def resolve_intent_classifier_runtime(extra: Optional[Mapping[str, Any]], node_attributes: Optional[Mapping[str, Any]], *, backend: Optional[Path]=None) -> IntentClassifierRuntimeConfig:
    ...
    if isinstance(sp_file, str) and sp_file.strip():
        ...
        system_prompt_text = _load_default_system_prompt()
    if not use_slm:
        return IntentClassifierRuntimeConfig(use_intent_slm=False, artifact_root=None, max_new_tokens=max_new_tokens, system_prompt_text=system_prompt_text, resolve_error=None)
    raw_ad = merged.get('artifact_dir')
    if not isinstance(raw_ad, str) or not raw_ad.strip():
        return IntentClassifierRuntimeConfig(use_intent_slm=True, artifact_root=None, max_new_tokens=max_new_tokens, system_prompt_text=system_prompt_text, resolve_error='missing_artifact_dir')
    try:
        candidate = _resolve_path_entry(raw_ad.strip(), root)
        resolved_root = candidate.resolve()
    except OSError as exc:
        return IntentClassifierRuntimeConfig(use_intent_slm=True, artifact_root=None, max_new_tokens=max_new_tokens, system_prompt_text=system_prompt_text, resolve_error=f'path_resolve_error:{exc}')
    if not _is_under_allowed(resolved_root, prefixes):
        _LOG.warning('intent_classifier_artifact_path_denied', extra={'artifact_dir': str(resolved_root), 'allowed_prefixes': [str(p) for p in prefixes]})
        return IntentClassifierRuntimeConfig(use_intent_slm=True, artifact_root=None, max_new_tokens=max_new_tokens, system_prompt_text=system_prompt_text, resolve_error='artifact_dir_not_allowlisted')
    layout_err = _validate_artifact_layout(resolved_root)
    if layout_err:
        _LOG.warning('intent_classifier_artifact_layout_invalid', extra={'artifact_dir': str(resolved_root), 'reason': layout_err})
        return IntentClassifierRuntimeConfig(use_intent_slm=True, artifact_root=None, max_new_tokens=max_new_tokens, system_prompt_text=system_prompt_text, resolve_error=layout_err)
    return IntentClassifierRuntimeConfig(use_intent_slm=True, artifact_root=resolved_root, max_new_tokens=max_new_tokens, system_prompt_text=system_prompt_text, resolve_error=None)
...
def build_intent_classifier_for_tick(cfg: IntentClassifierRuntimeConfig):
    ...
    if not cfg.use_intent_slm or cfg.artifact_root is None:
        if cfg.use_intent_slm and cfg.resolve_error:
            _LOG.warning('intent_classifier_slm_disabled', extra={'reason': cfg.resolve_error})
        return None
    try:
        import peft
    except ImportError:
        ...
        return None
    primary = PeftIntentClassifier(artifact_root=cfg.artifact_root, max_new_tokens=cfg.max_new_tokens, system_prompt=cfg.system_prompt_text)
