...
from app.game_engine.agent_runtime.llm_providers.minimax_anthropic import MinimaxAnthropicMessagesHttpLlmClient
...
def build_http_llm_client(cfg: AgentLlmServiceConfig, api_key: str) -> LlmClient:
    ...
    low = base.lower()
    if '/anthropic' in low:
        return MinimaxAnthropicMessagesHttpLlmClient(base_url=base, api_key=api_key, default_model=cfg.model or 'MiniMax-M2.7', default_temperature=cfg.temperature, default_max_tokens=cfg.max_tokens)
    if 'chatcompletion' in low or '/text/' in low:
        return MinimaxNativeTextHttpLlmClient(base_url=base, api_key=api_key, default_model=cfg.model or 'MiniMax-M2.7', default_temperature=cfg.temperature, default_max_tokens=cfg.max_tokens)
    return OpenAiCompatibleHttpLlmClient(base_url=base or 'https://api.openai.com/v1', api_key=api_key, default_model=cfg.model or 'gpt-4o-mini', default_temperature=cfg.temperature, default_max_tokens=cfg.max_tokens)
