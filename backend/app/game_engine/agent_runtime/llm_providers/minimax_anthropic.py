...
def _tool_schema_to_anthropic(schema: ToolSchema) -> Dict[str, Any]:
    return {'name': schema.name, 'description': schema.description, 'input_schema': dict(schema.input_schema)}
...
def _validate_anthropic_tool_messages(messages: Sequence[Dict[str, Any]], *, allowed_tool_names: set[str]) -> None:
        ...
        if role == 'assistant':
            ...
            for b in blocks:
                ...
                if nm not in allowed_tool_names:
                    raise ValueError(f'messages[{idx}] assistant tool_use name {nm!r} not in request tools {sorted(allowed_tool_names)!r}')
                ids.add(tid)
            ...
            for b in blocks:
                ...
                if btype == 'tool_result':
                    if seen_text:
                        raise ValueError(f'messages[{idx}] user: tool_result blocks must come before any text block in the same message')
            ...
            if tool_result_ids:
                if not pending_ids:
                    raise ValueError(f'messages[{idx}] user has tool_result but no preceding assistant tool_use ids (protocol violation)')
                got = set(tool_result_ids)
                if got != pending_ids:
                    raise ValueError(f'messages[{idx}] tool_result ids {sorted(got)!r} != pending assistant tool_use ids {sorted(pending_ids)!r}')
                pending_ids = None
            elif pending_ids:
                raise ValueError(f'messages[{idx}] user message must carry tool_result for pending tool_use ids {sorted(pending_ids)!r}')
    if pending_ids:
        raise ValueError(f'messages end with assistant tool_use without a following user tool_result turn (pending ids: {sorted(pending_ids)!r})')
    ...
    * ``ToolResultsTurn`` → ``{role: user, content: [{type: tool_result, ...}]}``.
    ...
    for t in turns:
        if isinstance(t, TextTurn):
            ...
            messages.append({'role': role, 'content': [{'type': 'text', 'text': t.text or ' '}]})
            ...
            for r in t.results:
                content_blocks.append({'type': 'tool_result', 'tool_use_id': r.id, 'content': _tool_result_content_wire(r.text or ''), 'is_error': not r.ok})
            if content_blocks:
