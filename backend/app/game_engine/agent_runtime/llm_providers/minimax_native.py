...
class MinimaxNativeTextHttpLlmClient:
    ...
    def complete(self, *, system: str, user: str, call_spec: Optional[LlmCallSpec]=None) -> str:
        ...
        body: Dict[str, Any] = {'model': model, 'messages': [{'role': 'system', 'content': system or ' '}, {'role': 'user', 'content': user or ' '}], 'stream': False, 'temperature': float(temperature), 'max_completion_tokens': int(max_tokens)}
