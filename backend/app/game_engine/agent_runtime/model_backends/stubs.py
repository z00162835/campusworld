...
class StubCrossEncoderRerankBackend:
    ...
    def score_pairs(self, query: str, passages: Sequence[str]) -> List[float]:
        ...
        for p in passages:
            ...
            s = int.from_bytes(raw[:4], 'big') / 4294967295
...
class StubChatGenerativeBackend:
    ...
    model_id = 'stub-chat'
    _EMPTY_SLOT = '{"target_hint":null,"named_spans":[],"entities":[],"mandatory_tools":[]}'
    ...
    def complete_chat(self, messages: Sequence[Dict[str, Any]], *, max_new_tokens: int=256, temperature: float=0.0) -> ChatGenerativeResult:
        return ChatGenerativeResult(text=self._fixed_text, latency_ms=0.0, backend_kind=self.backend_kind, model_id=self.model_id)
