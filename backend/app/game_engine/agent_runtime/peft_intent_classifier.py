...
from app.models.agent_model.intent_classifier.runtime.inference import greedy_decode_intent_json, intent_classifier_torch_device_dtype, validate_intent_payload
...
class PeftIntentClassifier(IntentClassifier):
    ...
    def __init__(self, *, artifact_root: Path, max_new_tokens: int, system_prompt: str):
    ...
    def classify_intent(self, user_message: str, *, agent_id: Optional[str]=None, metadata: Optional[Dict[str, object]]=None) -> IntentClassification:
        ...
        try:
            obj = greedy_decode_intent_json(model=bundle.model, tokenizer=bundle.tokenizer, device=bundle.device, system_prompt=self._system_prompt, user_message=text, max_new_tokens=self._max_new_tokens)
        except Exception as exc:
            elapsed_ms = (time.perf_counter() - t0) * 1000.0
            _LOG.warning('intent_slm_inference_failed', extra={'error': str(exc), 'intent_slm_latency_ms': round(elapsed_ms, 3)})
            raise
        (intent, confidence, reason_tokens) = validate_intent_payload(obj)
        elapsed_ms = (time.perf_counter() - t0) * 1000.0
        _LOG.info('intent_slm_ok', extra={'intent': intent, 'intent_confidence': confidence, 'intent_slm_latency_ms': round(elapsed_ms, 3), 'intent_source': 'small_model'})
        return IntentClassification(intent=intent, confidence=confidence, reason_tokens=reason_tokens, source='small_model', latency_ms=round(elapsed_ms, 3))
