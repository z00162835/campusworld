...
def merge_phase_config(phase: str, instance_phase_llm: Dict[str, PhaseLlmPhaseConfig], ctx_overrides: Optional[Dict[str, Any]]) -> PhaseLlmPhaseConfig:
...
def to_llm_call_spec(phase_cfg: PhaseLlmPhaseConfig, *, mode_models: Dict[str, str], default_model: str) -> LlmCallSpec:
    ...
    return LlmCallSpec(mode=phase_cfg.mode, model=resolved_model, temperature=phase_cfg.temperature, max_tokens=phase_cfg.max_tokens, timeout_sec=phase_cfg.timeout_sec, extra=dict(phase_cfg.extra or {}))
