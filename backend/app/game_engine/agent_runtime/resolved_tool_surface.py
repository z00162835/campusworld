...
from app.game_engine.agent_runtime.execution_gate import evaluate_execution_gate, guard_error_message
...
def build_resolved_tool_surface(*, node_tool_allowlist: Sequence[str], tool_command_context: CommandContext, blocked_names: Optional[FrozenSet[str]]=None) -> ResolvedToolSurface:
    """
...
class PreauthorizedToolExecutor:
    ...
    def execute_command(self, context: CommandContext, command_name: str, args: List[str]) -> CommandResult:
        ...
        if primary not in self.surface.allowed_command_names:
            return CommandResult.error_result(f'command not allowed for agent tools: {command_name}', error='not_on_resolved_surface')
        if cmd is None:
        ...
        data.update({'guard_decision': decision.reason_code, 'guard_intent': decision.intent, 'guard_effective_profile': decision.effective_profile})
        res.data = data
        return res
