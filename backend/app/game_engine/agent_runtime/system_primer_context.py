...
from app.game_engine.agent_runtime.command_caller_graph import resolve_caller_location_id, resolve_caller_node_id, resolve_room_display_name
from app.game_engine.agent_runtime.world_runtime_queries import installed_worlds_from_session
PRIMER_SECTIONS: Tuple[Tuple[str, str], ...] = (('identity', '1. Identity'), ('structure', '2. Structure'), ('ontology', '3. Ontology'), ('world', '4. World'), ('actions', '5. Actions'), ('interaction', '6. Interaction'), ('memory', '7. Memory'), ('invariants', '8. Invariants'), ('commands', '9. Commands'))
_SECTION_KEYS = {key for (key, _) in PRIMER_SECTIONS}
_SECTION_ALIASES = {'examples': 'commands'}
...
def _neutral_primer_values(*, for_agent: Optional[str], caller_location_name: Optional[str], root_room_label: Optional[str]) -> Dict[str, str]:
    return {'AGENT_SERVICE_ID': for_agent or 'aico', 'CALLER_LOCATION': caller_location_name or 'Singularity Room', 'ROOT_ROOM_LABEL': root_room_label or 'Singularity Room'}
...
def _resolve_runtime_placeholders(*, for_agent: Optional[str], caller_location_name: Optional[str], primary_world_root: Optional[str], root_room_label: Optional[str]) -> Dict[str, str]:
    root = root_room_label if root_room_label is not None else primary_world_root
    return _neutral_primer_values(for_agent=for_agent, caller_location_name=caller_location_name, root_room_label=root)
...
def merge_system_prompt_with_primer_tier1(base_system_prompt: str, *, for_agent: Optional[str]=None, caller_location_name: Optional[str]=None, primary_world_root: Optional[str]=None, root_room_label: Optional[str]=None, enabled: bool=True) -> str:
    ...
    snippet = identity_and_invariants_snippet(for_agent=for_agent, caller_location_name=caller_location_name, primary_world_root=primary_world_root, root_room_label=root_room_label).strip()
    if not snippet:
        return base_system_prompt if base_system_prompt is not None else ''
    pointer = 'Authoritative per-turn command JSON shapes live under "Tools available" in the Plan user message; for narrative-only detail run `primer commands`.'
    if not base:
        return snippet + '\n\n' + pointer + '\n'
    return snippet + '\n\n' + base + '\n\n' + pointer + '\n'

def build_ontology_primer(section: Optional[str]=None, *, for_agent: Optional[str]=None, raw: bool=False, caller_location_name: Optional[str]=None, primary_world_root: Optional[str]=None, root_room_label: Optional[str]=None, primer_path: Optional[Path]=None, session=None, primer_command_context=None, append_graph_annex: bool=True) -> str:
    ...
    if raw:
        ...
        if section is None or section == '':
            ...
            if key not in _SECTION_KEYS:
                raise ValueError(f"unknown primer section '{section}'; known: {sorted(_SECTION_KEYS)}")
            assert _primer_sections_cache is not None
            chosen = _primer_sections_cache.get(key, '')
    if raw:
        return chosen.rstrip() + '\n'
    neutral = _resolve_runtime_placeholders(for_agent=for_agent, caller_location_name=eff_caller_location, primary_world_root=primary_world_root, root_room_label=root_room_label)
...
def identity_and_invariants_snippet(*, for_agent: Optional[str]=None, caller_location_name: Optional[str]=None, primary_world_root: Optional[str]=None, root_room_label: Optional[str]=None) -> str:
    parts: List[str] = []
    for key in ('identity', 'invariants'):
        chunk = build_ontology_primer(section=key, for_agent=for_agent, caller_location_name=caller_location_name, primary_world_root=primary_world_root, root_room_label=root_room_label, append_graph_annex=False).rstrip()
        if chunk:
