...
def mandatory_observation_gap(
    mandatory_tool_names: List[str],
    plan_tool_results: List[ToolResult],
    *,
    plan_trace: Optional[Sequence[Dict[str, Any]]] = None,
    ...
    mandatory_keys = {
        normalize_command_name(m) for m in mandatory_tool_names if normalize_command_name(m)
    }
