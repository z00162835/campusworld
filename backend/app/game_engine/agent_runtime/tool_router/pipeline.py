...
from app.game_engine.agent_runtime.model_backends.factories import build_chat_generative_backend, build_cross_encoder_rerank_backend, build_query_embedding_backend
...
from app.game_engine.agent_runtime.tool_router.router_thresholds import router_confidence_heuristic, router_should_clarify
...
def run_tool_router(*, cfg: ToolRouterConfig, user_message: str, world_snapshot: str, stm_snippet: Optional[str], intent_hint: Optional[Dict[str, Any]], tool_schemas: Sequence[ToolSchema], agent_extra: Optional[Dict[str, Any]]=None) -> Optional[RouterResult]:
    if not cfg.enabled:
        return None
    if not tool_schemas:
        return None
    ...
    ner_raw = extract_entity_spans_gliner(user_message, model_id=cfg.gliner_model_id, labels=ner_labels)
    ...
    enrich = build_enrich_query_text(user_message=user_message, world_snapshot=world_snapshot, stm_snippet=stm_snippet, rule_hints=rule_hints, entity_spans=entity_spans, lexicon_hits=lex_hits)
    ...
    allowed_tool_names = frozenset((str(getattr(s, 'name', '') or '').strip() for s in tool_schemas if getattr(s, 'name', None)))
    ...
    return RouterResult(candidates=candidates, mandatory_tool_names=list(mandatory), suggested_tool_names=[], router_confidence=confidence, source=source, clarify=clarify_flag, lexicon_active_id=lex_id, threshold_revision=cfg.threshold_revision, tool_registry_revision=registry_revision, latency_ms=latency_ms, enrich_query_text=enrich, enforcement_level=cfg.enforcement_level)

def format_tool_router_hint(rr: RouterResult) -> str:
    lines = ['Tool router (pre-Plan):', f'  source: {rr.source}', f'  confidence: {rr.router_confidence:.4f}', f'  clarify: {rr.clarify} (True=suggest user/tool disambiguation)', f"  lexicon_active_id: {rr.lexicon_active_id or '(none)'}", f"  threshold_revision: {rr.threshold_revision or '(none)'}", f"  tool_registry_revision: {rr.tool_registry_revision or '(none)'}", f'  mandatory_tool_names: {rr.mandatory_tool_names}', '  top_candidates:']
