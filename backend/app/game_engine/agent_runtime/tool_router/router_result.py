...
class RouterResult:
    ...
    # Fingerprint of this tick's tool schemas (name+description); see tool_surface_revision module.
    ...
    def to_payload_dict(self) -> Dict[str, Any]:
        return {
            "candidates": [
                {"tool_name": c.tool_name, "score": c.score, "tier": c.tier.value} for c in self.candidates
            ],
            "mandatory_tool_names": list(self.mandatory_tool_names),
            "suggested_tool_names": list(self.suggested_tool_names),
            "router_confidence": self.router_confidence,
            "source": self.source,
            "clarify": self.clarify,
            "lexicon_active_id": self.lexicon_active_id,
    ...
    def to_trace_dict(self) -> Dict[str, Any]:
        return {
            "tool_router_source": self.source,
            "tool_router_confidence": self.router_confidence,
            "tool_router_clarify": self.clarify,
            "tool_router_lexicon_active_id": self.lexicon_active_id,
            "tool_router_threshold_revision": self.threshold_revision,
            "tool_router_registry_revision": self.tool_registry_revision,
            "tool_router_latency_ms": round(self.latency_ms, 3),
            "tool_router_enforcement": self.enforcement_level.value,
            "tool_router_mandatory": list(self.mandatory_tool_names),
            "tool_router_top_candidates": [c.tool_name for c in self.candidates[:8]],
        }
