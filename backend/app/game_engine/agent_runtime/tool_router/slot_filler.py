...
def schema_path() -> Path:
    return backend_root() / 'app' / 'models' / 'agent_model' / 'tool_router' / 'schemas' / 'tool_router_slot_output.schema.json'

def repair_prompt_path_v1() -> Path:
    return backend_root() / 'app' / 'models' / 'agent_model' / 'tool_router' / 'prompts' / 'slot_repair_v1.en.txt'
...
def repair_prompt_text_v1() -> str:
    ...
    try:
        ...
        return 'Return one JSON object only, fixing syntax: target_hint, named_spans, entities, mandatory_tools. No prose outside the JSON.'
...
class SlotFiller:
    ...
    def extract(self, *, user_message: str, enrich_query: str, repair: bool=False) -> Tuple[Optional[Dict[str, Any]], List[str]]:
        mandatory: List[str] = []
        sys_prompt = 'Output a single JSON object only. Keys: target_hint (string|null); named_spans (array of {text,type}); entities (array of {normalized_text,entity_type}); mandatory_tools (array of command names, may be empty).'
        ...
        if (obj is None or not validate_slot_against_schema(obj)) and repair:
            repair_messages = messages + [{'role': 'assistant', 'content': res.text or ''}, {'role': 'user', 'content': repair_prompt_text_v1()}]
