...
def resolve_tool_runtime_view(*, pre_tool: Optional['PreauthorizedToolExecutor'], tool_command_context: Optional[CommandContext], budgets: ToolGatherBudgets, counters: ToolGatherCounters) -> ToolRuntimeView:
    ...
    if counters.commands_run >= budgets.max_commands_per_tick:
        return ToolRuntimeView(False, 'tick_command_budget_exhausted', pre_tool, tool_command_context, budgets)
    if counters.observation_chars >= budgets.max_chars_observations_per_tick:
        return ToolRuntimeView(False, 'tick_char_budget_exhausted', pre_tool, tool_command_context, budgets)
