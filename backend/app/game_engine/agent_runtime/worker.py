...
class SysSampleWorker(AgentWorker):
    ...
    @classmethod
    def create(cls, session, agent_node_id: int, invoker_context: Optional[CommandContext]=None) -> 'SysSampleWorker':
        ...
        fb = invoker_context or CommandContext(user_id=str(agent_node_id), username='agent_worker', session_id=f'agent_worker_{agent_node_id}', permissions=[], roles=[], db_session=session)
        ...
        fb = CommandContext(user_id=fb.user_id, username=fb.username, session_id=fb.session_id, permissions=list(fb.permissions or []), roles=list(fb.roles or []), db_session=fb.db_session, caller=fb.caller, game_state=fb.game_state, metadata=fb_meta)
        ...
        return cls(memory=mem, framework=fw, tools=tools, tool_command_context=tool_ctx)
...
class LlmPdcaAssistantWorker(AgentWorker):
    ...
    def create(cls, session, agent_node_id: int, invoker_context: Optional[CommandContext]=None, *, llm_client: Optional[LlmClient]=None, agent_llm_config: Optional['AgentLlmServiceConfig']=None, tick_inputs: Optional['NpcAgentTickInputs']=None, tick_hooks: Optional[AgentTickHooks]=None, runtime_observability: Optional[AgentRuntimeObservability]=None) -> 'LlmPdcaAssistantWorker':
        ...
        from app.game_engine.agent_runtime.resolved_tool_surface import PreauthorizedToolExecutor, build_resolved_tool_surface
        ...
        if tick_inputs is not None:
            ...
            if int(agent.id) != int(agent_node_id):
                raise ValueError(f'tick_inputs.agent.id {agent.id} does not match agent_node_id {agent_node_id}')
        ...
        if tier1_on:
            ...
            from app.game_engine.agent_runtime.system_primer_context import merge_system_prompt_with_primer_tier1
            cfg = cfg.model_copy(update={'system_prompt': merge_system_prompt_with_primer_tier1(cfg.system_prompt or '', for_agent=service_id, caller_location_name=caller_location_name, enabled=True)})
