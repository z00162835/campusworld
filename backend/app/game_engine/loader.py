...
from .runtime_store import OperationResult, WorldErrorCode, WorldInstallerService, WorldRuntimeRepository, WorldRuntimeStatus
...
class GameLoader:
    ...
    def __init__(self, engine):
        ...
        self.search_paths = [self.backend_root / 'app' / 'games', self.project_root / 'games', self.backend_root / 'games']
    ...
    def _result(self, ok: bool, world_id: str, status_before: str, status_after: str, message: str, error_code: Optional[WorldErrorCode]=None, details: Optional[Dict[str, Any]]=None) -> OperationResult:
        return OperationResult(ok=ok, world_id=world_id, status_before=status_before, status_after=status_after, message=message, error_code=error_code.value if error_code else None, details=details or {})
    ...
    def _sync_world_entry_visibility_after_load(self, world_id: str) -> None:
        ...
        try:
            ...
            if not ok:
                self.logger.warning('sync_world_entry_visibility after load failed world_id=%s code=%s', world_id, err)
        except Exception as e:
            self.logger.warning('sync_world_entry_visibility after load raised world_id=%s: %s', world_id, e)
    ...
    def discover_games(self) -> List[str]:
        ...
        for search_path in self.search_paths:
            ...
            try:
                ...
                self.logger.error(f'Search path {search_path} error while: {e}')
    ...
    def _load_manifest(self, game_dir: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f'Failed to read manifest: {manifest_path} err={e}')
            return None

    def _load_world_package_snapshot(self, game_name: str, game_path: Path, manifest: Dict[str, Any], module: Any) -> Dict[str, Any]:
        """
        ...
        if not callable(snapshot_loader):
            return {'data_dir': str(data_root), 'snapshot_loaded': False, 'world_data_validated': True}
        snapshot = snapshot_loader(str(data_root))
        return {'data_dir': str(data_root), 'snapshot_loaded': True, 'world_data_validated': True, 'snapshot': snapshot, 'snapshot_meta': getattr(snapshot, 'meta', {}), 'snapshot_counts': {'spatial': {'buildings': len(getattr(snapshot, 'spatial', {}).get('buildings', [])), 'floors': len(getattr(snapshot, 'spatial', {}).get('floors', [])), 'rooms': len(getattr(snapshot, 'spatial', {}).get('rooms', []))}, 'entities': {'npcs': len(getattr(snapshot, 'entities', {}).get('npcs', [])), 'items': len(getattr(snapshot, 'entities', {}).get('items', [])), 'zones': len(getattr(snapshot, 'entities', {}).get('zones', []))}, 'concepts': {'goals': len(getattr(snapshot, 'concepts', {}).get('goals', [])), 'processes': len(getattr(snapshot, 'concepts', {}).get('processes', [])), 'rules': len(getattr(snapshot, 'concepts', {}).get('rules', [])), 'behaviors': len(getattr(snapshot, 'concepts', {}).get('behaviors', [])), 'skills': len(getattr(snapshot, 'concepts', {}).get('skills', []))}, 'relationships': len(getattr(snapshot, 'relationships', []))}}
    ...
    def _try_run_graph_seed(self, game_name: str, manifest: Dict[str, Any], module: Any, package_info: Dict[str, Any]) -> Tuple[Optional[Dict[str, Any]], Optional[WorldErrorCode], str]:
        ...
        if not package_info.get('snapshot_loaded') or package_info.get('snapshot') is None:
            return (None, WorldErrorCode.GRAPH_SEED_FAILED, 'graph_seed enabled but snapshot not loaded (requires load_package_snapshot)')
        ...
        if engine is None or 'postgresql' not in str(engine.url).lower():
            return (None, WorldErrorCode.GRAPH_SEED_FAILED, 'graph seed requires a configured PostgreSQL database URL')
        profile_fn = getattr(module, 'get_graph_profile', None)
        if not callable(profile_fn):
            return (None, WorldErrorCode.GRAPH_SEED_FAILED, 'world package must export get_graph_profile() when graph_seed is enabled')
        try:
        ...
        try:
            ...
            with db_session_context() as session:
                out = run_graph_seed(session, game_name, snapshot, profile, strict_relationships=strict)
                session.commit()
    ...
    def _load_game_module(self, game_name: str, game_path: Path) -> Optional[Any]:
        try:
            ...
            self.logger.error(f"Load world module '{game_name}' failed: {e}")
            return None
    ...
    def _create_game_instance(self, game_name: str, game_module: Any) -> Optional[BaseGame]:
        try:
            ...
            if not callable(fn):
                self.logger.error(f"World module '{game_name}' missing callable get_game_instance")
                return None
            instance = fn()
            return instance
        except Exception as e:
    ...
    def load_game(self, game_name: str) -> Dict[str, Any]:
        ...
        if not lock.acquire(blocking=False):
            return self._result(False, game_name, state_before, state_before, 'world operation in progress', WorldErrorCode.WORLD_BUSY).to_dict()
        try:
            if game_name in self.loaded_games:
                return self._result(False, game_name, state_before, state_before, 'world already loaded in runtime', WorldErrorCode.WORLD_STATE_CONFLICT).to_dict()

            def _run(job_id: str) -> OperationResult:
                game_path = self._find_game_path(game_name)
                if not game_path:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, f"world package '{game_name}' not found", WorldErrorCode.WORLD_NOT_FOUND)
                manifest = self._load_manifest(game_path)
                if not manifest:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'manifest.yaml missing or invalid', WorldErrorCode.WORLD_MANIFEST_INVALID)
                required = ['world_id', 'version', 'api_version', 'data_dir']
                if not all((manifest.get(k) for k in required)):
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'manifest required fields missing', WorldErrorCode.WORLD_MANIFEST_INVALID, details={'manifest_required': required})
                if str(manifest['world_id']) != game_name:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'manifest world_id mismatch', WorldErrorCode.WORLD_MANIFEST_INVALID)
                module = self._load_game_module(game_name, game_path)
                if not module:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'failed to import world module', WorldErrorCode.WORLD_LOAD_FAILED)
                try:
                    package_info = self._load_world_package_snapshot(game_name, game_path, manifest, module)
                except WorldDataPackageError as e:
                    try:
                        error_code = WorldErrorCode(e.error_code)
                    except Exception:
                        error_code = WorldErrorCode.WORLD_DATA_INVALID
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, f'world data package validation failed: {e.message}', error_code)
                except Exception as e:
                    ...
                    try:
                        ...
                        error_code = WorldErrorCode.WORLD_DATA_INVALID
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, f'world data package validation failed: {e}', error_code)
                (seed_details, seed_err, seed_msg) = self._try_run_graph_seed(game_name, manifest, module, package_info)
                if seed_err:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, f'graph seed failed: {seed_msg}', seed_err, details={'version': str(manifest.get('version')), 'manifest': manifest, 'package': _strip_snapshot_from_package_info(package_info), 'graph_seed_error': seed_msg, 'job_id': job_id})
                ...
                if not game_instance:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'failed to create world instance', WorldErrorCode.WORLD_LOAD_FAILED)
                initialize = getattr(game_instance, 'initialize_game', None)
                if callable(initialize) and (not initialize()):
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'initialize_game returned false', WorldErrorCode.WORLD_LOAD_FAILED)
                ...
                return self._result(True, game_name, state_before, WorldRuntimeStatus.INSTALLED.value, 'world loaded', details={'version': str(manifest.get('version')), 'manifest': manifest, 'package': _strip_snapshot_from_package_info(package_info), 'job_id': job_id})
            try:
                result = self.service.run_with_job(world_id=game_name, action='load', status_before=state_before, enter_status=WorldRuntimeStatus.LOADING.value, exec_fn=_run)
                return result.to_dict()
            except Exception as e:
                return self._result(False, game_name, state_before, WorldRuntimeStatus.BROKEN.value, f'persist runtime state failed: {e}', WorldErrorCode.WORLD_DB_WRITE_FAILED).to_dict()
        finally:
            lock.release()
    ...
    def unload_game(self, game_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                result = self.service.run_with_job(world_id=game_name, action='unload', status_before=state_before, enter_status=WorldRuntimeStatus.UNLOADING.value, exec_fn=_run)
                return result.to_dict()
            except Exception as e:
                return self._result(False, game_name, state_before, WorldRuntimeStatus.BROKEN.value, f'persist runtime state failed: {e}', WorldErrorCode.WORLD_DB_WRITE_FAILED).to_dict()
        finally:
        ...
        try:
            ...
            def _run(job_id: str) -> OperationResult:
                ...
                if not manifest or not all((manifest.get(k) for k in required)):
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'manifest required fields missing', WorldErrorCode.WORLD_MANIFEST_INVALID)
                module = self._load_game_module(game_name, game_path)
                if not module:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'reload failed while importing world module', WorldErrorCode.WORLD_RELOAD_FAILED)
                try:
                    package_info = self._load_world_package_snapshot(game_name, game_path, manifest, module)
                except WorldDataPackageError as e:
                    try:
                ...
                (seed_details, seed_err, seed_msg) = self._try_run_graph_seed(game_name, manifest, module, package_info)
                if seed_err:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, f'graph seed failed: {seed_msg}', seed_err, details={'job_id': job_id, 'version': str(manifest.get('version')), 'package': _strip_snapshot_from_package_info(package_info), 'graph_seed_error': seed_msg})
                if seed_details is not None:
                    package_info = {**package_info, 'graph_seed': seed_details}
                instance = self._create_game_instance(game_name, module)
                if not instance:
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'reload failed while creating world instance', WorldErrorCode.WORLD_RELOAD_FAILED)
                initialize = getattr(instance, 'initialize_game', None)
                if callable(initialize) and (not initialize()):
                    return self._result(False, game_name, state_before, WorldRuntimeStatus.FAILED.value, 'reload failed while initializing world instance', WorldErrorCode.WORLD_RELOAD_FAILED)
                ...
                self.game_modules[game_name] = module
                self._sync_world_entry_visibility_after_load(game_name)
                return self._result(True, game_name, state_before, WorldRuntimeStatus.INSTALLED.value, 'world reloaded', details={'job_id': job_id, 'version': str(manifest.get('version')), 'package': _strip_snapshot_from_package_info(package_info)})
            try:
                result = self.service.run_with_job(world_id=game_name, action='reload', status_before=state_before, enter_status=WorldRuntimeStatus.RELOADING.value, exec_fn=_run)
                return result.to_dict()
            except Exception as e:
                return self._result(False, game_name, state_before, WorldRuntimeStatus.BROKEN.value, f'persist runtime state failed: {e}', WorldErrorCode.WORLD_DB_WRITE_FAILED).to_dict()
        finally:
    ...
    def get_game_info(self, game_name: str) -> Optional[Dict[str, Any]]:
        ...
        if not game_instance:
            ...
            return {'name': game_name, 'version': state.get('version'), 'description': '', 'author': '', 'status': state.get('status', WorldRuntimeStatus.NOT_INSTALLED.value)}
        state = self.repository.get_state(game_name)
        return {'name': game_instance.name, 'version': game_instance.version, 'description': getattr(game_instance, 'description', ''), 'author': getattr(game_instance, 'author', ''), 'status': state.get('status', WorldRuntimeStatus.INSTALLED.value)}
    ...
    def load_installed_worlds_at_start(self) -> List[str]:
        ...
        for world_id in targets:
            ...
            if result.get('ok'):
                ...
                self.logger.error("startup load installed world '%s' failed: %s", world_id, result)
        self.logger.info('load_installed_worlds_at_start attempted=%s loaded=%s', targets, loaded)
        return loaded
    ...
    def auto_load_games(self, only_world_ids: Optional[List[str]]=None) -> List[str]:
        ...
        for game_name in targets:
            ...
            if result.get('ok'):
                ...
                self.logger.error(f"Auto-load world '{game_name}' failed: {result}")
        self.logger.info(f'Auto-load complete, successfully loaded {len(loaded)} worlds: {loaded}')
        return loaded
