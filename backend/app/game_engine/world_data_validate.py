...
_MINIMAL_SPATIAL_FILES = ['world.yaml', 'buildings.yaml', 'floors.yaml', 'rooms.yaml', 'relationships.yaml']
...
def _minimal_spatial_validate(data_root: Path) -> None:
    ...
    if missing:
        raise WorldDataPackageError('WORLD_DATA_UNAVAILABLE', f'required world data files missing: {missing}')
