...
class WorldEntryService:
    ...
    def resolve_portal(self, world_id: str, caller_context: Optional[Dict[str, Any]]=None) -> WorldEntryDecision:
        ...
        if not wid:
            return WorldEntryDecision(ok=False, world_id=wid, spawn_key='', error_code='WORLD_ENTRY_INVALID_TARGET', message='世界名不能为空')
        try:
            with db_session_context() as session:
                ...
                if not node:
                    return WorldEntryDecision(ok=False, world_id=wid, spawn_key='', error_code='WORLD_ENTRY_PORTAL_MISSING', message=f'奇点屋中未找到世界入口: {wid}')
                attrs = dict(node.attributes or {})
                if not bool(attrs.get('portal_enabled', True)):
                    return WorldEntryDecision(ok=False, world_id=wid, spawn_key='', error_code='WORLD_ENTRY_PORTAL_DISABLED', message=f'世界入口当前不可用: {wid}')
                spawn_key = str(attrs.get('portal_spawn_key') or attrs.get('entry_room_id') or 'campus')
                return WorldEntryDecision(ok=True, world_id=wid, spawn_key=spawn_key, metadata={'attributes': attrs})
        except Exception as e:
            self.logger.error('resolve world portal failed: %s', e)
            return WorldEntryDecision(ok=False, world_id=wid, spawn_key='', error_code='WORLD_ENTRY_RESOLVE_FAILED', message=f'世界入口解析失败: {wid}')
    ...
    def authorize_entry(self, portal: WorldEntryDecision, user: Optional[Any]=None) -> WorldEntryDecision:
        ...
        try:
            allowed = evaluate_policy_expr(interact_expr, user_permissions=list(getattr(user, 'permissions', []) or []), user_roles=list(getattr(user, 'roles', []) or []), object_attrs=attrs)
        except PolicyExprError:
            allowed = False
        if not allowed:
            return WorldEntryDecision(ok=False, world_id=portal.world_id, spawn_key=portal.spawn_key, error_code='WORLD_ENTRY_FORBIDDEN', message=f'当前用户无权限进入世界: {portal.world_id}', metadata=portal.metadata)
        return portal
    ...
    def _migrate_legacy_root_portal_to_world_entrance(self, session: Session, world_id: str) -> None:
        ...
        if not wnt:
            return
        wid = str(world_id or '').strip().lower()
        candidates = session.query(Node).filter(and_(Node.type_code == 'world', Node.location_id == root.id, Node.attributes['world_id'].astext == wid)).all()
        for node in candidates:
            attrs = dict(node.attributes or {})
            portalish = bool(attrs.get('portal_spawn_key') or attrs.get('entry_hint') or attrs.get('portal_world_id') == wid or (str(node.name or '').strip().lower() == wid))
            if not portalish:
                continue
            node.type_code = 'world_entrance'
            node.type_id = wnt.id
    ...
    def _find_world_entry_node(self, session: Session, world_id: str) -> Optional[Node]:
        wid = str(world_id or '').strip().lower()
        return session.query(Node).filter(and_(Node.type_code == 'world_entrance', or_(Node.attributes['portal_world_id'].astext == wid, Node.attributes['world_id'].astext == wid))).first()
    ...
    def _create_world_entry_node(self, session: Session, world_id: str, root_node_id: int) -> Optional[Node]:
        ...
        node = Node(type_id=nt.id, type_code='world_entrance', name=wid, description=f"{wid} 世界入口。输入 'enter {wid}' 进入。", is_active=True, is_public=True, access_level='normal', location_id=root_node_id, home_id=root_node_id, attributes={'world_id': wid, 'portal_world_id': wid, 'portal_spawn_key': spawn_key, 'portal_enabled': True, 'entity_kind': 'exit', 'presentation_domains': ['room'], 'access_locks': {'view': 'all()', 'interact': 'all()'}, 'entry_hint': f'enter {wid}', **({'destination_node_id': dest_id} if dest_id is not None else {})}, tags=['world_entrance', wid])
        session.add(node)
        session.flush()
        return node
