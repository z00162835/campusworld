...
def find_world_room_node(session: Session, world_id: str, package_node_id: str) -> Optional[Node]:
    ...
    if not wid or not pid:
        return None
    return session.query(Node).filter(Node.type_code == 'room', Node.attributes['world_id'].astext == wid, Node.attributes['package_node_id'].astext == pid, Node.is_active == True).first()
...
def get_world_entry_package_node_id(session: Session, world_id: str) -> Optional[str]:
    ...
    if not wid:
        return None
    meta = session.query(Node).filter(Node.type_code == 'world', Node.attributes['world_id'].astext == wid, Node.is_active == True).first()
    if not meta:
        return None
