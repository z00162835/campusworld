...
class CampusLifeGameListCommand(GameCommand):
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        """执行场景列表命令"""
        games = [{'name': 'campus_life', 'description': '园区生活模拟场景', 'version': '1.0.0', 'status': 'available', 'author': 'CampusWorld OS Team'}]
        message = '可用场景:\n'
        message += '=' * 50 + '\n'
        for game in games:
...
class CampusLifeGameLoadCommand(GameCommand):
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if game_name == 'campus_life':
            game_info = {'name': game_name, 'status': 'loaded', 'load_time': '2024-01-01 12:00:00'}
            if not context.game_state:
                context.game_state = {}
            context.game_state.update({'current_game': game_name, 'is_running': True, 'game_info': game_info})
...
class CampusLifeGameUnloadCommand(GameCommand):
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if context.game_state:
            context.game_state.update({'current_game': None, 'is_running': False, 'game_info': {}})
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if self.is_game_running(context):
            ...
            if current_game:
                pass
        if game_name == 'campus_life':
            game_info = {'name': game_name, 'status': 'running', 'switch_time': '2024-01-01 12:00:00'}
            if not context.game_state:
                context.game_state = {}
            context.game_state.update({'current_game': game_name, 'is_running': True, 'game_info': game_info})
    ...
    def execute(self, context: CommandContext, args: List[str]) -> CommandResult:
        ...
        if game_info:
            for (key, value) in game_info.items():
                message += f'{key}: {value}\n'
        return CommandResult.success_result(message, {'current_game': current_game, 'is_running': self.is_game_running(context), 'game_info': game_info}, CommandType.GAME)

    def _get_specific_help(self) -> str:
        return '\n园区世界状态命令:\n  game_status  - 显示当前场景状态\n\n显示信息:\n  - 当前运行的场景\n  - 场景运行状态\n  - 场景详细信息\n'
CAMPUS_LIFE_GAME_COMMANDS = [CampusLifeGameListCommand(), CampusLifeGameLoadCommand(), CampusLifeGameUnloadCommand(), CampusLifeGameSwitchCommand(), CampusLifeGameStatusCommand()]
