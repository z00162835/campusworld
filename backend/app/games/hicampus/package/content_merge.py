...
ROOM_P0_STR = ('room_name', 'room_name_en', 'room_description', 'room_short_description', 'room_ambiance', 'room_type', 'room_code', 'uns')
BUILDING_P0_STR = ('building_type', 'building_status', 'building_class', 'uns', 'building_name', 'building_description', 'building_tagline')
FLOOR_P0_STR = ('display_name', 'floor_name', 'floor_code', 'uns', 'floor_description', 'floor_short_description')
...
def _merge_by_id(rows: List[Dict[str, Any]], extras: List[Dict[str, Any]], *, kind: str) -> None:
    ...
    for row in extras:
        ...
        rid = str(row['id'])
        if rid not in by_id:
            raise DataPackageError(ERROR_WORLD_DATA_REFERENCE_BROKEN, f'{kind} sidecar id not in spatial package: {rid}')
        target = by_id[rid]
    ...
    for f in spatial.get('floors') or []:
        ...
        if f.get('floor_number') is None and f.get('floor_no') is not None:
            try:
                ...
                raise DataPackageError(ERROR_WORLD_DATA_INVALID, f"floor_no must be int-compatible: {f.get('id')}") from exc
...
def validate_spatial_p0(spatial: Dict[str, Any], *, required_room_ids: Set[str]) -> None:
    ...
    for rid in sorted(required_room_ids):
        ...
        if missing:
            raise DataPackageError(ERROR_WORLD_DATA_INVALID, f'room {rid} missing required description keys: {missing}')
    for b in buildings:
        if not isinstance(b, dict) or not b.get('id'):
            continue
        missing = [k for k in BUILDING_P0_STR if not _nonempty_str(b, k)]
        if missing:
            raise DataPackageError(ERROR_WORLD_DATA_INVALID, f"building {b.get('id')} missing required description keys: {missing}")
    for f in floors:
        ...
        if missing:
            raise DataPackageError(ERROR_WORLD_DATA_INVALID, f"floor {f.get('id')} missing required description keys: {missing}")

def collect_spatial_completeness_gaps(spatial: Dict[str, Any], *, required_room_ids: Set[str]) -> Tuple[List[str], List[str], List[str]]:
    """
