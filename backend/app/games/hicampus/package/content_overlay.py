...
def _find_node_by_pkg(session: Session, world_id: str, package_node_id: str) -> Optional[Node]:
    ...
    nodes = session.query(Node).filter(Node.attributes['package_node_id'].astext == str(package_node_id), Node.is_active == True).all()
    for n in nodes:
...
def diff_spatial_vs_db(session: Session, world_id: str, data_root: Path) -> Dict[str, Any]:
    ...
    def _scan(rows: List[Dict[str, Any]], kind: str) -> None:
        for row in rows:
            ...
            if field_diffs:
                diffs.append({'kind': kind, 'package_node_id': pkg_id, 'node_id': int(node.id), 'fields': field_diffs})
...
def apply_spatial_content_overlay(session: Session, world_id: str, data_root: Path, *, dry_run: bool=False, write_revision_snapshot: bool=True) -> Dict[str, Any]:
    """
    ...
    if not dry_run and applied:
        ...
        session.rollback()
    return {'ok': True, 'dry_run': dry_run, 'world_id': wid, 'applied_node_ids': applied, 'missing_in_graph': missing, 'revision_snapshot_path': snapshot_path}
...
def content_validate_report(data_root: Path, *, required_room_ids: Optional[Set[str]]=None) -> Dict[str, Any]:
    ...
    return {'ok': len(gb) + len(gf) + len(gr) == 0, 'building_gaps': gb, 'floor_gaps': gf, 'room_gaps': gr}
