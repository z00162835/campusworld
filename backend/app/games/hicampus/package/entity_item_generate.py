...
_PRESERVED_ITEM_IDS = frozenset({'hicampus_device_gate_terminal_01', 'hicampus_furniture_bench_01'})
...
def _placement_for_room(room: Dict[str, Any], floor_by_id: Dict[str, Dict[str, Any]], building_by_id: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not bcode:
        bcode = next((t.upper() for t in _room_tags(room) if len(t) == 2 and t[0] == 'f' and t[1].isdigit()), 'UNK')
    ...
    return {'floor_id': fid or None, 'floor_no': floor_no_i, 'building_id': bid or None, 'building_code': bcode, 'building_type': btype, 'layer_role': layer_role, 'layer': layer_lane, 'floor_uns': str(floor.get('uns') or '').strip() or None}
...
def _build_item_row(*, template_key: str, template: Dict[str, Any], room: Dict[str, Any], seq: int, placement: Dict[str, Any]) -> Dict[str, Any]:
    rid = str(room['id'])
    ...
    layer = str(placement.get('layer') or '') or next((t.split(':', 1)[1] for t in tags if t.startswith('layer:')), '')
    ctx = {'room_name': rname, 'room_id': rid, 'building_code': bcode, 'floor_no': floor_no, 'layer': layer or 'standard', 'layer_role': layer_role}
    ...
    row: Dict[str, Any] = {'id': iid, 'world_id': 'hicampus', 'type_code': template['type_code'], 'entity_kind': 'item', 'display_name': display_name, 'location_ref': rid, 'attributes': attrs, 'presentation_domains': list(template.get('presentation_domains') or ['room']), 'access_locks': copy.deepcopy(template.get('access_locks') or {'view': 'all()', 'interact': 'all()'}), 'tags': [str(t) for t in template.get('tags') or []], 'source_ref': 'entities/items.yaml'}
    return row

def generate_items(*, data_root: Optional[Path]=None, package_dir: Path=_PACKAGE_DIR) -> List[Dict[str, Any]]:
        ...
        for dr in device_rules:
            ...
            for spec in dr.get('add_templates') or []:
                ...
                for _ in range(n):
                    ...
                    generated.append(_build_item_row(template_key=tid, template=tdef, room=room, seq=per_key_seq[tid], placement=placement))
