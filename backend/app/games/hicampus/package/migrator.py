...
def _filter_migrations_by_range(migrations: List[Dict[str, Any]], from_version: str, to_version: str) -> List[Dict[str, Any]]:
...
def build_migration_plan(data_root: Path, from_version: Optional[str]=None, to_version: Optional[str]=None) -> List[Dict[str, Any]]:
    """
    ...
    for path in list_migration_files(migrations_dir):
        ...
        result.append({'name': path.name, 'from_version': m.get('from_version'), 'to_version': m.get('to_version'), 'operations': m.get('operations', []), 'rollback': m.get('rollback', [])})
...
def migration_dry_run(data_root: Path, from_version: str, to_version: str, *, post_validate: bool=True) -> Dict[str, Any]:
    """
    ...
    report: Dict[str, Any] = {'from_version': from_version, 'to_version': to_version, 'selected_migrations': [m['name'] for m in plan], 'operation_preview': preview}
    if post_validate:
        try:
