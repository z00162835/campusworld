...
_ARCH_ABBR = {'office': 'OFF', 'meeting': 'MTG', 'manager': 'MGR', 'restroom': 'WCR', 'electrical': 'ELE', 'monitoring': 'SEC', 'circulation': 'COR', 'pantry': 'PAN', 'canteen_dining': 'DIN', 'kitchen': 'KIT', 'storage': 'STO', 'classroom': 'CLS', 'lab': 'LAB', 'prep_lab': 'PRE', 'expo_hall': 'EXP', 'av_room': 'AVC', 'dorm_unit': 'DRM', 'lounge': 'LNG'}
...
def _filter_by_tags(rooms: List[Dict[str, Any]], include: Optional[Set[str]], exclude: Optional[Set[str]], match_all: bool) -> List[Dict[str, Any]]:
        ...
        if include:
            if match_all:
                if not include.issubset(tags):
                    continue
            elif not tags & include:
                continue
        out.append(r)
    return out

def generate_spatial(*, data_root: Optional[Path]=None, package_dir: Path=_PACKAGE_DIR, seed: str='hicampus-spatial-v1', include_tags: Optional[Set[str]]=None, exclude_tags: Optional[Set[str]]=None, match_all_include: bool=False) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for (bcode, n_floors) in sorted(floor_expect.items()):
        ...
        for fn in range(1, n_floors + 1):
            ...
            fd_long = f'{bname}的{flabel}（{layer_role}）。动线与消防分区符合园区标准；本层由空间生成器按 profile 生成配套房间。'
            floors_out.append({'id': fid, 'world_id': 'hicampus', 'building_id': bid, 'floor_no': nn, 'floor_number': nn, 'type_code': 'building_floor', 'display_name': f'{bcode} · {flabel}', 'floor_name': flabel, 'floor_code': floor_code, 'uns': uns_f, 'floor_description': fd_long, 'floor_short_description': fd_short, 'tags': merge_tags(btags, floor_tags, [f'layer_role:{layer_role}'])})
            ...
            for arche in sequence:
                ...
                ctx = {'building': bname, 'floor_label': flabel, 'seq': seqn, 'code': room_code, 'bcode': bcode}
                ...
                rooms_out.append({'id': rid, 'world_id': 'hicampus', 'floor_id': fid, 'type_code': 'room', 'display_name': rname, 'room_name': rname, 'room_name_en': rname_en, 'room_type': meta['room_type'], 'room_code': room_code, 'uns': f'hicampus/{bcode}/L{nn:02d}/{room_code}', 'room_short_description': str(meta['short_tpl']).format(**ctx), 'room_description': str(meta['desc_tpl']).format(**ctx), 'room_ambiance': str(meta['ambiance_tpl']).format(**ctx), 'room_floor': nn, 'room_building': bname, 'room_campus': 'hicampus', 'tags': rtags})
...
def main(argv: Optional[List[str]]=None) -> int:
    ...
    p.add_argument('--allow-partial-rooms', action='store_true', help='Allow --include-tags/--exclude-tags to shrink room list when using --write (default: filters apply only with --dry-run)')
    ...
    if args.write and use_filters and (not args.allow_partial_rooms):
        (inc, exc) = (set(), set())
    (floors, rooms) = generate_spatial(data_root=args.data_root, package_dir=args.package_dir, seed=args.seed, include_tags=inc or None, exclude_tags=exc or None, match_all_include=bool(args.match_all))
