...
PRESERVE_CONNECT_IDS: Set[str] = {'rel_gate_bridge', 'rel_bridge_gate', 'rel_bridge_plaza', 'rel_plaza_bridge', 'rel_plaza_f1_circulation', 'rel_f1_circulation_plaza', 'rel_circulation_restroom_w', 'rel_restroom_circulation_e'}
...
def _connect_pair(src: str, tgt: str, d_fwd: str) -> List[Dict[str, Any]]:
    ...
    return [{'id': _rel_topo_id(src, tgt, d_fwd), 'rel_type_code': 'connects_to', 'source_id': src, 'target_id': tgt, 'directed': True, 'attributes': {'direction': d_fwd, 'topology_auto': True}}, {'id': _rel_topo_id(tgt, src, d_rev), 'rel_type_code': 'connects_to', 'source_id': tgt, 'target_id': src, 'directed': True, 'attributes': {'direction': d_rev, 'topology_auto': True}}]
...
def _make_circulation_room(floor_row: Dict[str, Any], building_row: Dict[str, Any], *, seq: int=1) -> Dict[str, Any]:
    ...
    return {'id': rid, 'world_id': 'hicampus', 'floor_id': fid, 'type_code': 'room', 'display_name': f'{bname} · {floor_name} 交通核 {seq}', 'room_name': f'{bname} · {floor_name} 交通核 {seq}', 'room_name_en': f'Circulation Core {room_code}', 'room_type': 'circulation', 'room_code': room_code, 'uns': uns, 'room_short_description': '电梯厅与楼梯间前室。', 'room_description': '连接垂直交通与走廊；导向标识指向防火分区与疏散楼梯。', 'room_ambiance': '电梯到达提示音与脚步声。', 'room_floor': floor_no, 'room_building': bname, 'room_campus': 'hicampus', 'tags': base_tags}
...
def _circulation_items(circ_room: Dict[str, Any], floor_row: Dict[str, Any], building_row: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    placement = {'floor_id': fid, 'floor_no': floor_no, 'building_id': str(floor_row.get('building_id') or ''), 'building_code': bcode, 'building_type': str(building_row.get('building_type') or 'administrative'), 'layer_role': layer_role, 'layer': layer_lane, 'floor_uns': uns_f}
    ...
    def item_row(suffix: str, type_code: str, display_suffix: str, attrs: Dict[str, Any], tags: List[str]) -> Dict[str, Any]:
        ...
        return {'id': iid, 'world_id': 'hicampus', 'type_code': type_code, 'entity_kind': 'item', 'display_name': f'{display_suffix}', 'location_ref': room_id, 'attributes': attrs, 'presentation_domains': ['room'], 'access_locks': {'view': 'all()', 'interact': 'all()'}, 'tags': tags, 'source_ref': 'entities/items.yaml'}
    lighting_attrs = {'item_kind': 'device', 'device_role': 'lighting', 'controllable': True, 'status': 'on', 'lighting': {'brightness_pct': 78, 'color_temp_k': 4000, 'scene': 'transit'}, 'telemetry': {'power_w': None}, 'room_list_name': '照明回路', 'device_id': f'{room_id}_lighting_fixture_01', 'asset_tag': f"HC-{_short_hash(room_id + 'lt')[:8].upper()}", 'placement': dict(placement)}
    lounge_attrs = {'item_kind': 'furniture', 'furniture_role': 'lounge', 'pieces': {'sofa_count': 2, 'chair_count': 4, 'table_count': 1}, 'room_list_name': '沙发与等候座', 'device_id': f'{room_id}_lounge_furniture_01', 'asset_tag': f"HC-{_short_hash(room_id + 'lg')[:8].upper()}", 'placement': dict(placement)}
    wifi_attrs = {'item_kind': 'device', 'device_role': 'wifi_ap', 'controllable': True, 'status': 'on', 'network': {'mode': 'ap', 'bands': ['2.4g', '5g'], 'ssid': f'HiCampus-{bcode}-L{floor_no:02d}', 'encryption': 'wpa2', 'ip_mode': 'dhcp', 'vlan_hint': 100 + int(_short_hash(room_id + 'vlan')[:4], 16) % 80}, 'telemetry': {'rssi_dbm': None, 'clients': 0, 'uplink_mbps': None}, 'room_list_name': '无线接入点', 'device_id': f'{room_id}_wifi_ap_01', 'asset_tag': f"HC-{_short_hash(room_id + 'wf')[:8].upper()}", 'placement': dict(placement)}
...
def write_relationships(data_root: Path, new_connects: List[Dict[str, Any]]) -> None:
    ...
    path.write_text(yaml.safe_dump({'relationships': merged}, allow_unicode=True, sort_keys=False, default_flow_style=False), encoding='utf-8')

def write_rooms(data_root: Path, rooms: List[Dict[str, Any]]) -> None:
    path = data_root / 'rooms.yaml'
    path.write_text(yaml.safe_dump({'rooms': rooms}, allow_unicode=True, sort_keys=False, default_flow_style=False), encoding='utf-8')

def write_items(data_root: Path, items: List[Dict[str, Any]]) -> None:
    path = data_root / 'entities' / 'items.yaml'
    path.write_text(yaml.safe_dump({'items': items}, allow_unicode=True, sort_keys=False, default_flow_style=False), encoding='utf-8')
