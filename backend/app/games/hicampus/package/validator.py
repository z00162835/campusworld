...
import yaml
from .contracts import DataPackageError, ERROR_WORLD_DATA_BASELINE_MISMATCH, ERROR_WORLD_DATA_INVALID, ERROR_WORLD_DATA_REFERENCE_BROKEN, ERROR_WORLD_DATA_SCHEMA_UNSUPPORTED, ERROR_WORLD_DATA_SEMANTIC_CONFLICT, ERROR_WORLD_DATA_UNAVAILABLE
...
ALLOWED_REL_TYPE_CODES = frozenset({'contains', 'connects_to', 'located_in', 'has_entry', 'served_by', 'adjacent_to', 'governs', 'applies_to', 'enables', 'requires', 'executes', 'located_in_zone'})
ALLOWED_CONCEPT_SCOPES = frozenset({'world', 'room', 'zone', 'npc', 'building', 'floor', 'global'})
...
def _load_l4_baseline() -> tuple[Dict[str, int], Set[str]]:
    ...
    default_floors: Dict[str, int] = {'F1': 23, 'F2': 3, 'F3': 6, 'F4': 7, 'F5': 3, 'F6': 9}
...
def _find_building_entry_hubs(*, buildings: List[Dict[str, Any]], floors: List[Dict[str, Any]], rooms: List[Dict[str, Any]]) -> Dict[str, str]:
    """
...
def _validate_world_entry_reachability(*, rooms: List[Dict[str, Any]], relationships: List[Dict[str, Any]], entry_room_id: str, required_entry_hubs: Dict[str, str]) -> None:
    ...
    missing: List[Tuple[str, str]] = [(bcode, hub) for (bcode, hub) in sorted(required_entry_hubs.items()) if hub not in reachable]
    if not missing:
        return
    missing_str = ', '.join([f'{bcode}:{hub}' for (bcode, hub) in missing])
    raise DataPackageError(ERROR_WORLD_DATA_SEMANTIC_CONFLICT, f"world entry reachability broken: from {entry_room_id} cannot reach building entry hubs ({missing_str}). Hint: run topology generator 'python -m app.games.hicampus.package.topology_connect_generate --write' to regenerate connects_to.")
...
def _validate_connects_to_direction_conflicts(relationships: List[Dict[str, Any]], room_ids: Set[str]) -> None:
    ...
    for ((src, direction), rels) in buckets.items():
        ...
        raise DataPackageError(ERROR_WORLD_DATA_SEMANTIC_CONFLICT, f"direction conflict: source={src} direction={direction} has multiple connects_to (rels={','.join(rel_ids)}). Rule: one room cannot reach more than one room on the same direction.")
REQUIRED_FILES = ['world.yaml', 'buildings.yaml', 'floors.yaml', 'rooms.yaml', 'relationships.yaml', 'package_meta.yaml', 'entities/npcs.yaml', 'entities/items.yaml', 'entities/zones.yaml', 'concepts/goals.yaml', 'concepts/processes.yaml', 'concepts/rules.yaml', 'concepts/behaviors.yaml', 'concepts/skills.yaml']
...
def validate_data_package(data_root: Path) -> Dict[str, Any]:
    ...
    entities = {'npcs': _require_list(npcs_doc, 'npcs', 'entities/npcs.yaml'), 'items': _require_list(items_doc, 'items', 'entities/items.yaml'), 'zones': _require_list(zones_doc, 'zones', 'entities/zones.yaml')}
    concepts = {'goals': _require_list(goals_doc, 'goals', 'concepts/goals.yaml'), 'processes': _require_list(processes_doc, 'processes', 'concepts/processes.yaml'), 'rules': _require_list(rules_doc, 'rules', 'concepts/rules.yaml'), 'behaviors': _require_list(behaviors_doc, 'behaviors', 'concepts/behaviors.yaml'), 'skills': _require_list(skills_doc, 'skills', 'concepts/skills.yaml')}
    ...
    if schema_ver not in SUPPORTED_SCHEMA_VERSIONS:
        raise DataPackageError(ERROR_WORLD_DATA_SCHEMA_UNSUPPORTED, f'schema_version {schema_ver} not in supported {sorted(SUPPORTED_SCHEMA_VERSIONS)}')
    ...
    for (bucket, arr) in entities.items():
        for row in arr:
            ...
            if tc not in allowed_pkg_types:
                raise DataPackageError(ERROR_WORLD_DATA_REFERENCE_BROKEN, f'entity type_code not mapped for graph profile: {bucket} {eid} ({tc})')
            loc = row.get('location_ref')
            if loc and str(loc) not in room_ids:
                raise DataPackageError(ERROR_WORLD_DATA_REFERENCE_BROKEN, f'entity location_ref not a room id: {bucket} {eid} -> {loc}')
            zref = row.get('zone_ref')
            if not zref:
                continue
            if bucket == 'zones':
                if str(zref) not in room_ids:
                    raise DataPackageError(ERROR_WORLD_DATA_REFERENCE_BROKEN, f'zone.zone_ref not a room id: {eid} -> {zref}')
            elif str(zref) not in zone_ids:
                raise DataPackageError(ERROR_WORLD_DATA_REFERENCE_BROKEN, f'entity zone_ref not a zone id: {bucket} {eid} -> {zref}')
    ...
    for rel in relationships:
        ...
        if rtc not in ALLOWED_REL_TYPE_CODES:
            raise DataPackageError(ERROR_WORLD_DATA_SEMANTIC_CONFLICT, f'relationship.rel_type_code not allowed: {rid} ({rtc})')
    ...
    if entry_hubs:
        _validate_world_entry_reachability(rooms=rooms, relationships=relationships, entry_room_id=_WORLD_ENTRY_ROOM_ID, required_entry_hubs=entry_hubs)
        ...
        for row in arr:
            ...
            if scope == 'zone':
                if zone_ids and (not any((str(t) in zone_ids for t in bindings))):
                    raise DataPackageError(ERROR_WORLD_DATA_SEMANTIC_CONFLICT, f'concept.scope=zone should bind at least one zone id: {cid}')
