...
class AdminAccount(DefaultAccount):
    ...
    def __init__(self, username: str, email: str, **kwargs):
        admin_attrs = {'roles': ['admin'], 'permissions': ['user.*', 'campus.*', 'world.*', 'system.*', 'admin.*'], 'access_level': 'admin', 'is_verified': True, 'max_failed_attempts': 10, 'created_by': kwargs.get('created_by', 'system'), **kwargs}
    ...
    def get_admin_dashboard_data(self) -> Dict[str, Any]:
        """获取管理员仪表板数据"""
        return {'username': self.username, 'role': 'admin', 'permissions': self.permissions, 'last_login': self.last_login.isoformat() if self.last_login else None, 'last_activity': self.last_activity.isoformat() if self.last_activity else None, 'system_status': 'active', 'can_manage_users': True, 'can_manage_campus': True, 'can_manage_world': True, 'can_manage_system': True}
...
class DeveloperAccount(DefaultAccount):
    ...
    def __init__(self, username: str, email: str, **kwargs):
        dev_attrs = {'roles': ['dev'], 'permissions': ['user.view', 'campus.view', 'campus.edit', 'campus.manage', 'world.view', 'world.edit', 'world.manage', 'system.view', 'system.debug', 'system.test', 'system.develop', 'logs.view', 'ontology.read', 'ontology.write', 'graph.read', 'graph.write'], 'access_level': 'developer', 'is_verified': True, 'max_failed_attempts': 8, 'created_by': kwargs.get('created_by', 'admin'), **kwargs}
    ...
    def get_dev_dashboard_data(self) -> Dict[str, Any]:
        """获取开发者仪表板数据"""
        return {'username': self.username, 'role': 'dev', 'permissions': self.permissions, 'last_login': self.last_login.isoformat() if self.last_login else None, 'last_activity': self.last_activity.isoformat() if self.last_activity else None, 'can_develop': self.can_develop_features(), 'can_debug': self.can_access_debug_mode(), 'can_test': self.can_run_tests(), 'can_view_logs': self.can_view_logs()}
...
class UserAccount(DefaultAccount):
    ...
    def __init__(self, username: str, email: str, **kwargs):
        user_attrs = {'roles': ['user'], 'permissions': ['user.login', 'user.logout', 'user.view_profile', 'user.edit_profile', 'campus.view', 'world.view'], 'access_level': 'normal', 'is_verified': False, 'max_failed_attempts': 5, 'created_by': kwargs.get('created_by', 'system'), **kwargs}
    ...
    def get_user_dashboard_data(self) -> Dict[str, Any]:
        """获取用户仪表板数据"""
        return {'username': self.username, 'role': 'user', 'permissions': self.permissions, 'last_login': self.last_login.isoformat() if self.last_login else None, 'last_activity': self.last_activity.isoformat() if self.last_activity else None, 'is_verified': self.is_verified, 'can_view_campus': self.can_view_campus(None), 'can_view_world': self.can_view_world(None), 'can_edit_profile': self.can_edit_profile()}
...
class CampusUserAccount(UserAccount):
    ...
    def __init__(self, username: str, email: str, **kwargs):
        campus_attrs = {'roles': ['user', 'campus_user'], 'permissions': ['user.login', 'user.logout', 'user.view_profile', 'user.edit_profile', 'campus.view', 'campus.join', 'campus.leave', 'campus.participate', 'world.view', 'ontology.read', 'graph.read'], 'campus_memberships': [], 'campus_activities': [], **kwargs}
    ...
    def join_campus(self, campus, role: str='member') -> bool:
        ...
        try:
            ...
            if relationship:
                ...
                memberships.append({'campus_id': campus.id if hasattr(campus, 'id') else None, 'campus_name': campus.name if hasattr(campus, 'name') else 'Unknown', 'role': role, 'joined_at': datetime.now().isoformat()})
    ...
    def get_campus_role(self, campus) -> Optional[str]:
        ...
        return None
ACCOUNT_TYPES = {'admin': AdminAccount, 'dev': DeveloperAccount, 'user': UserAccount, 'campus_user': CampusUserAccount}
