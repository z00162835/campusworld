...
def greedy_decode_intent_json(*, model, tokenizer, device, system_prompt: str, user_message: str, max_new_tokens: int) -> Dict[str, Any]:
    """Single-turn chat prompt → strict JSON object with intent label."""
    import torch
    messages: List[Dict[str, str]] = [{'role': 'system', 'content': system_prompt.strip()}, {'role': 'user', 'content': user_message}]
    tmpl_kw = chat_template_kwargs_for_intent(tokenizer)
    prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True, **tmpl_kw)
    inputs = tokenizer(prompt, return_tensors='pt')
    inputs = {k: v.to(device) for (k, v) in inputs.items()}
    nt = clamp_intent_max_new_tokens(max_new_tokens)
    with torch.inference_mode():
        output_ids = model.generate(**inputs, max_new_tokens=int(nt), do_sample=False, pad_token_id=tokenizer.pad_token_id, eos_token_id=tokenizer.eos_token_id)
