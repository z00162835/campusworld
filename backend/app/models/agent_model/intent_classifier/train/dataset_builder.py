...
def load_jsonl(path: Path) -> List[IntentTrainSample]:
    ...
    for (line_no, line) in enumerate(path.read_text(encoding='utf-8').splitlines(), start=1):
        ...
        rows.append(IntentTrainSample(text=text, label=label, sample_id=obj.get('id') if obj.get('id') is not None else None, language=str(obj['language']) if obj.get('language') is not None else None, reason_tokens=reason_tokens, has_execute_confirmation=obj.get('has_execute_confirmation') if 'has_execute_confirmation' in obj else None, conflict_case=obj.get('conflict_case') if 'conflict_case' in obj else None))
    return rows
...
def build_classifier_payload(sample: IntentTrainSample) -> Dict[str, object]:
    ...
    return {'intent': sample.label, 'confidence': confidence, 'reason_tokens': reason_tokens, 'source': 'small_model'}
...
def build_chat_turns(system_prompt: str, sample: IntentTrainSample) -> List[Dict[str, str]]:
    ...
    return [{'role': 'system', 'content': system_prompt.strip()}, {'role': 'user', 'content': sample.text}, {'role': 'assistant', 'content': assistant_text}]
...
def train_val_indices(n: int, val_ratio: float, seed: int) -> Tuple[List[int], List[int]]:
    ...
    for i in range(n - 1, 0, -1):
        rnd = 1103515245 * rnd + 12345 & 2147483647
        j = rnd % (i + 1)
