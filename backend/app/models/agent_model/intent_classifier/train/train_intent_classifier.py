...
import yaml
from app.models.agent_model.intent_classifier.train.dataset_builder import IntentTrainSample, build_chat_turns, load_jsonl, split_samples
...
def train_stub(samples: Sequence[IntentTrainSample]) -> Dict[str, object]:
    ...
    return {'total_samples': total, 'label_counts': label_counts, 'majority_label': majority, 'note': 'stub summary only'}
...
def run_supervised_training(*, samples: Sequence[IntentTrainSample], system_prompt: str, training_cfg: Mapping[str, Any], output_dir: Path) -> Dict[str, Any]:
    ...
    probe_prompt = tokenizer.apply_chat_template(probe_turns[:-1], tokenize=True, add_generation_prompt=True, return_tensors=None)
    probe_completion = tokenizer.apply_chat_template(probe_turns[-1:], tokenize=True, add_generation_prompt=False, return_tensors=None)
    ...
    def row_dict(idx: int) -> Dict[str, Any]:
        s = samples[idx]
        return {'text': s.text, 'label': s.label, 'reason_tokens': list(s.reason_tokens), 'conflict_case': s.conflict_case, 'has_execute_confirmation': s.has_execute_confirmation}
    ...
    def tok_batch(examples: Dict[str, List[Any]]) -> Dict[str, List[List[int]]]:
        ...
        for i in range(batch_n):
            ...
            sample = IntentTrainSample(text=str(examples['text'][i]), label=str(examples['label'][i]), reason_tokens=reason_tokens, conflict_case=examples['conflict_case'][i], has_execute_confirmation=examples['has_execute_confirmation'][i])
            turns = build_chat_turns(system_prompt, sample)
            p_ids = tokenizer.apply_chat_template(turns[:-1], tokenize=True, add_generation_prompt=True, return_tensors=None)
            c_ids = tokenizer.apply_chat_template(turns[-1:], tokenize=True, add_generation_prompt=False, return_tensors=None)
    ...
    model = AutoModelForCausalLM.from_pretrained(base_model_id, trust_remote_code=True, torch_dtype=torch_dtype)
    target_modules = training_cfg.get('lora_target_modules') or ['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj']
    ...
    lora_config = LoraConfig(r=int(training_cfg.get('lora_r') or 16), lora_alpha=int(training_cfg.get('lora_alpha') or 32), lora_dropout=float(training_cfg.get('lora_dropout') or 0.05), bias='none', task_type=TaskType.CAUSAL_LM, target_modules=target_modules)
    model = get_peft_model(model, lora_config)
    if bool(training_cfg.get('gradient_checkpointing')):
        model.enable_input_require_grads()
        model.gradient_checkpointing_enable()
    collator = DataCollatorForCausalPad(tokenizer)
    train_kw: Dict[str, Any] = dict(output_dir=str(output_dir / 'hf_trainer'), num_train_epochs=float(training_cfg.get('num_train_epochs') or 3), per_device_train_batch_size=int(training_cfg.get('per_device_train_batch_size') or 1), per_device_eval_batch_size=int(training_cfg.get('per_device_eval_batch_size') or 1), gradient_accumulation_steps=int(training_cfg.get('gradient_accumulation_steps') or 4), learning_rate=float(training_cfg.get('learning_rate') or 0.0002), weight_decay=float(training_cfg.get('weight_decay') or 0.01), warmup_ratio=float(training_cfg.get('warmup_ratio') or 0.03), logging_steps=int(training_cfg.get('logging_steps') or 1), save_strategy='epoch', bf16=bool(training_cfg.get('bf16')), fp16=bool(training_cfg.get('fp16')), seed=seed, report_to=[], save_total_limit=2)
    if eval_ds is not None:
    ...
    try:
        ...
        training_args = TrainingArguments(**train_kw)
    trainer = Trainer(model=model, args=training_args, train_dataset=train_ds, eval_dataset=eval_ds, data_collator=collator, tokenizer=tokenizer)
    ...
    (output_dir / 'training_config.json').write_text(json.dumps(training_cfg_dump, ensure_ascii=False, indent=2), encoding='utf-8')
    return {'metrics': metrics, 'adapter_dir': str(adapter_dir.resolve()), 'train_examples': len(train_idx), 'eval_examples': len(val_idx), 'base_model_id': base_model_id}
...
class DataCollatorForCausalPad:
    ...
    def __call__(self, features: List[MutableMapping[str, Any]]) -> Dict[str, Any]:
        ...
        batch = {'input_ids': torch.tensor(input_batch, dtype=torch.long), 'labels': torch.tensor(label_batch, dtype=torch.long), 'attention_mask': torch.tensor(attn_batch, dtype=torch.long)}
        return batch
...
def main() -> int:
    ...
    parser.add_argument('--system-prompt-file', default=str(_prompt_path()), help='System prompt path used during SFT')
    parser.add_argument('--stub-metadata-only', action='store_true', help='Skip HF training; emit export_meta.json summary only')
    ...
    export_meta: Dict[str, Any] = {'model_version': args.model_version, 'data_version': args.data_version, 'trained_at': datetime.now(tz=timezone.utc).isoformat(), 'data_path': str(data_path), 'system_prompt_file': str(Path(args.system_prompt_file).resolve()), 'stub_summary': summary_stub}
    if args.stub_metadata_only:
        ...
        return 0
    try:
        train_report = run_supervised_training(samples=samples, system_prompt=system_prompt, training_cfg=training_cfg, output_dir=out_dir)
    except ImportError as exc:
        print('Missing ML dependencies. Use Conda env campusworld (conda activate campusworld; cd backend), install PyTorch for your platform, then:\n  pip install -r requirements/ml-intent-classifier.txt\n', f'Original error: {exc}')
        return 2
