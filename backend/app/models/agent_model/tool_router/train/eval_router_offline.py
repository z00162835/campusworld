...
def _per_label_counts(pairs: Sequence[Tuple[Set[str], Set[str]]]) -> Tuple[MutableMapping[str, int], MutableMapping[str, int], MutableMapping[str, int]]:
    tp: Dict[str, int] = {}
    fp: Dict[str, int] = {}
    fn: Dict[str, int] = {}
...
def parse_gold_rows(raw: Iterable[Mapping[str, Any]]) -> List[GoldRow]:
    ...
    for (i, obj) in enumerate(raw):
        ...
        out.append(GoldRow(example_id=eid if isinstance(eid, str) and eid.strip() else None, gold_tool_names=[str(x) for x in gtn], mandatory_subset=[str(x) for x in ms], data_source=ds))
    return out
...
def parse_pred_rows(raw: Iterable[Mapping[str, Any]]) -> List[PredRow]:
    ...
    for (i, obj) in enumerate(raw):
        ...
        out.append(PredRow(example_id=eid if isinstance(eid, str) and eid.strip() else None, predicted_tool_names=[str(x) for x in ptn]))
    return out

def align_gold_pred(gold: Sequence[GoldRow], pred: Sequence[PredRow]) -> List[Tuple[GoldRow, PredRow]]:
...
def evaluate_pairs(pairs: Sequence[Tuple[GoldRow, PredRow]]) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {'n_pairs': n, 'subset_exact_match_rate': subset_hits / n if n else 0.0, 'macro_f1': macro_f1, 'mandatory_recall': mand_hits / mand_count if mand_count else None, 'mandatory_evaluated_n': mand_count, 'by_data_source': {k: {'n': int(v['n']), 'subset_exact_match_rate': v['subset_hits'] / v['n'] if v['n'] else 0.0, 'mandatory_recall': v['mand_hits'] / v['mand_n'] if v['mand_n'] else None} for (k, v) in sorted(by_source.items())}}
    return out
...
def main() -> int:
    ...
    p.add_argument('--validate-gold', action='store_true', help='Validate each gold line against tool_router_train_row.schema.json')
    args = p.parse_args()
