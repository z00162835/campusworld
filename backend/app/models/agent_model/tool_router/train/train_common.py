...
SLOT_SYSTEM_PROMPT = 'Output a single JSON object only. Keys: target_hint (string|null); named_spans (array of {text,type}); entities (array of {normalized_text,entity_type}); mandatory_tools (array of command names, may be empty).'
ROUTER_SYSTEM_PROMPT = 'Output a single JSON object only with key "tool_names" (array of strings). List every applicable registered tool/command name for this turn; use [] if none.'
