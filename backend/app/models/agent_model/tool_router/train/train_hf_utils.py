...
def encode_chat_sft_example(tokenizer: PreTrainedTokenizerBase, *, system: str, user: str, assistant_text: str, max_seq_len: int, completion_reserved: int) -> Dict[str, Any]:
...
def run_lora_training(*, base_model_id: str, output_dir: str, train_features: List[Dict[str, Any]], tokenizer: Optional[PreTrainedTokenizerBase], epochs: float, batch_size: int, grad_accum: int, lr: float, lora_r: int, lora_alpha: int, warmup_ratio: float, bf16: bool, seed: int) -> None:
    ...
    if use_cuda:
        ...
        model = AutoModelForCausalLM.from_pretrained(base_model_id, torch_dtype=dtype, trust_remote_code=True, device_map='auto')
    else:
        model = AutoModelForCausalLM.from_pretrained(base_model_id, torch_dtype=torch.float32, trust_remote_code=True)
    peft_config = LoraConfig(r=lora_r, lora_alpha=lora_alpha, lora_dropout=0.05, bias='none', task_type=TaskType.CAUSAL_LM, target_modules=['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj'])
    ...
    args = TrainingArguments(output_dir=output_dir, num_train_epochs=epochs, per_device_train_batch_size=batch_size, gradient_accumulation_steps=grad_accum, learning_rate=lr, warmup_ratio=warmup_ratio, logging_steps=logging_steps, save_strategy='epoch', seed=seed, bf16=use_bf16, fp16=bool(use_cuda and (not use_bf16)), remove_unused_columns=False, report_to=[])
