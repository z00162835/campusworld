...
class DefaultObject(GraphNodeInterface):
    ...
    def __init__(self, name: str, **kwargs):
        ...
        known_fixed_fields = {'description', 'is_active', 'is_public', 'access_level', 'location_id', 'home_id', 'tags', 'attributes', 'disable_auto_sync'}
        extra_attributes = {k: v for (k, v) in kwargs.items() if k not in known_fixed_fields and (not hasattr(self, f'_node_{k}'))}
    ...
    def execute_command(self, command_string: str, caller=None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not cmdset:
                return {'success': False, 'error': '命令集合未初始化', 'command': command_string}
            ...
            if not results:
                error_result = {'success': False, 'error': '命令执行失败，无结果返回', 'command': command_string}
            ...
            if len(results) == 1:
                ...
                return result
            combined_result = {'success': all((r.get('success', False) for r in results)), 'command': command_string, 'results': results, 'result_count': len(results)}
    ...
    def _add_command_to_history(self, command: str, result: Dict[str, Any]):
        """添加命令到历史记录"""
        history_entry = {'command': command, 'timestamp': datetime.now(), 'result': result, 'success': result.get('success', False)}
    ...
    def check_permission(self, required_permission: str) -> bool:
        ...
        if not self.has_permission(required_permission):
            return False
        permission_hierarchy = {'user': 1, 'moderator': 2, 'admin': 3, 'owner': 4}
    ...
    def get_room_list_display_name(self, looker=None, **kwargs) -> str:
        ...
        if isinstance(inner, dict):
            for key in ('room_list_name', 'look_list_name', 'short_name'):
                ...
                if isinstance(v, str) and v.strip():
                    return v.strip()
        for cand in (str(a.get('display_name') or '').strip(), str(self._node_name or '').strip()):
            if ' · ' in cand:
    ...
    def get_display_desc(self, looker=None, **kwargs) -> str:
        ...
        text_keys = ('long_description', 'short_description', 'description', 'entry_description', 'look_description', 'examine_desc')
    ...
    def build_synthetic_look_desc(self) -> str:
        ...
        return format_attributes_from_schema_definition(self._attributes_for_schema_look(), sd).strip()
    ...
    def room_line_format_kwargs(self) -> Dict[str, Any]:
        ...
        return {'name': self.get_room_list_display_name(), 'extra_name_info': self.get_display_extra_name_info(), 'hints': hints, 'state_suffix': '', 'power_hint': '', 'wifi_hint': '', 'av_hint': '', 'short_blurb': str(a.get('short_blurb') or '')[:200], 'hint': str(a.get('look_hint') or a.get('entry_hint') or '')[:200]}
    ...
    def _schedule_node_sync(self) -> None:
        ...
        try:
            ...
            logging.getLogger('campusworld.model.sync').warning(
                f'Failed to sync object to graph database: {type(self).__name__}, error: {e}'
            )
...
class DefaultAccount(DefaultObject):
    ...
    def __init__(self, username: str, email: str, **kwargs):
        ...
        account_attrs = {'username': username, 'email': email, 'is_verified': False, 'is_locked': False, 'is_suspended': False, 'login_count': 0, 'failed_login_attempts': 0, 'max_failed_attempts': 5, 'roles': ['user'], 'permissions': [], 'hashed_password': kwargs.get('hashed_password', ''), 'last_login': None, 'last_activity': None, 'lock_reason': None, 'suspension_reason': None, 'suspension_until': None, 'created_by': kwargs.get('created_by', 'system'), 'created_at': datetime.now(), 'updated_at': datetime.now(), **kwargs}
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        """获取账户状态摘要"""
        return {'username': self.username, 'email': self.email, 'is_verified': self.is_verified, 'is_locked': self.is_locked, 'is_suspended': self.is_suspended, 'roles': self.roles, 'permissions': self.permissions, 'login_count': self.login_count, 'last_login': self.last_login.isoformat() if self.last_login else None, 'last_activity': self.last_activity.isoformat() if self.last_activity else None, 'lock_reason': self.lock_reason, 'suspension_reason': self.suspension_reason, 'suspension_until': self.suspension_until.isoformat() if self.suspension_until else None}
