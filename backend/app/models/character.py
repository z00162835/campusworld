...
class Character(DefaultObject):
    ...
    def __init__(self, name: str, config: Dict[str, Any]=None, **kwargs):
        ...
        default_attrs = {'character_type': 'generic', 'character_class': 'commoner', 'character_race': 'human', 'short_description': '', 'long_description': '', 'level': 1, 'experience': 0, 'strength': 10, 'agility': 10, 'intelligence': 10, 'constitution': 10, 'wisdom': 10, 'charisma': 10, 'health': 100, 'max_health': 100, 'energy': 100, 'max_energy': 100, 'mana': 50, 'max_mana': 50, 'is_alive': True, 'is_conscious': True, 'is_awake': True, 'can_move': True, 'can_talk': True, 'can_see': True, 'can_hear': True, 'can_touch': True, 'can_walk': True, 'can_run': True, 'can_jump': True, 'can_climb': False, 'can_swim': False, 'can_fly': False, 'is_player': False, 'is_npc': True, 'is_ai': False, 'last_action_time': None, 'last_rest_time': None, 'action_costs': {'walk': 5, 'run': 15, 'jump': 10, 'climb': 20, 'swim': 25, 'fly': 30, 'rest': 0, 'talk': 0, 'look': 0}, 'recovery_rates': {'energy_per_hour': 10, 'health_per_hour': 2, 'mana_per_hour': 5}, **kwargs}
    ...
    def at_action_cost(self, action_name: str) -> Dict[str, Any]:
        ...
        if cost > 0 and self.energy >= cost:
            self.energy -= cost
            return {'success': True, 'energy_cost': cost, 'remaining_energy': self.energy}
        elif cost > 0:
            return {'success': False, 'error': f'{self.name} 体力不足，需要 {cost} 点体力'}
    ...
    def calculate_recovery(self, duration: int) -> Dict[str, int]:
        ...
        return {'energy_recovery': actual_energy_recovery, 'health_recovery': actual_health_recovery, 'mana_recovery': actual_mana_recovery}
    ...
    def execute_command(self, command_string: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not parts:
                return {'success': False, 'error': '空命令', 'command': command_string}
            command_name = parts[0]
            args = parts[1:] if len(parts) > 1 else []
            command = self.get_command(command_name)
            if not command:
                return {'success': False, 'error': f'未知命令: {command_name}', 'command': command_string}
            from app.commands.context import CommandContext
            context = CommandContext(caller=self, **kwargs)
            if not command.validate_args(args):
                return {'success': False, 'error': f'参数验证失败: {command.get_usage()}', 'command': command_string}
            if not command.check_permission(context):
                return {'success': False, 'error': '权限不足', 'command': command_string}
            result = command.execute(context, args)
            self._add_command_to_history(command_string, result)
            return {'success': result.success, 'message': result.message, 'error': result.error, 'command': command_string, 'command_type': result.command_type}
        except Exception as e:
            error_result = {'success': False, 'error': str(e), 'command': command_string}
    ...
    def get_base_stats(self) -> Dict[str, int]:
        """获取基础属性"""
        return {'strength': self._node_attributes.get('strength', 10), 'agility': self._node_attributes.get('agility', 10), 'intelligence': self._node_attributes.get('intelligence', 10), 'constitution': self._node_attributes.get('constitution', 10), 'wisdom': self._node_attributes.get('wisdom', 10), 'charisma': self._node_attributes.get('charisma', 10)}
    ...
    def take_damage(self, damage: int) -> Dict[str, Any]:
        ...
        if new_health <= 0:
            self.die()
            return {'success': True, 'message': f'{self.name} 受到 {damage} 点伤害并死亡', 'damage': damage, 'died': True}
        return {'success': True, 'message': f'{self.name} 受到 {damage} 点伤害', 'damage': damage, 'remaining_health': new_health}
    ...
    def heal(self, amount: int) -> Dict[str, Any]:
        ...
        return {'success': True, 'message': f'{self.name} 恢复了 {actual_healing} 点生命值', 'healing': actual_healing, 'current_health': self.health}
    ...
    def gain_experience(self, amount: int) -> Dict[str, Any]:
        ...
        return {'success': True, 'message': f'{self.name} 获得了 {amount} 点经验值', 'experience_gained': amount, 'total_experience': new_exp}
    ...
    def can_perform_action(self, action_type: str) -> bool:
        ...
        if not self.is_conscious:
            return False
        ability_map = {'move': 'can_move', 'talk': 'can_talk', 'see': 'can_see', 'hear': 'can_hear', 'touch': 'can_touch', 'walk': 'can_walk', 'run': 'can_run', 'jump': 'can_jump', 'climb': 'can_climb', 'swim': 'can_swim', 'fly': 'can_fly'}
    ...
    def get_character_info(self) -> Dict[str, Any]:
        """获取角色基础信息"""
        return {'id': self.id, 'uuid': self._node_uuid, 'name': self._node_name, 'type': self._node_attributes.get('character_type'), 'class': self._node_attributes.get('character_class'), 'race': self._node_attributes.get('character_race'), 'level': self.level, 'experience': self._node_attributes.get('experience', 0), 'health': self.health, 'max_health': self.max_health, 'energy': self.energy, 'max_energy': self.max_energy, 'mana': self.mana, 'max_mana': self.max_mana, 'stats': self.get_base_stats(), 'is_alive': self.is_alive, 'is_conscious': self.is_conscious, 'is_player': self.is_player, 'is_npc': self.is_npc, 'is_ai': self._node_attributes.get('is_ai', False), 'abilities': {'can_move': self._node_attributes.get('can_move', True), 'can_talk': self._node_attributes.get('can_talk', True), 'can_see': self._node_attributes.get('can_see', True), 'can_hear': self._node_attributes.get('can_hear', True), 'can_touch': self._node_attributes.get('can_touch', True), 'can_walk': self._node_attributes.get('can_walk', True), 'can_run': self._node_attributes.get('can_run', True), 'can_jump': self._node_attributes.get('can_jump', True), 'can_climb': self._node_attributes.get('can_climb', False), 'can_swim': self._node_attributes.get('can_swim', False), 'can_fly': self._node_attributes.get('can_fly', False)}, 'available_commands': self.get_available_commands(), 'location_id': self.location_id, 'created_at': self._node_created_at.isoformat() if self._node_created_at else None, 'updated_at': self._node_updated_at.isoformat() if self._node_updated_at else None}
