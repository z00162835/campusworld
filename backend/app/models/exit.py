...
class Exit(DefaultObject):
    ...
    def __init__(self, name: str, source_room_id: int, destination_room_id: int, config: Dict[str, Any]=None, **kwargs):
        disable_auto_sync = bool(kwargs.pop('disable_auto_sync', False))
        self._node_type = 'exit'
        default_attrs = {'exit_name': name, 'exit_aliases': [], 'exit_description': '', 'exit_short_description': '', 'source_room_id': source_room_id, 'destination_room_id': destination_room_id, 'exit_locks': {}, 'exit_permissions': [], 'exit_requirements': [], 'exit_roles': [], 'is_hidden': False, 'is_locked': False, 'is_one_way': False, 'is_closed': False, 'is_broken': False, 'move_message': '', 'arrive_message': '', 'move_fail_message': '', 'exit_cost': 0, 'exit_delay': 0, 'exit_type': 'normal', 'exit_subtype': '', 'exit_key': None, 'exit_script': None, 'exit_effects': [], 'exit_created_date': None, 'exit_last_used': None, 'exit_use_count': 0}
    ...
    def at_post_traverse(self, character: 'Character', source_room_id: Optional[int], destination: 'Room'):
        """移动后钩子"""
        pass
        ...
        try:
            ...
            try:
                if isinstance(source_id, int):
                    room_node = session.query(Node).filter(Node.id == source_id, Node.type_code == 'room', Node.is_active == True).first()
                else:
                    return None
                if room_node:
                    from .room import Room
                    room_obj = Room(name=room_node.name, config={'attributes': room_node.attributes or {}})
        ...
        try:
            ...
            try:
                if isinstance(dest_id, int):
                    room_node = session.query(Node).filter(Node.id == dest_id, Node.type_code == 'room', Node.is_active == True).first()
                else:
                    return None
                if room_node:
                    from .room import Room
    ...
    def get_exit_info(self) -> Dict[str, Any]:
        """获取出口详细信息"""
        return {'id': self.id if hasattr(self, 'id') else None, 'uuid': self._node_uuid, 'name': self._node_name, 'exit_name': self._node_attributes.get('exit_name'), 'aliases': self.get_aliases(), 'description': self.get_description(), 'exit_type': self._node_attributes.get('exit_type'), 'source_room_id': self._node_attributes.get('source_room_id'), 'destination_room_id': self._node_attributes.get('destination_room_id'), 'is_locked': self._node_attributes.get('is_locked', False), 'is_hidden': self._node_attributes.get('is_hidden', False), 'is_one_way': self._node_attributes.get('is_one_way', False), 'is_closed': self._node_attributes.get('is_closed', False), 'use_count': self._node_attributes.get('exit_use_count', 0), 'last_used': self._node_attributes.get('exit_last_used')}
