...
class CombatMixin(ComponentMixin):
    ...
    def attack_target(self, target: 'CombatMixin') -> dict:
        ...
        return {'success': True, 'damage': actual_damage, 'target_health': target.health, 'target_alive': target.is_alive}
...
class ModelFactory:
    ...
    def create_custom_object(self, name: str, object_type: str, **attributes) -> Dict[str, Any]:
        ...
        Returns:
            对象配置字典
        """
        base_config = {'name': name, 'type': object_type, 'attributes': attributes or {}, 'tags': [object_type], 'is_active': True, 'is_public': True, 'access_level': 'normal'}
        if object_type == 'player':
            base_config.update({'is_movable': True, 'is_interactive': True, 'is_visible': True, 'level': 1, 'health': 100, 'max_health': 100, 'energy': 100, 'max_energy': 100})
        elif object_type == 'npc':
            base_config.update({'is_movable': False, 'is_interactive': True, 'is_visible': True, 'level': 1, 'health': 50, 'max_health': 50})
        elif object_type == 'item':
            base_config.update({'is_movable': True, 'is_interactive': True, 'is_visible': True, 'rarity': 'common'})
        elif object_type == 'location':
            base_config.update({'is_movable': False, 'is_interactive': False, 'is_visible': True})
        return base_config
...
model_factory.register_model('user', None)
