...
class Node(TraitFilterMixin, Base, BaseNode):
    ...
    location = relationship('Node', foreign_keys=[location_id], remote_side=[id], backref='located_objects', overlaps='contents')
    home = relationship('Node', foreign_keys=[home_id], remote_side=[id], backref='home_objects')

    @property
    def contents(self):
        """获取当前位置的内容"""
        return self.located_objects
    source_relationships = relationship('Relationship', foreign_keys='Relationship.source_id', back_populates='source_node', lazy='dynamic')
    target_relationships = relationship('Relationship', foreign_keys='Relationship.target_id', back_populates='target_node', lazy='dynamic')

    def get_related_nodes(self, relationship_type: str=None):
        """获取相关节点"""
        if relationship_type:
            return [rel.target_node for rel in self.source_relationships if rel.type_code == relationship_type and rel.is_active]
    ...
    @classmethod
    def get_active_nodes(cls, session: Session, type_code: str=None, trait_class: str=None, required_any_mask: int=0, required_all_mask: int=0) -> List['Node']:
        """获取活跃节点"""
    ...
    @classmethod
    def get_paginated(cls, session: Session, page: int=1, page_size: int=20, type_code: str=None, is_active: bool=True, **filters) -> Dict[str, Any]:
        """
        分页获取节点
    ...
    def get_related_nodes(cls, session: Session, node_id: int, relationship_type: str=None) -> List['Node']:
        ...
        """
        source_query = session.query(cls).join(Relationship, and_(Relationship.source_id == node_id, Relationship.target_id == cls.id))
        target_query = session.query(cls).join(Relationship, and_(Relationship.target_id == node_id, Relationship.source_id == cls.id))
        if relationship_type:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """转换为字典"""
        return {'id': self.id, 'uuid': str(self.uuid) if self.uuid is not None else None, 'type_code': self.type_code, 'name': self.name, 'description': self.description, 'is_active': self.is_active, 'is_public': self.is_public, 'access_level': self.access_level, 'trait_class': self.trait_class, 'trait_mask': int(self.trait_mask or 0), 'location_id': self.location_id, 'home_id': self.home_id, 'geom_geojson': self.geom_geojson, 'ts_data_ref_id': str(self.ts_data_ref_id) if self.ts_data_ref_id is not None else None, 'attributes': self.attributes, 'tags': self.tags, 'created_at': self.created_at.isoformat() if self.created_at else None, 'updated_at': self.updated_at.isoformat() if self.updated_at else None}
...
class Relationship(TraitFilterMixin, Base, BaseRelationship):
    ...
    __table_args__ = (Index('idx_relationship_type', 'type_code'), Index('idx_relationship_source', 'source_id'), Index('idx_relationship_target', 'target_id'), Index('idx_relationship_attributes', 'attributes', postgresql_using='gin'), Index('idx_relationship_tags', 'tags', postgresql_using='gin'))
    ...
    def get_between_nodes(cls, session: Session, source_id: int, target_id: int, type_code: str=None) -> List['Relationship']:
        """获取两个节点之间的关系"""
        query = session.query(cls).filter(or_(and_(cls.source_id == source_id, cls.target_id == target_id), and_(cls.source_id == target_id, cls.target_id == source_id)))
        if type_code:
            query = query.filter(cls.type_code == type_code)
        return query.all()

    @classmethod
    def get_active_relationships(cls, session: Session, type_code: str=None, trait_class: str=None, required_any_mask: int=0, required_all_mask: int=0) -> List['Relationship']:
        """获取活跃关系"""
        query = session.query(cls).filter(cls.is_active == True)
        if type_code:
            query = query.filter(cls.type_code == type_code)
    ...
    def to_dict(self) -> Dict[str, Any]:
        """转换为字典"""
        return {'id': self.id, 'uuid': str(self.uuid) if self.uuid is not None else None, 'type_code': self.type_code, 'source_id': self.source_id, 'target_id': self.target_id, 'source_role': self.source_role, 'target_role': self.target_role, 'is_active': self.is_active, 'weight': self.weight, 'trait_class': self.trait_class, 'trait_mask': int(self.trait_mask or 0), 'attributes': self.attributes, 'tags': self.tags, 'created_at': self.created_at.isoformat() if self.created_at else None, 'updated_at': self.updated_at.isoformat() if self.updated_at else None}
...
class NodeType(Base):
    ...
    __table_args__ = (Index('idx_node_type_code', 'type_code'), Index('idx_node_type_parent', 'parent_type_code'), Index('idx_node_type_status', 'status'), Index('idx_node_type_schema', 'schema_definition', postgresql_using='gin'), Index('idx_node_type_schema_default', 'schema_default', postgresql_using='gin'), Index('idx_node_type_inferred', 'inferred_rules', postgresql_using='gin'), Index('idx_node_type_ui', 'ui_config', postgresql_using='gin'))
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'id': self.id, 'type_code': self.type_code, 'parent_type_code': self.parent_type_code, 'type_name': self.type_name, 'typeclass': self.typeclass, 'status': self.status, 'classname': self.classname, 'module_path': self.module_path, 'description': self.description, 'schema_definition': self.schema_definition, 'schema_default': self.schema_default, 'inferred_rules': self.inferred_rules, 'tags': self.tags, 'ui_config': self.ui_config, 'trait_class': self.trait_class, 'trait_mask': int(self.trait_mask or 0), 'created_at': self.created_at.isoformat() if self.created_at else None, 'updated_at': self.updated_at.isoformat() if self.updated_at else None}
...
class RelationshipType(Base):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'id': self.id, 'type_code': self.type_code, 'type_name': self.type_name, 'typeclass': self.typeclass, 'status': self.status, 'constraints': self.constraints, 'description': self.description, 'schema_definition': self.schema_definition, 'inferred_rules': self.inferred_rules, 'tags': self.tags, 'ui_config': self.ui_config, 'trait_class': self.trait_class, 'trait_mask': int(self.trait_mask or 0), 'is_directed': self.is_directed, 'is_symmetric': self.is_symmetric, 'is_transitive': self.is_transitive, 'created_at': self.created_at.isoformat() if self.created_at else None, 'updated_at': self.updated_at.isoformat() if self.updated_at else None}
...
class NodeTagIndex(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    __table_args__ = (UniqueConstraint('node_id', 'tag', name='idx_node_tag_indexes_unique'),)
