...
class AgentLongTermMemory(Base):
    ...
    payload = Column(JSONB, nullable=False, server_default=text("'{}'::jsonb"))
    source_memory_entry_id = Column(BigInteger, ForeignKey('agent_memory_entries.id', ondelete='SET NULL'), nullable=True)
...
class AgentLongTermMemoryLink(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    agent_node_id = Column(Integer, ForeignKey('nodes.id', ondelete='CASCADE'), nullable=False)
    source_ltm_id = Column(BigInteger, ForeignKey('agent_long_term_memory.id', ondelete='CASCADE'), nullable=False)
    target_ltm_id = Column(BigInteger, ForeignKey('agent_long_term_memory.id', ondelete='CASCADE'), nullable=False)
