...
class BulletinBoard(DefaultObject):
    ...
    _MODE_ALIASES = {'next': 'next', 'n': 'next', 'prev': 'prev', 'p': 'prev', 'previous': 'prev'}
    ...
    def __init__(self, name: str='bulletin_board', config: Optional[Dict[str, Any]]=None, **kwargs):
        ...
        attrs = {'board_key': self.DEFAULT_BOARD_KEY, 'display_name': 'bulletin_board', 'desc': 'System bulletin board in SingularityRoom', 'entry_room': 'singularity_room', 'entity_kind': 'item', 'presentation_domains': ['room'], 'access_locks': {'view': 'all()', 'interact': 'all()'}, 'is_system_singleton': True, 'supports_markdown_notice': True, 'aliases': list(self._LOOK_ALIASES), 'created_at': datetime.now().isoformat()}
        ...
        super().__init__(name=name, attributes=attrs, tags=list(dict.fromkeys(tags)), disable_auto_sync=bool(kwargs.get('disable_auto_sync', False)), is_public=True, access_level='normal')
