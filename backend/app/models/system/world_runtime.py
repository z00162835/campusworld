...
class WorldRuntimeState(Base):
    ...
    __table_args__ = (Index('ix_world_runtime_states_status', 'status'), Index('ix_world_runtime_states_updated_at', 'updated_at'))
...
class WorldInstallJob(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
    __table_args__ = (Index('ix_world_install_jobs_world_id', 'world_id'), Index('ix_world_install_jobs_status', 'status'), Index('ix_world_install_jobs_created_at', 'created_at'), Index('ix_world_install_jobs_fingerprint', 'request_fingerprint'))
