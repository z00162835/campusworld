...
class ProtocolHandler(ABC):
    ...
    def create_context(self, user_id: str, username: str, session_id: str, permissions: List[str], session: Optional[Any]=None, game_state: Optional[Dict[str, Any]]=None, db_session: Optional[Any]=None, roles: Optional[List[str]]=None, metadata: Optional[Dict[str, Any]]=None) -> CommandContext:
        """创建命令上下文"""
        caller = None
        ...
        md = initial_metadata_for_session(db_session=db_session, user_id=user_id, username=username, extra=metadata)
        return CommandContext(user_id=user_id, username=username, session_id=session_id, permissions=permissions, caller=caller, session=session, game_state=game_state, db_session=db_session, roles=resolved_roles, metadata=md)
