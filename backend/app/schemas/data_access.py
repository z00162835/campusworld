...
class PermissionTemplate(BaseModel):
    ...
    world_ids: Optional[List[int]] = Field(default=None, description='If non-empty, node attributes.world_id (string) must match one of these.')
    type_codes: Optional[List[str]] = Field(default=None, description='If non-empty, node.type_code must be in this set.')
    relationships_codes: Optional[List[str]] = Field(default=None, description='If non-empty, relationship.type_code must be in this set.')
    node_ids: Optional[List[int]] = Field(default=None, description='If non-empty, node.id must be in this set (rare narrow allow).')
    exclude_nodes_without_world_id: bool = Field(default=False, description='If True, deny nodes with missing/null world_id in attributes (system-domain).')
