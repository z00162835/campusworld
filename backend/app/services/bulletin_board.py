...
class BulletinBoardService:
    ...
    def __init__(self, root_manager: Optional['RootNodeManager']=None, bulletin_manager: Optional[SystemBulletinManager]=None):
    ...
    def _strip_html_tags(self, text: str) -> str:
        ...
        no_script = re.sub('<(script|style)\\b[^>]*>.*?</\\1>', '', text, flags=re.IGNORECASE | re.DOTALL)
        no_tags = re.sub('<[^>]+>', '', no_script)
        return no_tags
    ...
    def _markdown_to_terminal_text(self, content_md: str) -> str:
        ...
        text = re.sub('`([^`]+)`', '\\1', text)
        text = re.sub('^\\s*#{1,6}\\s*', '', text, flags=re.MULTILINE)
    ...
    def list_notices(self, page: int=1, page_size: int=10) -> Dict[str, Any]:
        result = self._with_board(lambda session, board_id: self._mgr.list_published_notices(session, board_id, page=page, page_size=page_size))
        if result is None:
            return {'items': [], 'total': 0, 'total_pages': 1, 'page': max(1, page), 'page_size': page_size}
        return result

    def get_notice_by_page_index(self, page: int, index: int, page_size: int=10) -> Optional[Dict[str, Any]]:
        got = self._with_board(lambda session, board_id: self._mgr.get_notice_by_page_index(session, board_id, page, index, page_size=page_size))
        return got

    def get_notice_by_id(self, notice_id: int, *, public_only: bool=True) -> Optional[Dict[str, Any]]:
        got = self._with_board(lambda session, board_id: self._mgr.get_notice_by_id(session, notice_id, board_node_id=board_id, public_only=public_only))
        return got

    def admin_list_notices(self, *, status: Optional[str]=None, include_inactive: bool=True, page: int=1, page_size: int=10) -> Dict[str, Any]:
        got = self._with_board(lambda session, board_id: self._mgr.list_notices(session, board_id, include_inactive=include_inactive, status=status, page=page, page_size=page_size))
        if got is None:
            return {'items': [], 'total': 0, 'total_pages': 1, 'page': max(1, page), 'page_size': page_size}
        return got

    def publish_notice(self, title: str, content_md: str, *, author_id: Optional[int]=None) -> Optional[Dict[str, Any]]:
        node = self._with_board(lambda session, board_id: self._mgr.create_notice(session, board_id, title, content_md, status='published', author_id=author_id))
        if not node:
            return None
    ...
    def edit_notice(self, notice_id: int, *, title: Optional[str]=None, content_md: Optional[str]=None, editor_id: Optional[int]=None) -> Optional[Dict[str, Any]]:
        return self._with_board(lambda session, board_id: self._mgr.update_notice(session, notice_id, board_node_id=board_id, title=title, content_md=content_md, editor_id=editor_id))

    def archive_notice(self, notice_id: int, *, editor_id: Optional[int]=None) -> Optional[Dict[str, Any]]:
        return self._with_board(lambda session, board_id: self._mgr.archive_notice(session, notice_id, board_node_id=board_id, editor_id=editor_id))
