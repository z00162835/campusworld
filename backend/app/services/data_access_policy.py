...
def apply_world_scoped_node_read_policy(query: Query, policy: Optional[DataAccessV1], deny_all: bool, world_id_path: str) -> Query:
...
def relationship_policy_sql(rel_column, source_node_column, target_node_column, policy: DataAccessV1) -> ColumnElement:
    parts: List[ColumnElement] = []
    pt = policy.permission_template
...
def apply_relationship_read_policy(query: Query, policy: Optional[DataAccessV1], deny_all: bool, source_alias: Any, target_alias: Any) -> Query:
    if deny_all or policy is None:
        return query.filter(false())
    return query.filter(relationship_policy_sql(Relationship, source_alias, target_alias, policy))
...
def relationship_row_visible(rel: Relationship, source: Node, target: Node, policy: Optional[DataAccessV1], deny_all: bool) -> bool:
    if deny_all or policy is None:
        return False
    pt = policy.permission_template
    if pt.relationships_codes is not None and len(pt.relationships_codes) > 0:
...
def proposed_node_visible(type_code: str, attributes: Optional[Dict[str, Any]], policy: Optional[DataAccessV1], deny_all: bool, node_id: Optional[int]=None) -> bool:
...
def proposed_relationship_visible(type_code: str, source: Node, target: Node, policy: Optional[DataAccessV1], deny_all: bool) -> bool:
    if deny_all or policy is None:
        return False
    pt = policy.permission_template
    if pt.relationships_codes is not None and len(pt.relationships_codes) > 0:
...
def apply_ontology_relationship_type_read_policy(query: Query, policy: Optional[DataAccessV1], deny_all: bool) -> Query:
    if deny_all or policy is None:
        return query.filter(false())
    if policy.denied_relationships_codes:
