...
def search_ltm_by_embedding(session: Session, agent_node_id: int, query_vector: Sequence[float], *, k: int=8, embedding_model: Optional[str]=None, caller_account_node_id: Optional[int]=None, conversation_thread_id: Optional[uuid.UUID]=None) -> List[Tuple[int, str, float]]:
    """
...
def expand_ltm_linked_neighbors(session: Session, agent_node_id: int, seed_ltm_ids: Sequence[int], *, max_depth: int=2) -> Set[int]:
    """
...
def _ltm_belongs_to_agent(session: Session, ltm_id: int, agent_node_id: int) -> bool:
    row = session.query(AgentLongTermMemory.id).filter(AgentLongTermMemory.id == ltm_id, AgentLongTermMemory.agent_node_id == agent_node_id).first()
    return row is not None

def create_ltm_link(session: Session, *, agent_node_id: int, source_ltm_id: int, target_ltm_id: int, link_type: str, weight: float=1.0, payload: Optional[Dict[str, Any]]=None) -> AgentLongTermMemoryLink:
    ...
    row = AgentLongTermMemoryLink(agent_node_id=agent_node_id, source_ltm_id=source_ltm_id, target_ltm_id=target_ltm_id, link_type=link_type, weight=weight, payload=payload or {})
    session.add(row)
    session.flush()
    return row

def set_ltm_embedding(session: Session, ltm_id: int, agent_node_id: int, embedding: Sequence[float], *, embedding_model: str) -> None:
    ...
    n = session.query(AgentLongTermMemory).filter(AgentLongTermMemory.id == ltm_id, AgentLongTermMemory.agent_node_id == agent_node_id).first()
    if n is None:
...
def build_ltm_memory_context_for_tick(session: Session, agent_node_id: int, *, user_message: str, limit: int=8, max_chars: int=2000, caller_account_node_id: Optional[int]=None, conversation_thread_id: Optional[uuid.UUID]=None) -> Optional[str]:
    """
