...
def notice_node_to_dto(node: Node) -> Dict[str, Any]:
    ...
    attrs = _notice_attrs(node)
    return {'id': node.id, 'uuid': str(node.uuid), 'title': attrs.get('title', node.name), 'content_md': attrs.get('content_md', ''), 'status': attrs.get('status', 'published'), 'is_active': attrs.get('is_active', True), 'priority': attrs.get('priority', 'normal'), 'published_at': attrs.get('published_at'), 'updated_at': attrs.get('updated_at'), 'author_id': attrs.get('author_id'), 'notice_code': attrs.get('notice_code'), 'tags': attrs.get('tags', []) if isinstance(attrs.get('tags'), list) else []}
...
class SystemBulletinManager:
    ...
    def resolve_board_node_id(self, session: Session, root_node_id: int) -> Optional[int]:
        try:
            rows = session.query(Node).filter(and_(Node.type_code == 'system_bulletin_board', Node.location_id == root_node_id, Node.is_active == True, Node.attributes['board_key'].astext == BulletinBoard.DEFAULT_BOARD_KEY)).order_by(Node.created_at.asc()).limit(2).all()
            if not rows:
                return None
            if len(rows) > 1:
                logger.warning('duplicate system_bulletin_board detected under root=%s, keeping earliest id=%s', root_node_id, rows[0].id)
            return rows[0].id
    ...
    def _fetch_published_notice_candidates(self, session: Session, board_node_id: int) -> List[Node]:
        return session.query(Node).filter(and_(Node.type_code == 'system_notice', Node.location_id == board_node_id, Node.is_active == True, Node.attributes['status'].astext == 'published')).all()

    def list_published_notices(self, session: Session, board_node_id: int, page: int=1, page_size: int=10) -> Dict[str, Any]:
        if page < 1:
            page = 1
        if page_size < 1:
            page_size = 10
        try:
            ...
            for n in slice_rows:
                ...
                items.append({'id': dto['id'], 'title': dto['title'], 'published_at': dto['published_at'], 'priority': dto['priority'], 'status': dto['status']})
            return {'items': items, 'total': total, 'total_pages': total_pages, 'page': page, 'page_size': page_size}
        except Exception as e:
            logger.error('list_published_notices failed: %s', e)
            return {'items': [], 'total': 0, 'total_pages': 1, 'page': max(1, page), 'page_size': page_size}

    def get_notice_by_id(self, session: Session, notice_id: int, *, board_node_id: Optional[int]=None, public_only: bool=True) -> Optional[Dict[str, Any]]:
        try:
    ...
    def get_notice_by_page_index(self, session: Session, board_node_id: int, page: int, index: int, page_size: int=10) -> Optional[Dict[str, Any]]:
    ...
    def ensure_system_notice_type(self, session: Session) -> Optional[int]:
        try:
            ...
            if row:
                return row.id
            nt = NodeType(type_code='system_notice', type_name='SystemNotice', typeclass='app.models.system.system_notice.SystemNotice', classname='SystemNotice', module_path='app.models.system.system_notice', description='System bulletin notice', schema_definition=system_notice_node_type_schema_definition(), is_active=True)
            session.add(nt)
            session.commit()
            session.refresh(nt)
            return nt.id
    ...
    def create_notice(self, session: Session, board_node_id: int, title: str, content_md: str, *, status: str='published', author_id: Optional[int]=None, notice_code: Optional[str]=None, **kwargs: Any) -> Optional[Node]:
        ...
        if not type_id:
            return None
        try:
            notice = SystemNotice(title=title.strip(), content_md=str(content_md), status=status, author_id=author_id, notice_code=notice_code, disable_auto_sync=True, **kwargs)
            node = Node(uuid=notice._node_uuid, type_id=type_id, type_code='system_notice', name=notice._node_name, description=(notice._node_attributes or {}).get('title', ''), is_active=True, is_public=True, access_level='normal', location_id=board_node_id, home_id=board_node_id, attributes=notice._node_attributes, tags=getattr(notice, '_node_tags', []) or [])
            session.add(node)
            session.commit()
    ...
    def list_notices(self, session: Session, board_node_id: int, *, include_inactive: bool=False, status: Optional[str]=None, page: int=1, page_size: int=10) -> Dict[str, Any]:
        ...
        if page_size < 1:
            page_size = 10
        try:
            query = session.query(Node).filter(Node.type_code == 'system_notice', Node.location_id == board_node_id)
            ...
            return {'items': [notice_node_to_dto(n) for n in picked], 'total': total, 'total_pages': total_pages, 'page': page, 'page_size': page_size}
        except Exception as e:
            logger.error('list_notices failed: %s', e)
            return {'items': [], 'total': 0, 'total_pages': 1, 'page': max(1, page), 'page_size': page_size}

    def update_notice(self, session: Session, notice_id: int, *, board_node_id: Optional[int]=None, title: Optional[str]=None, content_md: Optional[str]=None, editor_id: Optional[int]=None) -> Optional[Dict[str, Any]]:
    ...
    def archive_notice(self, session: Session, notice_id: int, *, board_node_id: Optional[int]=None, editor_id: Optional[int]=None) -> Optional[Dict[str, Any]]:
