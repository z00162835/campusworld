...
TASK_PERMISSIONS: Final[Tuple[str, ...]] = (TASK_CREATE, TASK_READ, TASK_UPDATE, TASK_PUBLISH, TASK_CLAIM, TASK_ASSIGN, TASK_APPROVE, TASK_CANCEL, TASK_POOL_ADMIN, TASK_ADMIN)
    ...
    def has_permission(self, code: str) -> bool:
        ...
        for p in self.permissions:
            if p.endswith('.*') and code.startswith(p[:-1]):
                return True
        return False
SYSTEM_DEFAULT_PERMISSIONS: Final[FrozenSet[str]] = frozenset({TASK_CREATE, TASK_PUBLISH, TASK_CLAIM, TASK_READ})
SYSTEM_PRINCIPAL: Final[Principal] = Principal(id=0, kind='system', roles=frozenset(), group_tags=frozenset(), permissions=SYSTEM_DEFAULT_PERMISSIONS)
...
__all__ = ['TASK_CREATE', 'TASK_READ', 'TASK_UPDATE', 'TASK_PUBLISH', 'TASK_CLAIM', 'TASK_ASSIGN', 'TASK_APPROVE', 'TASK_CANCEL', 'TASK_POOL_ADMIN', 'TASK_ADMIN', 'TASK_PERMISSIONS', 'Principal', 'SYSTEM_DEFAULT_PERMISSIONS', 'SYSTEM_PRINCIPAL', 'register_task_permissions_into_admin']
