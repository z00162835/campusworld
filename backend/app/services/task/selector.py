...
from app.services.task.errors import EmptySelector, SelectorBoundsExceeded, UnknownTraitClass, UnknownTraitMaskName
_KNOWN_TRAIT_CLASSES: Set[str] = {'DEVICE', 'SPACE', 'AGENT', 'ENV', 'TASK', 'ITEM', 'PROCESS', 'ABSTRACT'}
...
def _known_trait_mask_names() -> Set[str]:
    ...
    return {name for name in dir(tm) if not name.startswith('_') and name.isupper() and isinstance(getattr(tm, name), int)}
...
def validate_selector(dsl: Optional[Dict[str, Any]], *, allow_empty_include: bool=False) -> ValidatedSelector:
    ...
    if not include and (not allow_empty_include):
        raise EmptySelector('selector.include must contain at least one clause (or supply TARGETS_PINNED edges and pass allow_empty_include=True)')
    ...
    return ValidatedSelector(raw=dict(dsl), schema_version=schema_version, anchor_ref=anchor_ref, bounds=bounds, include=include, exclude=exclude)
    ...
    for mask_field in ('trait_mask_any_of', 'trait_mask_all_of'):
        ...
        for name in names:
            if name not in known_masks:
                raise UnknownTraitMaskName(f'{where}.{mask_field} contains unknown trait_mask name {name!r}')
...
def enforce_bounds(validated: ValidatedSelector, *, actual_count: int) -> None:
    ...
    if actual_count < validated.bounds['min']:
        raise SelectorBoundsExceeded(reason='min', count=actual_count, limit=validated.bounds['min'])
    if actual_count > validated.bounds['max']:
        raise SelectorBoundsExceeded(reason='max', count=actual_count, limit=validated.bounds['max'])
__all__ = ['ValidatedSelector', 'validate_selector', 'enforce_bounds']
