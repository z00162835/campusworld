...
def list_pools(session: Session, *, include_inactive: bool=False, key_prefix: Optional[str]=None, limit: int=100, offset: int=0) -> List[Dict[str, Any]]:
...
def _row_to_dict(row) -> Dict[str, Any]:
    ...
    def _maybe_json(v):
        ...
        try:
            ...
            return v
    return {'id': int(row[0]), 'key': row[1], 'display_name': row[2], 'description': row[3], 'is_active': bool(row[4]), 'default_workflow_ref': _maybe_json(row[5]), 'default_visibility': row[6], 'default_priority': row[7], 'publish_acl': _maybe_json(row[8]), 'consume_acl': _maybe_json(row[9]), 'attributes': _maybe_json(row[10]), 'created_at': row[11], 'updated_at': row[12]}
