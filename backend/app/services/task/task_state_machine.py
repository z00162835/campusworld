...
def _load_workflow_spec(session: Session, *, key: str, version: int) -> Dict[str, Any]:
    row = session.execute(text('\n            SELECT spec, is_active\n              FROM task_workflow_definitions\n             WHERE key = :key AND version = :version\n            '), {'key': key, 'version': version}).first()
    if row is None:
...
def _resolve_active_workflow_version(session: Session, *, key: str) -> int:
    ...
    if row is None:
        raise WorkflowDefinitionNotFound(f'no active workflow_definition for key={key}')
    return int(row[0])

def _check_idempotency(session: Session, *, task_id: int, idempotency_key: Optional[str]) -> Optional[TransitionResult]:
    if not idempotency_key:
        return None
    row = session.execute(text('\n            SELECT event_seq, from_state, to_state, event, correlation_id, trace_id\n              FROM task_state_transitions\n             WHERE task_node_id = :tid AND idempotency_key = :key\n            '), {'tid': task_id, 'key': idempotency_key}).first()
    if row is None:
        return None
    snap = session.execute(text("SELECT (attributes->>'state_version')::int FROM nodes WHERE id = :tid"), {'tid': task_id}).first()
    state_version = int(snap[0]) if snap and snap[0] is not None else 0
    return TransitionResult(task_id=task_id, from_state=row[1], to_state=row[2], event=row[3], event_seq=int(row[0]), state_version=state_version, idempotent_replay=True, correlation_id=row[4], trace_id=row[5])

def _check_create_idempotency(session: Session, *, actor: Principal, idempotency_key: Optional[str]) -> Optional[TransitionResult]:
    ...
    if not idempotency_key:
        return None
    row = session.execute(text("\n            SELECT t.task_node_id,\n                   t.event_seq,\n                   t.from_state,\n                   t.to_state,\n                   t.event,\n                   t.correlation_id,\n                   t.trace_id,\n                   (n.attributes->>'state_version')::int AS state_version\n              FROM task_state_transitions t\n              JOIN nodes n ON n.id = t.task_node_id\n             WHERE t.event = 'create'\n               AND t.idempotency_key = :idem\n               AND t.actor_principal_kind = :actor_kind\n               AND (\n                    (:actor_kind = 'system' AND t.actor_principal_id IS NULL)\n                    OR t.actor_principal_id = :actor_id\n               )\n             ORDER BY t.created_at DESC\n             LIMIT 1\n            "), {'idem': idempotency_key, 'actor_kind': actor.kind, 'actor_id': actor.id}).first()
    if row is None:
        return None
    return TransitionResult(task_id=int(row[0]), from_state=row[2], to_state=row[3], event=row[4], event_seq=int(row[1]), state_version=int(row[7] or 0), idempotent_replay=True, correlation_id=row[5], trace_id=row[6])

def _check_required_role(session: Session, *, task_id: int, actor: Principal, required_role: str) -> None:
    """``actor_principal`` must hold ``role`` on an active assignment row."""
    if actor.kind == 'system':
        return
    row = session.execute(text('\n            SELECT 1\n              FROM task_assignments\n             WHERE task_node_id = :tid\n               AND is_active = TRUE\n               AND role = :role\n               AND principal_id = :pid\n               AND principal_kind = :pkind\n             LIMIT 1\n            '), {'tid': task_id, 'role': required_role, 'pid': actor.id, 'pkind': actor.kind}).first()
    if row is None:
        raise RoleRequiredError(f'actor (id={actor.id}, kind={actor.kind}) lacks active assignment with role={required_role} on task {task_id}')
...
def _add_assignment(session: Session, *, task_id: int, role: str, stage: str, actor: Principal, target_principal_id: Optional[int]=None, target_principal_kind: Optional[str]=None, target_principal_tag: Optional[str]=None) -> None:
    ...
    if pkind is None:
        ...
        ptag = None
    session.execute(text('\n            INSERT INTO task_assignments\n                (task_node_id, principal_id, principal_kind, principal_tag,\n                 role, stage, is_active, assigned_by, assigned_at)\n            VALUES\n                (:tid, :pid, :pkind, :ptag, :role, :stage, TRUE,\n                 :assigned_by, now())\n            '), {'tid': task_id, 'pid': pid, 'pkind': pkind, 'ptag': ptag, 'role': role, 'stage': stage, 'assigned_by': actor.id if actor.kind != 'system' else None})

def _next_event_seq(session: Session, *, task_id: int) -> int:
    row = session.execute(text('SELECT COALESCE(MAX(event_seq), 0) + 1 FROM task_state_transitions WHERE task_node_id = :tid'), {'tid': task_id}).first()
    return int(row[0]) if row and row[0] is not None else 1

def _update_node_state(session: Session, *, task_id: int, new_state: str, expected_version: int, extra_updates: Optional[Dict[str, Any]]=None) -> int:
    ...
    """
    patch: Dict[str, Any] = {'current_state': new_state}
    if extra_updates:
        patch.update(extra_updates)
    patch_json = json.dumps(patch, ensure_ascii=False)
    row = session.execute(text("\n            UPDATE nodes\n               SET attributes = jsonb_set(\n                       attributes || CAST(:patch AS jsonb),\n                       '{state_version}',\n                       to_jsonb(((attributes->>'state_version')::int + 1)))\n             WHERE id = :tid\n               AND type_code = 'task'\n               AND (attributes->>'state_version')::int = :expected\n            RETURNING (attributes->>'state_version')::int\n            "), {'tid': task_id, 'patch': patch_json, 'expected': expected_version}).first()
    if row is None:
        raise OptimisticLockError(f'task {task_id}: expected_version={expected_version} did not match')
    return int(row[0])

def _insert_state_transition(session: Session, *, task_id: int, event_seq: int, idempotency_key: Optional[str], from_state: str, to_state: str, event: str, actor: Principal, stage: str, reason: Optional[str], correlation_id: Optional[str], trace_id: Optional[str], metadata: Optional[Dict[str, Any]]) -> None:
    metadata = metadata or {}
    metadata.setdefault('_schema_version', 1)
    session.execute(text("\n            INSERT INTO task_state_transitions\n                (task_node_id, event_seq, idempotency_key, idempotency_expires_at,\n                 from_state, to_state, event,\n                 actor_principal_id, actor_principal_kind, stage, reason,\n                 correlation_id, trace_id, metadata, created_at)\n            VALUES\n                (:tid, :seq, :idem,\n                 CASE WHEN :idem IS NULL THEN NULL ELSE now() + INTERVAL '7 days' END,\n                 :from_state, :to_state, :event,\n                 :actor_id, :actor_kind, :stage, :reason,\n                 :corr, :trace, CAST(:metadata AS jsonb), now())\n            "), {'tid': task_id, 'seq': event_seq, 'idem': idempotency_key, 'from_state': from_state, 'to_state': to_state, 'event': event, 'actor_id': actor.id if actor.kind != 'system' else None, 'actor_kind': actor.kind, 'stage': stage, 'reason': reason, 'corr': correlation_id, 'trace': trace_id, 'metadata': json.dumps(metadata, ensure_ascii=False)})

def _insert_outbox(session: Session, *, task_id: int, pool_key: Optional[str], event_kind: str, payload: Dict[str, Any], correlation_id: Optional[str], trace_id: Optional[str]) -> None:
    payload = dict(payload)
    payload.setdefault('_schema_version', 1)
    session.execute(text('\n            INSERT INTO task_outbox\n                (task_node_id, pool_key, event_kind, payload,\n                 correlation_id, trace_id, created_at)\n            VALUES\n                (:tid, :pool_key, :kind, CAST(:payload AS jsonb),\n                 :corr, :trace, now())\n            '), {'tid': task_id, 'pool_key': pool_key, 'kind': event_kind, 'payload': json.dumps(payload, ensure_ascii=False), 'corr': correlation_id, 'trace': trace_id})

def _load_pool(session: Session, *, pool_id: int) -> Dict[str, Any]:
    row = session.execute(text('\n            SELECT id, key, is_active, publish_acl, consume_acl,\n                   default_workflow_ref, default_visibility, default_priority\n              FROM task_pools\n             WHERE id = :pid\n            '), {'pid': int(pool_id)}).first()
    if row is None:
        raise PoolNotFound(f'task_pool id={pool_id} not found')
    return {'id': int(row[0]), 'key': row[1], 'is_active': bool(row[2]), 'publish_acl': row[3] if isinstance(row[3], dict) else json.loads(row[3] or '{}'), 'consume_acl': row[4] if isinstance(row[4], dict) else json.loads(row[4] or '{}'), 'default_workflow_ref': row[5] if isinstance(row[5], dict) else json.loads(row[5] or '{}'), 'default_visibility': row[6], 'default_priority': row[7]}

def _load_pool_by_key(session: Session, *, pool_key: str) -> Dict[str, Any]:
    row = session.execute(text('SELECT id FROM task_pools WHERE key = :k'), {'k': pool_key}).first()
    if row is None:
...
def _handle_publish_event(session: Session, *, task_id: int, actor_principal: Principal, payload: Dict[str, Any], attrs: Dict[str, Any], stage: str) -> _EventEffects:
    del stage
    ...
    if not decision.allow:
        raise PublishAclDenied(f"actor (id={actor_principal.id}, kind={actor_principal.kind}) failed publish_acl on pool {pool['key']}: {decision.reason}")
    row = session.execute(text("\n            SELECT 1 FROM task_assignments\n             WHERE task_node_id = :tid AND is_active AND role = 'executor'\n             LIMIT 1\n            "), {'tid': task_id}).first()
    if row is not None:
        raise AlreadyClaimedError(f'task {task_id} already has an active executor; cannot publish')
    return _EventEffects(extra_node_updates={'pool_id': pool['id'], 'assignee_kind': 'pool'}, outbox_pool_key=pool['key'], post_assignments=[], retire_roles=[], metadata={'from_pool_id': attrs.get('pool_id'), 'to_pool_id': pool['id'], 'pool_key': pool['key']})

def _handle_claim_event(session: Session, *, task_id: int, actor_principal: Principal, payload: Dict[str, Any], attrs: Dict[str, Any], stage: str) -> _EventEffects:
    del payload
    pool_id = attrs.get('pool_id')
    if pool_id is None:
        raise PoolNotFound(f'task {task_id} has no pool_id; cannot evaluate consume_acl')
    pool = _load_pool(session, pool_id=int(pool_id))
    if not pool['is_active']:
        raise PoolInactive(f"pool {pool['key']} is not active")
    decision = evaluate_acl(actor_principal, pool['consume_acl'])
    if not decision.allow:
        raise ConsumeAclDenied(f"actor (id={actor_principal.id}, kind={actor_principal.kind}) failed consume_acl on pool {pool['key']}: {decision.reason}")
    return _EventEffects(extra_node_updates={}, outbox_pool_key=pool['key'], post_assignments=[{'role': 'executor', 'stage': stage, 'principal_kind': actor_principal.kind, 'principal_id': actor_principal.id}], retire_roles=[], metadata={})

def _handle_assign_event(session: Session, *, task_id: int, actor_principal: Principal, payload: Dict[str, Any], attrs: Dict[str, Any], stage: str) -> _EventEffects:
    del actor_principal
    ...
    if target_kind != 'group' and target_id is None:
        raise PreconditionFailed('assign requires payload.principal_id (or principal_kind=group with principal_tag)')
...
def _handle_complete_event(session: Session, *, task_id: int, actor_principal: Principal, payload: Dict[str, Any], attrs: Dict[str, Any], stage: str) -> _EventEffects:
    ...
    if isinstance(cs, dict):
        ...
        if total > 0 and terminal != total:
            raise PreconditionFailed(f'task {task_id} has {total - terminal} non-terminal children')
    pool_key: Optional[str] = None
    pool_id = attrs.get('pool_id')
    if pool_id is not None:
        pool_key = _load_pool(session, pool_id=int(pool_id))['key']
...
def create_task(*, title: str, actor: Principal, workflow_key: str='default_v1', pool_id: Optional[int]=None, priority: str='normal', visibility: str='private', assignee_kind: str='user', scope_selector: Optional[Dict[str, Any]]=None, tags: Optional[list[str]]=None, due_at: Optional[datetime]=None, correlation_id: Optional[str]=None, trace_id: Optional[str]=None, idempotency_key: Optional[str]=None, db_session: Optional[Session]=None) -> TransitionResult:
    ...
    with _transaction(db_session) as session:
        replay = _check_create_idempotency(session, actor=actor, idempotency_key=idempotency_key)
        ...
        type_id_row = session.execute(text("SELECT id FROM node_types WHERE type_code = 'task'")).first()
        if type_id_row is None:
            raise WorkflowDefinitionNotFound("node_types.type_code='task' missing; run schema migration first")
        ...
        if pool_id is not None:
            pool_key = _load_pool(session, pool_id=int(pool_id))['key']
        _insert_outbox(session, task_id=task_id, pool_key=pool_key, event_kind=_OUTBOX_EVENT_KIND['create'], payload={'task_id': task_id, 'from_state': '__init__', 'to_state': initial_state, 'event': 'create', 'workflow_ref': attributes['workflow_ref'], 'actor': {'id': actor.id, 'kind': actor.kind}}, correlation_id=correlation_id, trace_id=trace_id)
        return TransitionResult(task_id=task_id, from_state='__init__', to_state=initial_state, event='create', event_seq=event_seq, state_version=1, idempotent_replay=False, correlation_id=correlation_id, trace_id=trace_id)
...
def transition(task_id: int, event: str, actor_principal: Principal, expected_version: int, *, idempotency_key: Optional[str]=None, correlation_id: Optional[str]=None, trace_id: Optional[str]=None, payload: Optional[Dict[str, Any]]=None, db_session: Optional[Session]=None) -> TransitionResult:
    ...
    try:
        with _transaction(db_session) as session:
            replay = _check_idempotency(session, task_id=task_id, idempotency_key=idempotency_key)
            if replay is not None:
