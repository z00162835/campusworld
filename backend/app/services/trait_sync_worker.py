...
def _sync_node_type(session, type_code: str) -> int:
    ...
    updated = session.query(Node).filter(Node.type_code == type_code).update({Node.trait_class: nt.trait_class, Node.trait_mask: nt.trait_mask}, synchronize_session=False)
...
def _sync_relationship_type(session, type_code: str) -> int:
    ...
    updated = session.query(Relationship).filter(Relationship.type_code == type_code).update({Relationship.trait_class: rt.trait_class, Relationship.trait_mask: rt.trait_mask}, synchronize_session=False)
    return int(updated or 0)
...
def process_trait_sync_jobs(limit: int=100) -> Dict[str, int]:
    ...
    with db_session_context() as session:
        jobs = session.query(TraitSyncJob).filter(TraitSyncJob.status.in_(['pending', 'failed'])).order_by(TraitSyncJob.created_at.asc()).limit(int(limit)).all()
        for job in jobs:
            ...
            try:
                ...
                logger.info('trait_sync_job_done', event='trait.update.type_trait_synced', domain=job.domain, type_code=job.type_code, updated=changed, before_trait_class=pl.get('before_trait_class'), after_trait_class=pl.get('after_trait_class'), before_trait_mask=pl.get('before_trait_mask'), after_trait_mask=pl.get('after_trait_mask'))
                ...
                job.finished_at = datetime.utcnow()
                pl = job.payload if isinstance(job.payload, dict) else {}
                logger.error('trait_sync_job_failed', event='trait.update.type_trait_sync_failed', domain=job.domain, type_code=job.type_code, retries=job.retries, error=str(exc), before_trait_class=pl.get('before_trait_class'), after_trait_class=pl.get('after_trait_class'), before_trait_mask=pl.get('before_trait_mask'), after_trait_mask=pl.get('after_trait_mask'))
            finally:
                job.updated_at = datetime.utcnow()
        session.commit()
    return counters
