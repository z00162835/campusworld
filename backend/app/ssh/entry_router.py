...
class EntryRouter:
    ...
    def resolve_post_auth_destination(self, user_node: Any) -> RouteDecision:
        ...
        if not active_world:
            return RouteDecision(target_kind='singularity', reason='no_active_world')
        if not self._is_world_available(active_world):
            self.logger.warning(f'World unavailable, falling back to singularity room: {active_world}', extra={'event_type': 'entry_route_world_unavailable', 'world': active_world})
            return RouteDecision(target_kind='singularity', reason='world_unavailable')
        return RouteDecision(target_kind='world', reason='resume_active_world', world_name=active_world, world_spawn_key=last_world_location or 'campus')
