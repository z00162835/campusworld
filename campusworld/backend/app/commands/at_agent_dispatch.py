...
from app.commands.agent_commands import _find_agent_by_service_id
...
def try_dispatch_at_line(command_line: str, context: CommandContext) -> Optional[CommandResult]:
    ...
    rest = line[1:].strip()
    if not rest:
        return CommandResult.error_result("usage: @<handle> <message>")
    ...
    if ref:
        ...
        if not decision.allowed:
            return CommandResult.error_result("Permission denied for @ agent")

    if not context.db_session:
        return CommandResult.error_result("database session required")

    node = _find_agent_by_service_id(context.db_session, handle)
    if node is None:
        return CommandResult.error_result(f"no npc_agent with service_id={handle!r}")
