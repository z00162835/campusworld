...
class GameLoader:
    ...
    def _load_game_module(self, game_name: str, game_path: Path) -> Optional[Any]:
        ...
        try:
            ...
            # 添加游戏路径到Python路径
            game_parent = str(game_path.parent)
            if game_parent not in sys.path:
                sys.path.insert(0, game_parent)
            ...
            if spec and spec.loader:
                ...
                self.logger.debug(f"游戏模块 '{game_name}' 加载成功")
                return module
            else:
    ...
    def _create_game_instance(self, game_name: str, game_module: Any) -> Optional[BaseGame]:
        ...
        try:
            ...
            if not game_class:
                self.logger.error(f"游戏模块 '{game_name}' 中找不到 'Game' 类")
                return None
            
            # 检查是否为BaseGame的子类
            if not issubclass(game_class, BaseGame):
                self.logger.error(f"游戏类 '{game_name}' 不是 BaseGame 的子类")
                return None
            
            # 创建游戏实例
            game_instance = game_class()
            
            self.logger.debug(f"游戏实例 '{game_name}' 创建成功")
            return game_instance
    ...
    def unload_game(self, game_name: str) -> bool:
        ...
        try:
            ...
            if self.engine.unregister_game(game_name):
                # 清理本地引用
                game_instance = self.loaded_games.pop(game_name)
                game_module = self.game_modules.pop(game_name, None)
                
                # 停止游戏
                if hasattr(game_instance, 'stop'):
                    game_instance.stop()
