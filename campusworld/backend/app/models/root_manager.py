...
class RootNodeManager:
    ...
    def _create_singularity_room(self, session: Session) -> Optional[Node]:
        ...
        try:
            ...
            # 创建SingularityRoom实例（禁用自动同步，避免在创建阶段写入 nodes 表导致重复插入/uuid 冲突）
            ...
            node = Node(
                ...
                description=(singularity_room._node_attributes or {}).get('room_description', ''),
                is_active=True,
                is_public=True,
                access_level='normal',
                attributes=singularity_room._node_attributes,
                tags=getattr(singularity_room, "_node_tags", []) or []
            )
            ...
            if session is None:
                ...
                with db_session_context() as new_session:
                    if self._root_node_id:
                        root_node = new_session.query(Node).filter(
