...
def test_move_prefers_local_exit_over_cross_world_bridge():
    ...
    assert user.location_id == 55


def test_move_out_from_world_entry_gate_teleports_to_root():
    """Semantic exit: at world's entry room, ``out`` returns to singularity without a graph edge."""
    cmd = MovementCommand()
    user = MagicMock()
    user.id = "1"
    user.location_id = 10
    user.attributes = {"active_world": "hicampus", "world_location": "hicampus_gate"}
    room = MagicMock()
    room.id = 10
    room.type_code = "room"
    room.attributes = {"world_id": "hicampus", "package_node_id": "hicampus_gate"}
    root = MagicMock()
    root.id = 99
    ...
    with (
        patch("app.commands.game.direction_command.db_session_context", return_value=_session_cm(session)),
        patch("app.commands.game.direction_command.room_is_world_entry_gate", return_value=True),
        patch("app.commands.game.direction_command.root_manager") as rm,
        patch("app.ssh.game_handler.teleport_account_to_root") as tp,
    ):
        rm.ensure_root_node_exists.return_value = True
        rm.get_root_node.return_value = root
        ok, msg, err = cmd._move("1", "out")

    assert ok is True
    assert err is None
    assert "奇点" in msg
    tp.assert_called_once_with(session, user, root)
    session.commit.assert_called_once()
    assert n["i"] == 2


def test_move_wrong_direction_at_entry_gate_lists_out_in_available_dirs():
    """Error hint includes semantic ``out`` at world entry gate."""
    cmd = MovementCommand()
    user = MagicMock()
    user.attributes = {"active_world": "hicampus", "world_location": "hicampus_gate"}
    room = MagicMock()
    ...
    room.attributes = {"world_id": "hicampus", "package_node_id": "hicampus_gate"}
    rel = MagicMock()
    rel.attributes = {"direction": "north"}
    dest = MagicMock()
    dest.attributes = {"world_id": "hicampus", "package_node_id": "next"}
