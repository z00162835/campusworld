...
| US-003 | 作为用户（管理员/操作员/开发者/用户）使用CLI登陆系统后type /后可通过up或down按键选择系统对应提供的命令               | P0  | 各类用户登陆后 type `/` 打开命令面板（Textual 8 通过 Provider `discover()` 列出命令），可选中后执行；也可用 Tab 对当前输入做前缀补全                                      |
...
- **WebSocket** - 与后端实时通信
- **httpx** - 异步 HTTP（登录 `/api/v1/auth/login`）
- **asyncio** - 异步 I/O
...
- **用户名与 SSH 一致**：图账号 `account` 节点的名称（`Node.name`），与 SSH 登录相同；**不是** REST `/auth/register` 所用的邮箱。
- **密码**：交互式登录使用不回显输入。无 TTY 时仅可使用环境变量 `CAMPUS_PASSWORD`（有泄露风险，限于 CI/自动化）。
- **JWT**：获取后即代表已认证会话；请勿复制或共享 token。
- **token_file**：若该文件存在且非空，将先用其中 token 尝试连接 WebSocket；失败则提示重新输入用户名与密码。登录成功后会以 **0600** 权限写回 token。
- **落点与 SSH 一致**：WebSocket 认证成功后，若图账号尚无 `location_id`，服务端会执行与 SSH 相同的入口路由（`spawn_user`），保证 `look` 等命令能解析当前房间。
- **命令列表来源**：连接后客户端会向服务端请求补全列表；后端在 **HTTP 应用启动时** 即注册命令（不再依赖「先连一次 SSH」）。若欢迎区出现 “no command names” 提示，请检查后端日志中的 `initialize_commands` / 数据库策略种子是否失败。
- **生产环境**：设置 `server.use_ssl: true`，使用系统信任的 TLS 证书。
