...
F04 requires **`@<handle> <payload>`** to reach **`npc_agent`** instances from any room, with handle resolution (**`service_id`**, optional **`handle_aliases`**, uniqueness, **`enabled`**) and authorization aligned with **`agent_nlp`**.
...
2. **Authorization** for `@` lines uses **`authorize_command(agent_nlp)`**, so policy and audit paths match **`agent_nlp`** / **`aico`** (registered shorthands).

3. **Handle resolution** is centralized in **[`resolve_npc_agent_by_handle`](../../../backend/app/commands/npc_agent_resolve.py)** and reused by **`at_agent_dispatch`**, **`agent_nlp`**, **`aico`**, and other agent commands that resolve by `service_id` / alias.
...
- **`help`** listings do not need to show `@` as a command name; users discover **`agent_nlp`**, **`aico`**, and F04 docs.
