...
**Non-goal**：`--all` 不会把 `attributes` / `tags` / `trait_mask` / 向量列塞进 `data.results[*]`。per-node 展开请用 `describe <id>`。
...
| 返回字段      | 封闭 5 列（见 §6）                     | `attributes` 预览（≤12 key）+ 出边采样（≤8）     |
| 写操作        | 否                                     | 否                                               |
...
- [x] 文档层（本 plan 范围）
  - [x] `docs/commands/SPEC/features/F01_FIND_COMMAND.md` 存在且覆盖 §1–§13。
  - [x] `docs/commands/SPEC/SPEC.md` 在命令清单中列 `find`/`describe` 并链接本文件。
