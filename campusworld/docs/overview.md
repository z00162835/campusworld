...
CampusWorld 是新一代智慧园区操作系统，基于原生 AI 架构设计。它借鉴 MUD（Multi-User Dungeon）游戏世界的设计原理，通过全图数据结构构建知识本体，实现物理世界设备、系统、人的语义孪生与交互映射。
...
| 模块 | 路径 | 说明 |
|------|------|------|
| SSH 服务器 | `backend/app/ssh/` | 基于 Paramiko 的 SSH 服务器，处理游戏终端连接 |
| 命令系统 | `backend/app/commands/` | 继承自 `BaseCommand` 的命令体系，支持自动注册 |
| 游戏引擎 | `backend/app/game_engine/` | 游戏逻辑核心，支持多游戏内容包 |
