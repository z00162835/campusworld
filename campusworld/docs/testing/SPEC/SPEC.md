...
| **F03 AICO（快速）** | `conda run -n campusworld pytest tests/commands/test_agent_f02_commands.py tests/game_engine/test_agent_llm_config.py tests/game_engine/test_llm_pdca_framework.py tests/services/test_ltm_semantic_retrieval.py -m unit -q` | Agent NLP、`prompt_overrides` / `model_config` 合并、PDCA LLM、LTM tick 摘要拼接（mock）；不含 SSH。 |
