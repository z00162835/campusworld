...
- [系统架构](./architecture/README.md) — 技术栈、多端接入、Redis/可观测性说明（含 `campus` CLI 实验态说明）
...
| 日志英文术语表（SPEC/实现引用） | [glossary/TERMINOLOGY.md](./glossary/TERMINOLOGY.md) |
...
- **日志**：`backend/app/core/log/manager.py`；英文消息与术语见 [glossary/TERMINOLOGY.md](./glossary/TERMINOLOGY.md)，代理约束见根目录 `AGENTS.md`
...
- 命名与落点规则：[`standards/DOC_NAMING_SPEC.md`](./standards/DOC_NAMING_SPEC.md)
- 规划与 TODO 治理：[`standards/PLAN_TODO_GOVERNANCE.md`](./standards/PLAN_TODO_GOVERNANCE.md)
- 布局审计快照：[`standards/SPEC_LAYOUT_AUDIT.md`](./standards/SPEC_LAYOUT_AUDIT.md)
...
- `docs/README.md`：全局文档路由与模块索引，不承载模块细节正文。
- `docs/architecture/README.md`：架构约束与治理原则正文。
- `AGENTS.md`：面向 Agent 的执行规则与权威入口清单。
- [`docs/standards/DOC_NAMING_SPEC.md`](./standards/DOC_NAMING_SPEC.md)：文档命名与目录落点的唯一规范来源。
- [`docs/standards/PLAN_TODO_GOVERNANCE.md`](./standards/PLAN_TODO_GOVERNANCE.md)：规划文档、TODO、ACCEPTANCE 的统一治理规则。
...
- `docs/` 正文默认中文。
- SPEC 结构与命名校验命令：`cd backend && python scripts/validate_spec_layout.py`
