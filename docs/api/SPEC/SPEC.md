...
**已实现** — 世界交互与语义地图（挂载在 **`/api/v1`**；已认证；契约见 [`docs/frontend/SPEC/features/F01_CAMPUSWORLD_NextUI.md`](../../frontend/SPEC/features/F01_CAMPUSWORLD_NextUI.md) §8）：

| 前缀 | 端点 | 方法 | 说明 |
|------|------|------|------|
| `/world-sessions` | `/world-sessions/current` | GET | 首屏聚合（含默认 `viewLayer=room` 的 `focus_map`） |
| `/world-sessions` | `/world-sessions/enter-world`、`/leave-world` | POST | 进入/离开世界 |
| `/decision-center` | `/decision-center/actions` | POST | 决策动作 |
| `/decision-center` | `/decision-center/query` | POST | 决策查询（含 `space` 命令路径） |
| `/semantic-map` | `/semantic-map/focus` | GET | 语义地图焦点；query: `view_layer`, `anchor_id?`, `mode?`, `selected_entity_id?` |
| `/semantic-map` | `/semantic-map/actions` | POST | 地图动作：`drill`、`select`（高亮 + `space_summary` / `entity_inspect`） |
| `/semantic-map` | `/semantic-map/space-summary` | GET | 空间摘要（`space` 命令 SSOT） |
| `/semantic-map` | `/semantic-map/entity-inspect` | GET | 实体 inspect（look SSOT）；query: `node_id?`, `agent_id?` |
| `/semantic-map` | `/semantic-map/query` | POST | 语义地图查询，返回 `map_patch` |
| `/world-search` | `/world-search` | POST | 全局搜索 |
| `/world-history` | `/world-history/summary` | GET | 历史摘要（结构化分组） |
| `/world-history` | `/world-history/conversations/archive` | POST | 归档当前会话 AICO/Command 对话 |

### World history summary (`GET /world-history/summary`)
...
|------|------|------|
| `id` | string | 条目唯一 ID |
| `title` | string? | 展示标题；旧版仅含 `summary` 时客户端应回退解析 |
| `messageCount` | number? | 消息条数（command 条目含 assistant 时为 2） |
| `preview` | string? | AICO 线程预览文本 |
| `detail` | string? | command 条目详情/回复 |
| `sequence` | number? | 同 `createdAt` 归档批次内的顺序（越大越新；command 组返回时已倒序） |
| `createdAt` | string | ISO 8601 时间戳 |
| `summary` | string? | **已废弃**；仅兼容旧响应 |

**`WorldHistoryGroup` 字段**：`id`（如 `location`、`aico_conversations`、`command_conversations`）、`title`、`items[]`。

**`/world-history/conversations/archive` 请求体**（结构化 Pydantic；`extra` 字段拒绝；客户端须发送专用 DTO，不得透传 UI 消息对象）：

| 字段 | 约束 |
|------|------|
| `aico_threads[]` | 最多 20 条；每项含 `id`, `title`, `messages`, `updatedAt` |
| `aico_threads[].messages[]` | 最多 50 条（与前端会话 FIFO 上限一致） |
| `command_conversation[]` | 最多 50 条；仅 `id`, `role`, `mode`, `query?`, `answer` |
| 文本字段 | `query` / `answer` / `title` 等有长度上限 |
| 单批 payload | 服务端上限 512KB；超限 **422** |

**持久化**：归档批次写入 account 节点 **`owns`** 的 `world_conversation_archive` 图节点；`attributes.history_summary` 预存摘要项，`GET /world-history/summary` 默认只读摘要（`limit`/`offset` 分页，默认 50，最大 100），不展开完整消息 JSONB。单 account 归档批次数上限见 `world_interaction.max_archives_per_user`（默认 100）；写入时对 account 行加锁以防并发穿透。
