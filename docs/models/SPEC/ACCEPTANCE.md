...
- [x] ToolObservation 默认按命令风险采用 `full` / `summary`，并允许 `system_command_ability.attributes.agent_observation_policy` 覆盖；未知命令与 `semantic_pending=true` 命令默认 `summary`。
...
- [x] ToolObservation policy 在单次 gather 内按命令名缓存；重复命令不重复解析 ability policy。
