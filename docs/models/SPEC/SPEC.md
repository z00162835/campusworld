...
- **World environment（`trait_class: ENV`）**: `world_environment` 为每 world 单例的宏观天气/温湿度节点；`location_id` 指向 `world`；户外 room（`environment:outdoor` tag）在 `look` 时读取该节点。HiCampus 规范见 [`../../games/hicampus/SPEC/features/F09_WORLD_ENVIRONMENT.md`](../../games/hicampus/SPEC/features/F09_WORLD_ENVIRONMENT.md)。
- **World Default Spawn**: 某个世界内部默认出生点（例如 `hicampus/hicampus_gate`）。
...
- 世界内 spawn（如 `hicampus/hicampus_gate`）属于世界语义，不应覆盖系统入口语义。
...
- `F13` AICO 命令、交互式会话与流式体验（`aico -i` REPL、`-l`/`-d`/`-his`、`!`、NDJSON 流式与能力协商；**仅**内置 `aico`/`@aico`；与 [**F03**](features/F03_AICO_DEFAULT_SYSTEM_ASSISTANT.md) / [**F04**](features/F04_AT_AGENT_INTERACTION_PROTOCOL.md) / [**F08**](features/F08_AICO_TOOL_CONTEXT_AND_AGENT_LOOP.md) / [**F10**](features/F10_AICO_PERFORMANCE_AND_LATENCY.md) / [**F12**](features/F12_NLP_AGENT_MULTI_TURN_SESSION_MEMORY.md) 衔接）  
  [`features/F13_AICO_INTERACTIVE_UPGRADE.md`](features/F13_AICO_INTERACTIVE_UPGRADE.md)
...
- `F15` Agent Skill Registry & Injection（L4 经验 Skill 层）：单文件 `SKILL.md`+frontmatter（对标 Anthropic Agent Skills），**渐进式披露**（每阶段注入 eligible manifest：L1 `name`+`description` / L2 body 仅激活时注入），`SkillDefinition`/`SkillRegistry`/`SkillRunner`/`SkillInjection`，`activation_mode`（`phase_mapped`/`model_selected`），`skill_refs` 节点引用，`allowed_tool_groups` v1 占位不强制；Skill 内容注入 **user/input context 通道**（`skill-context`，**不**进平台 system message，优先级 < 平台 system/安全/command policy）；v1 启动期缓存完整源码（无 hot reload），确定性阶段映射，目标 model 经 `description` 自选（[**F17**](features/F17_AGENT_STATE_MACHINE.md) `selected_skill`）；skill 文本纳入 `prompt_fingerprint`，trace 含 `definition_hash`。落地 [**F09**](features/F09_CAMPUSWORLD_AGENT_ARCHITECTURE_FOUR_LAYERS.md) §6.4。  
