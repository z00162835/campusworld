...
| D9 | Unknown commands and `semantic_pending=true` commands default to ToolObservation `summary`; only explicitly modeled `document/read` commands default to `full`. |
...
- [x] Default unknown / semantic-pending command observations to `summary`; keep explicitly modeled read/document commands on `full`.
