...
**交叉引用：** [**F09**](F09_CAMPUSWORLD_AGENT_ARCHITECTURE_FOUR_LAYERS.md)（CampusWorld Agent 四层架构：L1–L4、映射与边界）、[**F03**](F03_AICO_DEFAULT_SYSTEM_ASSISTANT.md)（AICO 实例、`tool_allowlist`、PDCA + LLM 基线）、[**F10**](F10_AICO_PERFORMANCE_AND_LATENCY.md)（tick 延迟 SLO、ToolGather 耗时与工具路径单点收敛）、[**F02**](F02_INTELLIGENT_AGENT_SERVICE_TYPE.md)（`npc_agent`、运行记录）、[**F04**](F04_AT_AGENT_INTERACTION_PROTOCOL.md)（`@<handle>`）、[**F05**](F05_AGENT_COMMAND_LIST_AND_STATUS.md)（Agent 列表/状态）、[**F06**](F06_CAMPUSLIBRARY_KNOWLEDGE_WORLD.md)（CampusLibrary 知识检索，与本特性互补）、[**F12**](F12_NLP_AGENT_MULTI_TURN_SESSION_MEMORY.md)（会话 STM、LTM、compaction；**每用户+会话** 与 **系统 daemon 附身独占** 两类作用域；扩展 `memory_context` 语义）。
...
- **工具语义面向 Agent**：注册表中的命令名与参数应对 Agent **可读、可组合**（例如 `help`、`look`、自省类 **`agent tool`** / **`agent show`**）；候选集、组合策略与 **PDCA 各阶段是否调用工具** 由本节与 §6 规定 **策略位**（实现可配置，初稿给出 **v1 默认建议**）。
...
- 与 **F03** §5.5 **上下文整合** 的关系：**`memory_context`**（含 **LTM 检索**，以及 [**F12**](F12_NLP_AGENT_MULTI_TURN_SESSION_MEMORY.md) 规定的 **会话 STM**）与 **ToolObservation（命令观测）** 为注入源；合并顺序与优先级见 §6 与 F12 §7。
...
- **组合**：允许多条命令顺序执行（如 `help help` → `agent tool aico`）；**顺序**由 Plan 或 **preflight_policy** 决定；总条数与总字符 **封顶**（§8）。
...
- **`agent_run_records.command_trace`**：每条记录建议包含：`command_name`、`args` 摘要、`success`、`message_len`、按 ToolObservation policy 处理后的 `message_preview`、`observation_message_mode` 等；默认不记录内容哈希。
...
- **`agent tool <id>`** 所列工具与 **`tool_allowlist`**、策略过滤一致。
...
- Primer 文件：[`docs/models/SPEC/CAMPUSWORLD_SYSTEM_PRIMER.md`](../../CAMPUSWORLD_SYSTEM_PRIMER.md)；实现模块 [`system_primer_context.py`](../../../../backend/app/game_engine/agent_runtime/system_primer_context.py)。
