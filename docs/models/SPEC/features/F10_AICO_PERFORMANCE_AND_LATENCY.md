...
- **Tick 级汇总（对比版本或 ``phase_llm`` 变更）**：按 ``run_id`` / ``correlation_id`` 聚合启用了全链 DEBUG 时的 ``aico_http … elapsed_ms``，得到单次 tick **上游 HTTP 次数**与 **elapsed_ms 之和**，再算 **p50/p95**（可脚本化或按需抽样）。
...
| **Phase 1** | **纯配置**：`phase_llm.*.timeout_sec`、`max_tokens`、减小 `tool_gather_max_rounds_per_phase`；按需将 **`do` / `act` 等设为 `mode: skip`**（省 HTTP；**`do.skip`** 草稿见 [`AGENT_PDCA_PHASE_MERGE_TRADEOFFS.md`](../../AGENT_PDCA_PHASE_MERGE_TRADEOFFS.md) D1）。**注意：** 节点若 **省略** `phase_llm`，解析默认仍会跑 Do；**AICO 种子**才对 `do` 默认 `skip`（见该文档「Resolver defaults vs AICO seed」）。 |
...
- [x] 重复点 D1–D5：D4/D5（``_spec_for_phase``、``parse_bool_extra``）、D1/D2（``NpcAgentTickInputs`` / ``CallerGraphSnapshot``）、D3（``ToolRuntimeView``）已在代码路径落地；ADR 见 ``docs/architecture/adr/ADR-F10-AICO-Tool-Gather-Single-Gate.md``。
- [x] 工具单点判定（第 8 节）单测：``backend/tests/game_engine/test_tool_runtime_view.py``；``LlmPDCAFramework`` 端到端行为：``test_aico_harness_golden.py``、``test_llm_pdca_prompt_tuning.py`` 等。
...
| `nodes.attributes.phase_llm`（`npc_agent`） | 每 phase 的 `timeout_sec`、`mode: skip` 等 | 见 [`agent_node_phase_llm.py`](../../../../backend/app/game_engine/agent_runtime/agent_node_phase_llm.py) 与 F03 | 与 YAML 合并顺序见 `resolve_agent_llm_config`；**`do.mode: skip`** 时用户可见仅为 **`plan_out`**，Plan 段观测进 Check/trace，见 [`AGENT_PDCA_PHASE_MERGE_TRADEOFFS.md`](../../AGENT_PDCA_PHASE_MERGE_TRADEOFFS.md)（D1/D4）。 |
