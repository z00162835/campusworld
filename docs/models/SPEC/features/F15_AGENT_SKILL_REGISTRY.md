...
- **blocked** = 被 F16 PolicyEngine 在 `before_skill_activation` 拒绝（body 不注入，列入清单以明确 policy-denied 状态）
...
Skills are split by activation status this turn. Active skills' guidance applies; inactive skills are listed for awareness only; blocked skills were denied by policy and are not active this turn.
...
### Blocked by policy
- **<name>** — <description> (reason: <reason_code>)
```
...
- **Q1（收敛强度）：** 已决策：F16 `before_tool_call` 对 `SkillActivation.allowed_tool_groups` 做硬过滤；F15 v1 仍为占位（trace 不含 group 违规行），但 F16 实现后 `allowed_tool_groups` 即升级为硬约束。
- **Q2（组词汇）：** 已决策：F16 v1 引入细化 `tool_group` 词汇（`read`/`observe`/`agent_meta`/`identity`/`communicate`/`mutate`/`admin`），命令通过 `CommandToolSemantics.tool_groups` 声明；F15 seed skills 的 `allowed_tool_groups` 可逐步细化（如 `retrieval_reasoning` 从 `['read']` 扩展到 `['read', 'observe']`）。
