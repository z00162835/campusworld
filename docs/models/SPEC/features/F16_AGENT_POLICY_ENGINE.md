...
- [x] `evaluate_execution_gate` 外部 API + `GateDecision` + `reason_code` 集稳定，内部委托 PolicyEngine（D4）
...
- [ ] `before_tool_call` 读 `active_skill_context.allowed_tool_groups`，命令 tool group 不在并集时 deny（D3）；数据流协议见 §4.3；**P3 启用，P0/P1/P2 不启用**
