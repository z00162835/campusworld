...
| **F02 / PostgreSQL** | `conda run -n campusworld pytest -m postgres_integration` | 需可达 PostgreSQL + **pgvector**；含 `tests/db/test_agent_memory_tables.py`、`tests/commands/test_agent_f02_commands.py`（capabilities + **`run_npc_agent_nlp_tick` stub-LLM → `agent_run_records`**）、`tests/services/test_ltm_semantic_retrieval.py`（LTM 向量 + 链接扩展）等。迁移脚本含 **`ensure_multi_turn_conversation_memory_schema`**（多轮对话 STM / `agent_long_term_memory` 扩展列）；本地旧库请跑一次完整 migrate / `run_schema_migrations`。 |
| **F03 AICO（快速）** | `conda run -n campusworld pytest tests/commands/test_npc_agent_nlp.py tests/core/test_aico_observability_logging.py tests/game_engine/test_agent_llm_config.py tests/game_engine/test_llm_pdca_framework.py tests/game_engine/test_f12_stm_scope.py tests/services/test_ltm_semantic_retrieval.py -m unit -q` | `CommandResult` 形状与 **`run_npc_agent_nlp_tick`** passthrough（mock）；AICO 专用可观测日志与 hooks；`prompt_overrides` / `model_config` 合并、PDCA LLM、LTM tick 摘要拼接（mock）；F12 STM/compaction/plan 拼接辅助（`test_f12_stm_scope`）；**不含** `test_agent_f02_commands.py` 中需 PG 的用例。 |
...
| **F13 AICO 命令面（快速）** | `conda run -n campusworld pytest tests/commands/test_aico_f13.py tests/commands/test_f04_at_dispatch.py -q` | `-l` limit、`take_last_n_rounds`、`@aico` → `execute_aico_command`（mock）；不含 SSH REPL 集成。 |
